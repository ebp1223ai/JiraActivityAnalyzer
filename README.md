...
- Capacity is read from the exact App Server provider/model capability when available, with exact model metadata as a secondary source. Unsupported or missing data remains `unavailable`; the app does not substitute zero or a fixed 200,000-token limit.
- Unknown capacity or a known estimated overage opens one confirmation dialog with the complete UTF-8 byte estimator, input/output/reasoning/safety terms, required total, margin, overage, source, and snapshot SHA-256.
- Cancelling creates zero provider requests, threads, and turns. Confirming atomically permits one dispatch for the same `analysisRunId`; double clicks, re-renders, repeated IPC, route changes, timeout, parse errors, and incomplete responses do not retry.
- Only a completed provider turn with saved response, valid JSON, exact identity conservation, and exact Catalog validation can produce formal JSON, Golden HTML, or AI Analysis SQLite rows.
- Failed ChatGPT runs are permanently retained under `<APP_ROOT>/exports/ai-analysis/failed-staging/<analysisRunId>/`. Existing failed evidence is not overwritten or automatically removed, and the global Debug Folder includes the latest sanitized AI Analysis failed evidence.
- Provider usage remains actual or explicitly unavailable. Estimated prompt tokens are never represented as provider usage.

See [v0.3.12 implementation and verification](docs/v0.3.12-capacity-override-diagnostics.md).
## Earlier Releases
