...
async function run() {
  ...
  try {
    ...
    const provider = await callAiProvider({ ...environment.local, endpoint: `http://127.0.0.1:${address.port}/v1`, configFingerprint: "synthetic-fingerprint" }, "secret-value", "OK");
    server.close();
    ...
    console.log("v0.3.8 AI Analysis core tests passed");
