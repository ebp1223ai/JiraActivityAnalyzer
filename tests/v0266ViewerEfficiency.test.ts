...
assert.equal(viewerDiffPassesFilters(zeroAdded, { hideNoChange: false, hideZeroAdded: true, hideZeroDeleted: false, hideBeforeUnavailable: false }), true);
assert.equal(viewerDiffPassesFilters(zeroDeleted, { hideNoChange: false, hideZeroAdded: false, hideZeroDeleted: true, hideBeforeUnavailable: false }), true);
...
try {
  ...
  const noQuick = { hideNoChange: false, hideZeroAdded: false, hideZeroDeleted: false, hideBeforeUnavailable: false };
...
const migrated = normalizeUiPreferences({ userAllActivityEvents: { selectedUserIds: ["b", "a", "b", ""], userScopeMode: "all", diffQuickFilters: { hideNoChange: false, hideZeroAdded: true, hideZeroDeleted: false, hideBeforeUnavailable: false } } });
...
assert.match(quickUi, /Hide − = 0/);
assert.match(quickUi, /Hide Before unavailable/);
