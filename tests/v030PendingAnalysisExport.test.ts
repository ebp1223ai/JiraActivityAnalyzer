...
import { PENDING_ANALYSIS_CONTRACT_STATUS, PENDING_ANALYSIS_SCHEMA_NAME, PENDING_ANALYSIS_SCHEMA_VERSION, canonicalJson, sha256Canonical } from "../shared/pendingAnalysisContract.js";
...
async function run() {
  ...
  const userExport = await runPendingAnalysisExport({ exportId: "11111111-2222-4333-8444-555555555555", sourceView: "USER_ALL_ACTIVITY_EVENTS", query, userScope: scope, expectedFilteredCount: userView.filteredCount, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" });
  ...
  assert.equal(document.appVersion, "0.3.0");
  ...
  assert.equal(document.records[0].currentIssueContext.contextType, "CURRENT_SAVED_ISSUE_SNAPSHOT");
  assert.match(JSON.stringify(document), /測試使用者|之前|之後/);
  ...
  assert.ok(document.records.every((record: Record<string, any>) => record.evidenceIdentity.evidenceId.startsWith("pae_") && record.integrity.originalEvidenceAvailable));
  ...
  const issueExport = await runPendingAnalysisExport({ exportId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", sourceView: "ISSUE_ACTIVITY_EVENTS", query: targetQuery, issueKey: "SYNTH-300", expectedFilteredCount: issueTarget.filteredCount, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" });
  ...
  await assert.rejects(runPendingAnalysisExport({ exportId: "cancelled-run", sourceView: "ISSUE_ACTIVITY_EVENTS", query, issueKey: "SYNTH-300", expectedFilteredCount: issueView.filteredCount, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" }, { isCancelled: () => true }), (error: Error & { code?: string }) => error.code === "EXPORT_CANCELLED");
  await assert.rejects(runPendingAnalysisExport({ exportId: "count-mismatch", sourceView: "USER_ALL_ACTIVITY_EVENTS", query, userScope: scope, expectedFilteredCount: userView.filteredCount - 1, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" }), (error: Error & { code?: string }) => error.code === "COUNT_EXPORT_MISMATCH");
  let generationTouched = false;
  await assert.rejects(runPendingAnalysisExport({ exportId: "generation-change", sourceView: "ISSUE_ACTIVITY_EVENTS", query, issueKey: "SYNTH-300", expectedFilteredCount: issueView.filteredCount, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" }, { onProgress: (progress) => { if (!generationTouched && progress.status === "filtering") { generationTouched = true; const future = new Date(Date.now() + 10_000); fs.utimesSync(databasePath, future, future); } } }), (error: Error & { code?: string }) => error.code === "SOURCE_DATABASE_CHANGED");
    await assert.rejects(runPendingAnalysisExport({ exportId: "empty-empty", sourceView: "ISSUE_ACTIVITY_EVENTS", query: { ...query, filters: { actor: { values: ["missing"] } } }, issueKey: "SYNTH-300", expectedFilteredCount: 1, expectedDatabaseIdentity: "", databasePath, appRoot, appVersion: "0.3.0" }), (error: Error & { code?: string }) => error.code === "NO_FILTERED_RECORDS");
