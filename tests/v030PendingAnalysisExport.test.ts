...
async function run() {
  ...
  assert.equal(document.counts.changedRecordCount, document.counts.recordsWithDiffContentCount);
  assert.equal(document.counts.diffCoverageComplete, true);
  assert.equal(document.counts.diffHunkCount, document.records.reduce((total: number, record: any) => total + record.diff.diffHunks.length, 0));
  ...
  assert.equal(quickDocument.records.every((record: any) => record.diff.isSubstantiveChange && record.diff.addedLineCount > 0 && record.diff.beforeAvailability === "AVAILABLE"), true);
