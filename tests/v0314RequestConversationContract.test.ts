...
import { buildRequestPackage, loadRequestPackage, buildCoreAnalysisInstruction, PROVIDER_DELIVERY_MODE } from "../electron/aiAnalysisRequestPackageV0314.js";
...
assert.equal(request.requestPackage.inlineBlockCount, 0);
assert.equal(request.requestPackage.inlineFileContentCount, 0);
assert.equal(request.requestPackage.nativeInputFileCount, 0);
...
console.log("v0.3.15 Local File Workspace compatibility, conversation durability, response evidence, semantic matrix, and formal gate tests passed (43 assertions).");
