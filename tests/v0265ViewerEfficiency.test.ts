...
assert.ok(packageJson.version.split(".").map(Number).some((value, index, parts) => value > [0, 2, 65][index] && parts.slice(0, index).every((part, prior) => part === [0, 2, 65][prior])) || packageJson.version.split(".").map(Number).every((value, index) => value === [0, 2, 65][index]), "v0.2.65 regression must run on version 0.2.65 or newer");
