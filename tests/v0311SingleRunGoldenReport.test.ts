...
function providerResponse(compact: ReturnType<typeof buildCompactPayload>["payload"]) {
  return JSON.stringify({ schemaVersion: "ai-analysis-output-v3", records: compact.records.map((record) => {
...
test("Golden HTML is offline, escaped, interactive, printable, and conserves 117 records", () => {
  ...
  assert.deepEqual(validateGoldenHtml(html, 117), { valid: true, eventCount: 117, externalAssets: false, semanticSections: 14 });
