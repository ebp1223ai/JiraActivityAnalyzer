...
let workspacePath = "";
const providerRequest = (prompt: string, extra: Record<string, unknown> = {}) => ({ prompt, deliveryMode: "LOCAL_FILE_WORKSPACE" as const, workspacePath, allowedReadRoots: [workspacePath], finalProviderPayloadSha256: crypto.createHash("sha256").update(prompt, "utf8").digest("hex"), ...extra });
...
async function main() {
  ...
  workspacePath = path.join(temp, "workspace"); fs.mkdirSync(workspacePath);
  for (const name of ["pending-analysis.json", "common-rules.md", "skill-catalog.md", "rule-set-manifest.md"]) fs.writeFileSync(path.join(workspacePath, name), name === "pending-analysis.json" ? "{\"records\":[]}" : "# test", "utf8");
  const fake = path.join(root, "tests", "fixtures", "fake-codex-app-server.cjs");
  let opened = "";
  const signedOut = new ChatGptService({ testRuntimeExecutablePath: process.execPath, runtimeArgs: [fake, "signed_out"], openExternal: async (url) => { opened = url; } });
  ...
  const service = new ChatGptService({ testRuntimeExecutablePath: process.execPath, runtimeArgs: [fake, "connected"] });
  ...
  const cancelling = new ChatGptService({ testRuntimeExecutablePath: process.execPath, runtimeArgs: [fake, "cancel"] }); await cancelling.start();
  ...
  const crashing = new ChatGptService({ testRuntimeExecutablePath: process.execPath, runtimeArgs: [fake, "crash"] }); await crashing.start();
