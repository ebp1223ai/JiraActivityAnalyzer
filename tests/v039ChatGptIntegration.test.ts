...
async function main() {
  ...
  const cancelledRun = cancelling.runAnalysis({ prompt: "cancel" }); await waitFor(() => Boolean(cancellingRunId));
  ...
  await assert.rejects(crashing.runAnalysis({ prompt: "crash" }), /OUTCOME_UNKNOWN|CODEX_RUNTIME_EXITED/); assert.equal(crashing.getStatus().state, "runtime_error"); await crashing.stop();
