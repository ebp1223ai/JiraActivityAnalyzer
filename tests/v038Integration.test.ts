...
assert.equal(packageJson.version, read("VERSION").trim());
const [major, minor, patch] = packageJson.version.split(".").map(Number);
assert(major > 0 || minor > 3 || (minor === 3 && patch >= 8));
assert.match(read(".env.version"), /ENV_FORMAT_VERSION=4/);
...
assert.match(read("src/ai-analysis/AiAnalysisReconstructedPage.tsx"), /AI 連線與診斷/);
assert.match(read("src/ai-analysis/AiAnalysisReconstructedPage.tsx"), /分析工作區/);
assert.match(read("src/ai-analysis/AiAnalysisReconstructedPage.tsx"), /Activity Events 分析結果/);
assert.doesNotMatch(read("src/ai-analysis/AiAnalysisReconstructedPage.tsx"), /ANALYSIS DETAIL/i);
