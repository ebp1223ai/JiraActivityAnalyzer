# Skill Analysis Rule Set Manifest v0.8.1

- File: `Skill_Analysis_Rule_Set_Manifest_v0.8.1.md`
- Manifest Schema Version: `0.8.1`
- Previous Version: `0.8.0`
- Rule Set ID: `JAA-SKILL-RULESET-2026-08-24-DRAFT-081`
- Status: `review-draft`
- Prepared Date: `2026-08-24`
- Target Application: `JiraActivityAnalyzer`
- Target Application Version: `0.3.32`
- Prompt Locale: `zh-TW`
...
v0.8.1 延續 v0.8.0 的四檔選取、Attempt／Run、Active Result、Results、Decision v5、Evidence Quote Catalog 與 Debug lineage，不變更 279 Skills。本次進版：

1. Multi-skill independence 改以 Quote identity／source span 判定，不以父層 `evidenceRef` 判定。
2. 每個 Multi-skill Finding 至少需要一個 Exclusive PRIMARY_CHANGE Quote。
3. Boilerplate Gate 只檢查模型文字欄位，不檢查原始 Evidence Quote 本文。
4. `CATALOG_DETAIL_MISSING` 維持合法完成狀態，不得單獨造成 BLOCKED。
5. Validation receipts 必須反映真實 stage outcome，後續未執行階段不得標示 PASSED。
6. Decision v5 identity 必須端到端一致，禁止 v4 metadata 殘留。
7. `bridge-resumable-v4`、protected-token-safe segmentation 與 Bridge v13 維持 byte-identical。
8. v0.3.31 真實17筆 submission 作為 Quote-level Quality Gate 與 lifecycle replay evidence。
...
|---|---|---:|---|
| `rule_set_id` | `JAA-SKILL-RULESET-2026-08-24-DRAFT-081` | — | review-draft |
| `manifest_file` | `Skill_Analysis_Rule_Set_Manifest_v0.8.1.md` | runtime | 本檔載入時計算；不得自我綁定固定 Hash |
| `common_rules_version` | `1.6.1` | — | Quote-level Multi-skill Independence + Decision v5 |
| `common_rules_file` | `Skill_Classification_Common_Rules_v1.6.1.md` | 38785 | `b0ff832fbcc9d9f8e5f02903ed7351c7922ae746d04631ccb4334f5c70124659` |
...
| `classification_engine_version` | `JAA-CLASSIFICATION-1.6.1` | — | Quote-level Quality Gate + Decision v5 |
| `prompt_version` | `JAA-CHATGPT-ZH-TW-0.3.32` | runtime | effective instruction bytes／hash 逐 Run 保存 |
| `pipeline_version` | `JAA-ANALYSIS-PIPELINE-0.3.32` | runtime | deterministic validation／receipt truth／identity |
...
{
  "schemaVersion": "0.8.1",
  "ruleSetId": "JAA-SKILL-RULESET-2026-08-24-DRAFT-081",
  "status": "review-draft",
  "targetApplicationVersion": "0.3.32",
  "promptLocale": "zh-TW",
  ...
  "documents": [
    {
      ...
      "fileName": "Skill_Analysis_Rule_Set_Manifest_v0.8.1.md",
      "version": "0.8.1",
      "modelVisible": true,
    ...
    {
      "role": "COMMON_RULES",
      "fileName": "Skill_Classification_Common_Rules_v1.6.1.md",
      "version": "1.6.1",
      "bytes": 38785,
      "sha256": "b0ff832fbcc9d9f8e5f02903ed7351c7922ae746d04631ccb4334f5c70124659",
      "modelVisible": true
    },
    {
      "role": "SKILL_CATALOG",
  ...
  "contracts": {
    "classificationEngine": "JAA-CLASSIFICATION-1.6.1",
    "prompt": "JAA-CHATGPT-ZH-TW-0.3.32",
  ...
  "multiSkillIndependence": {
    "version": "jaa-multi-skill-quote-independence-v1",
    "authorityLevel": "QUOTE_IDENTITY_AND_SOURCE_SPAN",
    "parentEvidenceRefMayBeShared": true,
    "exclusivePrimaryQuoteRequiredPerSkill": true,
    "duplicateIdsWithIdenticalSpanCountAsSameQuote": true,
    "explanationCannotReplaceExclusiveQuote": true,
    "hostMutationAllowed": false,
    "missingExclusiveQuoteCode": "MULTI_SKILL_EXCLUSIVE_PRIMARY_QUOTE_MISSING"
  },
  "providerTransport": {
...
2. 本 Manifest
3. Common Rules v1.6.1
4. Skill Catalog v0.3.1

HTML Template v1.5.0 不送 ChatGPT。模型輸出為精確 N 筆 Decision v5 direct JSON array，每個 Skill Finding 只引用 `evidenceQuoteIds[]`，加上繁體中文 Analysis Report 與 final assistant summary。Multi-skill 時，每個 Skill 至少需要一個未被同 Record 其他 Skill 使用的 PRIMARY_CHANGE Quote。模型透過 opaque token 提交，不回傳 Run／source／rule identity，不負責 Package、統計、HTML、SQLite 或 shell 檔案 I/O。
...
- Target JAA v0.3.32。
- Common Rules 進版 v1.6.1，定義 Quote-level Multi-skill Independence。
- 同一父層 Activity Event／Comment／`evidenceRef` 的不同 Quote 可以分別支援不同 Skill。
- 每個 Multi-skill Finding 至少需要一個 Exclusive PRIMARY_CHANGE Quote。
- Boilerplate Gate 只檢查模型文字欄位，不再把來源 Quote 本文誤判為樣板文字。
- Validation Stage Receipt 進版 v2，禁止未執行階段標示 PASSED。
- 新增單一 `run_terminal` event 與 Quality Gate 專屬 root error。
- Decision v5、Catalog 279 Skills、HTML Template v1.5.0、Transport v4 與 Bridge v13 均不改語意。
