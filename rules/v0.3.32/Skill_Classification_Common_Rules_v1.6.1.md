...
- File: `Skill_Classification_Common_Rules_v1.6.1.md`
- Rules Version: `1.6.1`
...
- Quality Contract: `jaa-ai-analysis-quality-v3`
...
本版延續 v1.6.0 的 Evidence 權重、計分公式、Status 語意、Decision v5、Snapshot 與 Report Data Package 分層；本次只釐清 Quote-level Multi-skill Independence 與 Quality Gate 的權威判定粒度：

1. Multi-skill 獨立性以 frozen Evidence Quote Catalog 的 Quote identity 與 source span 判定，不得以父層 Activity Event／Comment 的 `evidenceRef` 判定。
2. 同一個 Activity Event、Comment 或 `evidenceRef` 可以支援多個 Skill，但每個 Skill Finding 至少要有一個未被同 Record 其他 Skill 使用的 `PRIMARY_CHANGE` Quote。
3. 不同 `evidenceQuoteId` 若實際指向完全相同的來源 span 與 quote hash，仍視為同一 Quote，不得用來湊出獨立證據。
4. 若某個 Skill 沒有專屬 PRIMARY_CHANGE Quote，模型應只保留最精確 Skill，或依狀態矩陣改為 `NEEDS_REVIEW`；JAA 不得自動刪除、合併或改寫 Skill。
5. Boilerplate Gate 只檢查模型撰寫的 Explanation、Negative Checks 與 Rationale；不得把原始 Evidence Quote 本文當成 boilerplate explanation。
6. `CATALOG_DETAIL_MISSING` 維持合法完成狀態；只要 Schema、Semantic 與 Quality Gate 通過，就不得僅因 Catalog 為 review-draft 而拒絕 Canonical Result。
7. Quality Gate 的 receipt 必須反映真實結果；後續未執行 stage 必須標為 `NOT_RUN_DUE_TO_PRIOR_FAILURE`，不得標示 `PASSED`。
8. 本版不變更 Decision v5 欄位、279 Skills、Evidence 權重、confidence enum、Report Data Package、HTML Template 或 SQLite schema。
...
- 每個 Skill Finding 至少必須有一個同 Record 其他 Skill 未引用的 `PRIMARY_CHANGE` Quote；這個 Quote 稱為 Exclusive Primary Quote。
...
4. 至少一個未被其他 Skill 使用的 Quote key，才滿足 Exclusive Primary Quote。
...
7. Explanation、Negative Checks 或 Rationale 即使不同，也不能取代 Exclusive Primary Quote。
8. JAA 只能驗證與回報，不得自動刪除 Skill、替換 Quote、合併 Finding、降狀態或修改模型原始 Decision。

若任一 Skill Finding 缺少 Exclusive Primary Quote，產生：

```text
MULTI_SKILL_EXCLUSIVE_PRIMARY_QUOTE_MISSING
```

Finding 必須指出 `recordIndex`、`skillId`、該 Skill 的 Primary Quote IDs、shared Quote keys 與缺少 exclusive key 的原因，不得只列出整筆所有 Skill ID。
...
11. Multi-skill 時，每個 Skill 至少引用一個未被同 Record 其他 Skill 使用的 PRIMARY_CHANGE Quote ID；若不同 ID 指向相同權威 Quote key，仍視為共用。
12. 無法提供專屬 Quote 時，模型應只保留最精確 Skill，或依狀態矩陣改為 `NEEDS_REVIEW`，不得複製相同 Quote 湊成多技能。
...
Exact duplicate ratio `> 0.50` 或單筆 Skill Finding `> 5` 的版本化預設行為是 Warning，不得自動改寫分類。`CATALOG_DETAIL_MISSING` 本身是合法完成狀態，不得單獨造成 BLOCKED。WARNING 未被 durable manual acceptance 前禁止 SQLite；BLOCKED 不可用一般接受操作繞過。
...
本節說明 v1.6.1 如何套用至 JiraActivityAnalyzer。Evidence 權重與計分公式維持 v1.1.0；本版補齊 Quote-level Multi-skill Independence、Quality Gate 真實狀態、Decision v5、Frozen Evidence Segment、Evidence Role、聚合驗證、診斷 HTML、Report Data Package、Snapshot reference、本機 HTML renderer 與跨 Run 共存邊界。
