# Skill Analysis Rule Set Manifest v0.8.3

- File: `Skill_Analysis_Rule_Set_Manifest_v0.8.3.md`
- Manifest Schema Version: `0.8.3`
- Previous Version: `0.8.2`
- Rule Set ID: `JAA-SKILL-RULESET-2026-08-24-DRAFT-083`
- Status: `review-draft`
...
v0.8.3 延續 v0.8.2 的 Multi-skill、Catalog Detail、Warning Gate、HTML lifecycle、四檔選取、Attempt／Run、Active Result、Decision v5、Evidence Quote Catalog 與 Debug lineage，不變更 279 Skills。本次依 v0.3.33 兩次真實 17 筆 Managed ChatGPT Run 進版：

1. 建立唯一且可驗證的 Provider lifecycle state machine。
2. `jaa_finalize_input_delivery` 成功時，Host 必須原子完成 `INPUT_DELIVERY_FINALIZED → INPUT_READY`，再回傳 immutable receipt。
3. `INPUT_READY → ANALYSIS_STARTED` 為合法 transition；不得再出現從 `INPUT_READING` 直接進入分析的矛盾。
4. Progress tool 為可重複、可恢復的 observability，不得因同值重送或暫時停滯終止 Run。
5. `jaa_get_delivery_status` 在 finalize 前後均為唯讀 idempotent；不得把合法查詢誤判為 handle replay。
6. 相同 binding 的 finalize 重送須回傳同一 receipt；只有不一致重播才 fail closed。
7. Bridge identity 進版為 `0.3.34-bridge-v14`；Transport 維持 `bridge-resumable-v4`。
8. Decision v5 的 `FAILED` 固定 `confidence=0`、`skillFindings=[]`、`unknownReasons=[]`；整體輸入問題必須是 Run-level failure，不得偽造 N 筆 FAILED。
9. Validation Receipt 必須遵守因果順序；Formal stage 失敗後的 Formal stage 不得顯示 PASSED。
10. root error、first failed stage、source／target state 與 derived consequence 必須一致。
11. 測試分成預設 Offline Replay 與明確 opt-in 的 Live Provider E2E，不得把 fixture 冒充真實分析。
12. Live E2E 必須實際使用操作者選定的 Pending JSON + 四份 MD；Provider 只接收 1 JSON + 3 model-visible MD，HTML Template 保持 local-only。
...
|---|---|---:|---|
| `rule_set_id` | `JAA-SKILL-RULESET-2026-08-24-DRAFT-083` | — | review-draft |
| `manifest_file` | `Skill_Analysis_Rule_Set_Manifest_v0.8.3.md` | runtime | 本檔載入時計算；不得自我綁定固定 Hash |
| `common_rules_version` | `1.6.3` | — | Multi-skill evidence coverage + Quality Gate v4 + Decision v5 |
| `common_rules_file` | `Skill_Classification_Common_Rules_v1.6.3.md` | 44599 | `9b7a9a537fa5d86d79f8881700043da26d15b4e575d8843c30235a5be74efb5a` |
...
| `classification_engine_version` | `JAA-CLASSIFICATION-1.6.3` | — | Quote-level Quality Gate + Decision v5 |
| `prompt_version` | `JAA-CHATGPT-ZH-TW-0.3.34` | runtime | effective instruction bytes／hash 逐 Run 保存 |
| `pipeline_version` | `JAA-ANALYSIS-PIPELINE-0.3.34` | runtime | deterministic validation／receipt truth／identity |
| `bridge_version` | `0.3.34-bridge-v14` | runtime | 行為進版；實際 bytes／SHA-256 由 Package Source build 計算並驗證 |
...
{
  "schemaVersion": "0.8.3",
  "ruleSetId": "JAA-SKILL-RULESET-2026-08-24-DRAFT-083",
  "status": "review-draft",
  "targetApplicationVersion": "0.3.34",
  "promptLocale": "zh-TW",
  ...
  "documents": [
    {
      ...
      "fileName": "Skill_Analysis_Rule_Set_Manifest_v0.8.3.md",
      "version": "0.8.3",
      "modelVisible": true,
    ...
    {
      "role": "COMMON_RULES",
      "fileName": "Skill_Classification_Common_Rules_v1.6.3.md",
      "version": "1.6.3",
      "bytes": 44599,
      "sha256": "9b7a9a537fa5d86d79f8881700043da26d15b4e575d8843c30235a5be74efb5a",
      "modelVisible": true
    },
    {
      "role": "SKILL_CATALOG",
  ...
  "contracts": {
    "classificationEngine": "JAA-CLASSIFICATION-1.6.3",
    "prompt": "JAA-CHATGPT-ZH-TW-0.3.34",
  ...
  "providerLifecycle": {
    "contract": "jaa-provider-lifecycle-v1",
    "authoritativeStates": ["ATTEMPT_CREATED", "PROVIDER_DISPATCHED", "INPUT_READING", "INPUT_DELIVERY_FINALIZED", "INPUT_READY", "ANALYSIS_STARTED", "ANALYSIS_IN_PROGRESS", "ARTIFACT_RECEIVED", "VALIDATION_COMPLETED", "CANONICAL_CREATED", "ANALYZED_RESULT_PUBLISHED", "ACTIVE_RESULT_COMMITTED", "REPORT_PACKAGE_CREATED", "HTML_RENDERED", "SQLITE_GATE", "TERMINAL"],
    "finalizeAtomicallySetsInputReady": true,
    "inputReadyToAnalysisStartedAllowed": true,
  ...
  "runtimeIdentity": {
    ...
    "promptTemplateVersion": "0.3.34-zh-TW-v15",
  ...
  "liveProviderValidation": {
    ...
    "command": "npm.cmd run test:live:v0.3.34:17",
...
→ INPUT_DELIVERY_FINALIZED → INPUT_READY
→ ANALYSIS_STARTED → ANALYSIS_IN_PROGRESS
→ ARTIFACT_RECEIVED → VALIDATION_COMPLETED
→ CANONICAL_CREATED → ANALYZED_RESULT_PUBLISHED
→ ACTIVE_RESULT_COMMITTED → REPORT_PACKAGE_CREATED
→ HTML_RENDERED → SQLITE_GATE → TERMINAL
```

- finalize 成功與 `INPUT_READY` 必須是同一 atomic Host operation。
- `INPUT_READY → ANALYSIS_STARTED` 必須合法。
- Delivery status 是唯讀查詢；finalize 前後均可重複呼叫。
- 完全相同的 finalize 重送回傳同一 immutable receipt。
- Progress event 是 telemetry；相同 event 重送或暫時無進度不應終止 Run。
