...
- File: `Skill_Classification_Common_Rules_v1.6.3.md`
- Rules Version: `1.6.3`
- Previous Version: `1.6.2`
- Schema Version: `1.6.3`
- Status: `proposed`
...
本版延續 v1.6.2 的 Multi-skill Evidence Coverage v2、Catalog Detail Status、Quality Gate v4、Validation Receipts V2、Decision v5、Quote Catalog、Snapshot 與 Report Data Package 分層；依 v0.3.33 兩次真實 17 筆 Managed ChatGPT Run 修正 Provider lifecycle 與輸入完成語意：

1. `jaa_finalize_input_delivery` 驗證成功後，JAA Host 必須在回傳 receipt 前原子完成 `INPUT_DELIVERY_FINALIZED → INPUT_READY`。
2. `modelInputDelivered=true`、4/4 files、N/N records、bytes／EOF／ACK／SHA-256 完整時，不得仍將 authoritative source state 保留為 `INPUT_READING`。
3. `INPUT_READY → ANALYSIS_STARTED` 是正式合法 transition；重複相同的 `ANALYSIS_STARTED` 採 idempotent ignore。
4. `jaa_report_analysis_progress` 是 observability／telemetry，不得因可恢復的重複、停滯或暫時倒退直接 terminalize Run。
5. `jaa_get_delivery_status` 是唯讀且 idempotent；finalize 前後與分析期間均可查詢，不得被視為 handle replay。
6. 相同 handle、finalAck、manifest hash 與 binding 的重複 finalize 回傳同一 immutable receipt；不一致的重複 finalize 才 fail closed。
7. Record-level `FAILED` 固定為 confidence=0、skillFindings=[]、unknownReasons=[]；具體錯誤只放 recordNegativeChecks／rationale。
8. 整體 input／transport／context access failure 是 Run-level failure，不得轉成 N 筆全部 FAILED 的假性 Decision Dataset。
9. Decision v5 正式狀態只允許 `CLASSIFIED`、`CATALOG_DETAIL_MISSING`、`UNKNOWN`、`EXCLUDED`、`FAILED`；舊文件中的 `NEEDS_REVIEW` 不屬於 v5 enum，不得送入正式 Artifact。
10. Validation Receipt 必須遵守因果順序；前一 Formal stage 已 FAILED 時，後續 Formal stage 不得顯示 PASSED。
11. Root error 必須保存 sourceState、targetState、attemptedTransition、firstFailedStage、rootErrorStage 與 derived consequences，禁止 null／互相矛盾。
12. 預設 Offline Replay 不聯絡 Provider；只有操作者明確 opt-in 的 Live Provider E2E 才可使用真實 1 JSON + 3 MD 聯絡 Managed ChatGPT。

v1.6.2 建立的 Multi-skill、Catalog Detail、Duplicate Warning、SQLite Warning Gate、HTML Capability V2 與其他規則全部維持。
v1.6.0 建立的下列規則全部維持：
...
## 11.8 Provider Lifecycle、Delivery Status 與 Run-level Failure

### 11.8.1 Authoritative transitions
...
- Finalize receipt 與 Host lifecycle 更新必須是同一 atomic operation。
- 成功 receipt 不得搭配舊的 `INPUT_READING` authoritative state。
- Provider completed 只代表 Turn 結束，不等於 Artifact／Canonical／HTML／SQLite 成功。
- 每個 Run 只允許一筆 idempotent `run_terminal`。

### 11.8.2 Delivery status 與 replay

- `jaa_get_delivery_status` 在 finalize 前後都可安全重複查詢。
- 完成後回傳的 status／receipt 必須 immutable，查詢不得消耗 handle。
- Segment cursor、ACK、Thread／Turn binding 與不同 finalAck 的非法重播才可回報 replay。
- 完全相同的 finalize 重試回傳原 receipt，不建立新 receipt 或新完成事件。

### 11.8.3 Progress telemetry

- `ANALYSIS_STARTED` 從 `INPUT_READY` 合法。
- 相同 progress 重複送達只記錄 duplicate evidence。
- completedCount 暫時不增加、重複或可恢復倒退時產生 warning，不終止 Turn。
- identity mismatch、跨 Run handle、負數或超過 expectedCount 才 fail closed。
- Progress failure 不得掩蓋較早的 root error，也不得把不存在的模型分析宣稱為 completed。

### 11.8.4 Run-level input access failure
...
### 11.8.5 Validation causal truth
...
本節說明 v1.6.3 如何套用至 JiraActivityAnalyzer。Evidence 權重與計分公式維持 v1.1.0；本版補齊 Quote-level Multi-skill Independence、Quality Gate 真實狀態、Decision v5、Frozen Evidence Segment、Evidence Role、聚合驗證、診斷 HTML、Report Data Package、Snapshot reference、本機 HTML renderer、跨 Run 共存與 Provider lifecycle 邊界。
