...
| Binding | Value | 狀態 |
|---|---|---|
| `manifest_schema_version` | `0.2.0` | contract schema |
| `rule_set_id` | `JAA-SKILL-RULESET-2026-08-14-DRAFT-02` | review-draft |
| `common_rules_version` | `1.2.0` | approved baseline |
| `common_rules_file` | `Skill_Classification_Common_Rules_v1.2.0.md` | 已整理 |
| `skill_catalog_version` | `0.3.1` | review-draft |
| `skill_catalog_file` | `Skill_Catalog_v0.3.1.md` | 已整理；279 項，逐 Skill 細節待補 |
| `classification_engine_version` | `JAA-CLASSIFICATION-1.2.0` | v0.3.20 最低相容契約 |
| `prompt_version` | `JAA-CHATGPT-ZH-TW-0.3.20` | 正式模式預設指令；實際 mode／hash 仍逐 Run 記錄 |
| `pipeline_version` | `JAA-ANALYSIS-PIPELINE-0.3.20` | deterministic assembly／validation |
| `report_template_version` | `JAA-ANALYSIS-REPORT-0.3.20` | 人類可讀報告；權威 count 由本機計算 |
...
├─ Skill_Classification_Common_Rules_v1.2.0.md
└─ Skill_Analysis_Rule_Set_Manifest.md
```

### 3.1 載入規則

1. 執行分析前驗證三個必要檔案存在且可讀。
2. 解析檔內版本，不可只相信檔名。
3. Catalog／Common Rules 版本必須與 Manifest binding 一致。
4. 每次 Run 記錄三個檔案的檔名、版本與 SHA-256。
...
9. 每次 Run 必須在 Snapshot 建立後計算三份 MD 的實際 bytes 與 SHA-256；Manifest 不以可變路徑或檔名取代內容 Hash。
10. 三份 MD 只定義分類語意與版本綁定；Provider transport、Tool Schema、retry、Runtime 與 SQLite gate 由 JAA 版本化程式契約負責，不應寫回 Catalog 當成技能規則。
...
`skillIds` 可包含同一 Group 或跨 Group 的多個 Skill；每個 Skill 必須符合 Common Rules v1.2.0 的獨立 Evidence-specific 判斷。`UNKNOWN`／`EXCLUDED` 可使用空陣列，但必須保留可解釋的負面檢查與原因。
...
| 同 Diff、相同來源、已有結果 | 執行前詢問：跳過、建立新版本或取代目前結果 |
...
9. AI Analysis DB 正式 Schema、Migration、備份與路徑命名。
