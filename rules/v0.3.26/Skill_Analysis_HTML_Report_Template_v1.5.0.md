...
本文件定義 Jira Activity Analyzer v0.3.26 的本機、deterministic、self-contained HTML。v1.5.0 在 v1.4.0 的進階篩選、Unique Issue 統計、FORMAL／DIAGNOSTIC mode 與離線多模板能力上，加入：

1. `AI_SUBMITTED`、`FORMAL_VALIDATED`、`CANONICAL_CREATED` 與 `SQLITE_ALLOWED` 各自獨立的 lifecycle 呈現。
2. Artifact Identity Receipt 逐欄位比對、Token binding 結果與 first failed stage。
3. Decision v5 `evidenceQuoteIds[]` 的 frozen Quote Catalog lineage。
4. Readable display text、exact source substring、container Segment 與 raw pointer／offset 四層 trace。
5. 可解碼 AI submission 被拒絕時，仍顯示完整 submitted decisions、Analysis Report、final summary 與聚合 findings。
6. 永久「Diagnostic Non-canonical／Canonical Not Created／SQLite Denied」警示。
7. AI Submission、Validation Stage、Quote Resolution 與 Identity Finding 多選篩選／欄位顯示。
...
```json
{
  ...
  "minimumRendererVersion": "JAA-LOCAL-HTML-RENDERER-1.5.0",
  "requiredCapabilities": [
    ...
    "LAYERED_VALIDATION_RECEIPTS_V1"
  ],
  "renderModes": [
    "FORMAL_CANONICAL",
...
- 同一 Formal Package 使用 v1.5.0 兩次，HTML 資料結果一致且 receipt hash 可追溯；檔名不得覆寫。
...
- Offline import 的 External Report Package 只能檢視與渲染，v0.3.26 不可直接寫入正式 SQLite。
