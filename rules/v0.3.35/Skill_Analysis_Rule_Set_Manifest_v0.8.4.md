# Skill Analysis Rule Set Manifest v0.8.4

- File: `Skill_Analysis_Rule_Set_Manifest_v0.8.4.md`
- Manifest Schema Version: `0.8.4`
- Previous Version: `0.8.3`
- Rule Set ID: `JAA-SKILL-RULESET-2026-08-24-DRAFT-084`
- Status: `review-draft`
- Prepared Date: `2026-08-24`
- Target Application: `JiraActivityAnalyzer`
- Target Application Version: `0.3.35`
- Prompt Locale: `zh-TW`
...
v0.8.4 延續 v0.8.3 的 Delivery Status／Finalize idempotency、FAILED／Run-level failure、Offline／Live 分層，以及 v0.8.2 的 Multi-skill、Catalog Detail、Warning Gate、HTML lifecycle、四檔選取、Active Result、Decision v5、Evidence Quote Catalog 與 Debug lineage，不變更 279 Skills。本次依 v0.3.34 真實 17 筆 Managed ChatGPT Run 進版：

1. Lifecycle 改由唯一 Run-scoped Host Controller 擁有，分離 required control state 與 optional analysis telemetry。
2. 第一個成功 segment read 推進 `PROVIDER_DISPATCHED → INPUT_READING`。
3. finalize 原子建立 receipt、更新同一 Controller 為 `INPUT_READY`，且在回覆成功前驗證 postcondition。
4. Progress 只屬 telemetry，不再是 Artifact、Canonical 或 SQLite 的必要控制閘。
5. 完整且可解碼、token／binding 合法的 Artifact submission 是分析完成的權威邊界。
6. Artifact 可從 `INPUT_READY`、`ANALYSIS_STARTED` 或 `ANALYSIS_IN_PROGRESS` 被接受；缺少 `ANALYSIS_COMPLETED` progress 不得拒絕。
7. Validation stage 9–14 只有在實際執行與 durable evidence 完成後才能 PASS，禁止 delegated／placeholder PASS。
8. Debug 與 UI 必須區分模型分析、Artifact 接收、內容驗證、正式發布與下游產物。
9. v0.3.34 真實 17 筆 Artifact 必須可 offline replay 至隔離 Canonical／Analyzed Result／Package／HTML。
10. Bridge 進版 `0.3.35-bridge-v15`；Transport v4、Decision v5、Catalog v0.3.1 與 Template／Renderer 1.5.1 維持不變。
...
|---|---|---:|---|
| `rule_set_id` | `JAA-SKILL-RULESET-2026-08-24-DRAFT-084` | — | review-draft |
| `manifest_file` | `Skill_Analysis_Rule_Set_Manifest_v0.8.4.md` | runtime | 本檔載入時計算；不得自我綁定固定 Hash |
| `common_rules_version` | `1.6.4` | — | Host control lifecycle + Artifact completion + Decision v5 |
| `common_rules_file` | `Skill_Classification_Common_Rules_v1.6.4.md` | 46756 | `3b8cccc062db30a1dbc19ea0b7d75d0e19e44150ab805b2d8ae43ef68f054d57` |
...
| `classification_engine_version` | `JAA-CLASSIFICATION-1.6.4` | — | Control lifecycle + Quote-level Quality Gate + Decision v5 |
| `prompt_version` | `JAA-CHATGPT-ZH-TW-0.3.35` | runtime | effective instruction bytes／hash 逐 Run 保存 |
| `pipeline_version` | `JAA-ANALYSIS-PIPELINE-0.3.35` | runtime | authoritative lifecycle／artifact completion／receipt truth |
| `bridge_version` | `0.3.35-bridge-v15` | runtime | 行為進版；實際 bytes／SHA-256 由 Package Source build 計算並驗證 |
...
{
  "schemaVersion": "0.8.4",
  "ruleSetId": "JAA-SKILL-RULESET-2026-08-24-DRAFT-084",
  "status": "review-draft",
  "targetApplicationVersion": "0.3.35",
  "promptLocale": "zh-TW",
  ...
  "documents": [
    {
      ...
      "fileName": "Skill_Analysis_Rule_Set_Manifest_v0.8.4.md",
      "version": "0.8.4",
      "modelVisible": true,
    ...
    {
      "role": "COMMON_RULES",
      "fileName": "Skill_Classification_Common_Rules_v1.6.4.md",
      "version": "1.6.4",
      "bytes": 46756,
      "sha256": "3b8cccc062db30a1dbc19ea0b7d75d0e19e44150ab805b2d8ae43ef68f054d57",
      "modelVisible": true
    },
    {
      "role": "SKILL_CATALOG",
  ...
  "contracts": {
    "classificationEngine": "JAA-CLASSIFICATION-1.6.4",
    "prompt": "JAA-CHATGPT-ZH-TW-0.3.35",
    ...
    "hostLifecycle": "jaa-host-control-lifecycle-v2",
  ...
  "artifactSubmission": {
    ...
    "allowedControlStates": ["INPUT_READY"],
  ...
  "providerLifecycle": {
    "contract": "jaa-host-control-lifecycle-v2",
  ...
  "liveProviderValidation": {
    ...
    "command": "npm.cmd run test:live:v0.3.35:17",
  ...
  "v0334ArtifactReplay": {
    "sourceBundle": "jira-activity-analyzer-debug-folder-20260824_175354(1).7z",
    "offlineOnly": true,
    ...
    "expectedFileCount": 4,
    "expectedRecordCount": 17,
    "expectedBytes": 319220,
    "expectedSegments": 80,
    "expectedDecisionCount": 17,
    "expectedSkillFindingCount": 44,
    "expectedEvidenceReferenceCount": 79,
    "expectedStatusDistribution": {"CATALOG_DETAIL_MISSING": 14, "UNKNOWN": 3},
...
### 8.1 Host control lifecycle 與 analysis telemetry
...
- 所有 control state 只能由同一 Run-scoped Host Controller 擁有。
...
- 完整有效 Artifact submission 是 analysis completion boundary，從 `INPUT_READY` 進入 `ARTIFACT_RECEIVED` 並記錄 `COMPLETED_BY_ARTIFACT`。
...
- v0.3.34 真實 Debug Bundle replay 必須建立隔離 Canonical／Analyzed Result／Package／HTML；SQLite receipt 使用 `NOT_RUN_BY_TEST_ISOLATION`。
...
Debug collector 以 `analysisAttemptId` 為第一層 correlation，只有透過 `linkedRunId` 收集 Run。Role files 由 receipt 的 exact versioned basename 收集，不使用無版本 alias。必須收入實體 AI submitted artifact、token／identity receipt（不含完整 token）、唯一 Host Controller snapshot、analysis telemetry、Quote Catalog、layered receipts，`navigation-decisions.jsonl`、quality findings、warning acceptance audit、SQLite receipt、Runtime Identity receipt、實際 Formal／Diagnostic／手動重繪 HTML、成功或失敗 HTML receipt 與 completeness manifest；不得只存包外 path。缺檔理由必須依真實 lifecycle。已有 submission 時不得標示 `not_applicable_no_submission`；應依事實使用 `submission_received_and_persisted` 或 `submission_validated_but_publish_blocked`。
