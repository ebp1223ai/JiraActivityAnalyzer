...
- File: `Skill_Classification_Common_Rules_v1.6.4.md`
- Rules Version: `1.6.4`
- Previous Version: `1.6.3`
- Schema Version: `1.6.4`
- Status: `proposed`
...
本版延續 v1.6.3 的 Delivery Status／Finalize idempotency、FAILED／Run-level failure、Validation causal truth，以及 v1.6.2 的 Multi-skill Evidence Coverage v2、Catalog Detail Status、Quality Gate v4、Validation Receipts V2、Decision v5、Quote Catalog、Snapshot 與 Report Data Package 分層；依 v0.3.34 真實 17 筆 Managed ChatGPT Run 修正 Host control lifecycle 與 Artifact completion 語意：

1. Lifecycle 由同一個 Run-scoped Host Controller 擁有，明確分為 required `controlState` 與 optional `analysisTelemetry`，不得存在第二個權威 state。
2. 第一個成功 segment read 將 control state 從 `PROVIDER_DISPATCHED` 推進為 `INPUT_READING`。
3. `jaa_finalize_input_delivery` 成功必須原子建立 receipt、更新同一 Controller 為 `INPUT_READY`，並驗證 postcondition 後才回覆模型成功。
4. `jaa_report_analysis_progress` 只更新 telemetry，不是 Artifact、Canonical 或 SQLite 的控制閘。
5. 完整且可解碼、token／binding 合法的 Artifact submission 是分析完成的權威邊界；不得要求先收到 `ANALYSIS_COMPLETED` progress checkpoint。
6. Artifact 可從 `INPUT_READY`、`ANALYSIS_STARTED` 或 `ANALYSIS_IN_PROGRESS` 被持久化並推進為 `ARTIFACT_RECEIVED`；缺少 progress 不得拒絕有效 Artifact。
7. Validation stage 9–14 只有在對應工作與 durable evidence 實際完成時才能 `PASSED`；禁止 `delegated PASS` 或 placeholder PASS。
8. Artifact 已保存但正式發布被阻擋時，Debug 必須分類為 `submission_validated_but_publish_blocked`，不得標示為沒有 submission。
9. UI 必須分開顯示模型輸入、模型分析、Artifact 接收、內容驗證、正式發布與下游產物狀態。
10. v0.3.34 真實 17 筆 Debug Bundle 必須可在隔離、offline、無 Provider／SQLite／Active Result 副作用下 replay 至 Canonical／Analyzed Result／Package／HTML。

v1.6.3 與 v1.6.2 建立的其他規則全部維持。
v1.6.0 建立的下列規則全部維持：
...
## 11.8 Host Control Lifecycle、Analysis Telemetry 與 Run-level Failure
...
### 11.8.2 Segment read、Finalize 與 Delivery status
...
### 11.8.3 Progress telemetry 與 Artifact completion
...
- 完整且可解碼、token／binding 合法的 Artifact submission 是分析完成的權威證據。
...
- UI 必須分別呈現 Input Delivery、Model Analysis、Artifact、Validation、Canonical、Analyzed Result、HTML 與 SQLite，不得以單一 `provider_failed` 掩蓋已完成的模型分析。
...
本節說明 v1.6.4 如何套用至 JiraActivityAnalyzer。Evidence 權重與計分公式維持 v1.1.0；本版補齊 Quote-level Multi-skill Independence、Quality Gate 真實狀態、Decision v5、Frozen Evidence Segment、Evidence Role、聚合驗證、診斷 HTML、Report Data Package、Snapshot reference、本機 HTML renderer、跨 Run 共存、單一 Host control lifecycle 與 Artifact completion 邊界。
