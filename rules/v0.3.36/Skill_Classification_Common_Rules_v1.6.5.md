...
- File: `Skill_Classification_Common_Rules_v1.6.5.md`
- Rules Version: `1.6.5`
...
本版延續 v1.6.4 的單一 Host Controller、Artifact-as-Completion、Validation causal truth，以及 v1.6.2／v1.6.3 的 Multi-skill Evidence Coverage v2、Catalog Detail Status、Quality Gate v4、Validation Receipts V2、Decision v5、Quote Catalog、Snapshot 與 Report Data Package 分層；依 v0.3.35 真實 17 筆 Managed ChatGPT Run 修正 terminal reconciliation、Artifact Identity 與 durable artifact truth：

1. Run 最終結果只能由單一 Host-side Terminal Outcome Reducer 決定；Provider callback、Bridge tool 與 UI 不得直接 terminalize Run。
2. `controlState >= ARTIFACT_RECEIVED` 必須 deterministic 推導 `analysisStarted=true`、`analysisCompleted=true` 與 telemetry=`COMPLETED_BY_ARTIFACT`。
3. Provider Turn completed／failed／cancelled 與 Artifact 狀態使用固定矩陣；已有合法 Artifact 時，Provider terminal event 不得覆蓋本機 pipeline。
4. Post-Artifact pipeline 必須以 `runId + artifactHash` 冪等、可重入並可由 Artifact handler、Turn terminal handler與 interrupted Run recovery 共同呼叫。
5. Approved Artifact Identity 只能從 immutable Runtime Contract Registry 凍結；dispatch-time expected 與 Artifact-time Host injection 必須為獨立來源。
6. Artifact 狀態分為 `received → persisted → content_validated → formally_published`；模型提交工具不得在 formal validation 前回覆 `published`。
7. Artifact final summary 與 Provider final assistant response 分檔保存；任何 receipt 綁定的 durable 檔案不得被後續事件覆寫。
8. Lifecycle、Validation receipts、root error、first failed stage 與實體 artifacts 必須 reconciliation 一致；矛盾狀態必須 fail closed 並列出 finding。
9. Debug／UI 必須保存真實 submission、identity、bytes/hash 與 numeric token telemetry；不得把已有 submission 標示為不存在。
10. v0.3.35 真實 17 筆 Debug Bundle 必須執行 as-recorded forensic replay 與 corrected-host-identity lifecycle replay；前者如實檢出 stale identity，後者在隔離環境走完 Canonical／Analyzed Result／Package／HTML。
...
本節說明 v1.6.5 如何套用至 JiraActivityAnalyzer。Evidence 權重與計分公式維持 v1.1.0；本版補齊 Quote-level Multi-skill Independence、Quality Gate 真實狀態、Decision v5、Frozen Evidence Segment、Evidence Role、聚合驗證、診斷 HTML、Report Data Package、Snapshot reference、本機 HTML renderer、跨 Run 共存、單一 Host control lifecycle、Terminal Outcome Reducer、Artifact Identity 與 Post-Artifact recovery 邊界。
