# JAA 技能分析 HTML 報告模板 v1.4.0

- File: `Skill_Analysis_HTML_Report_Template_v1.4.0.md`
- Template Schema Version: `jaa-html-report-template-v5`
- Template ID: `JAA-SKILL-ANALYSIS-GOLDEN-HTML`
- Template Version: `1.4.0`
- Previous Version: `1.3.0`
- Status: `proposed-for-v0.3.24`
- Locale: `zh-TW`
- Time Zone: `Asia/Taipei`
- Target Application: `JiraActivityAnalyzer`
- Target Application Version: `0.3.24`
- Minimum Renderer Version: `JAA-LOCAL-HTML-RENDERER-1.4.0`
- Required Decision Contract: `jaa-ai-analysis-decisions-v4`
...
本文件定義 Jira Activity Analyzer v0.3.24 的本機、deterministic、self-contained HTML。v1.4.0 在 v1.3.0 的進階篩選、欄位控制與診斷能力基礎上，加入：

1. `FORMAL_CANONICAL` 與 `DIAGNOSTIC_NON_CANONICAL` 兩種 mode。
2. Frozen Evidence Segment、`PRIMARY_CHANGE`／`SUPPORTING_CONTEXT` 與 source trace。
3. 聚合式 Validation／Quality findings 的統計與 deep link。
4. 診斷預覽的永久紅色非正式警示與 SQLite deny gate。
5. Readable／Raw／Segment Trace 三層 Evidence 呈現。
6. Evidence Role、Validation State 的多選篩選與欄位控制。
7. HTML 的直接資料來源改為 `jaa-analysis-report-data-package-v1`，不直接讀取 Canonical Result 或資料庫。
8. Unique Issue Snapshot、欄位適用性狀態與 declarative statistics DSL。
9. Template／Package capability negotiation，以及同一 Package 多模板離線重繪。
...
- 對 Decision v2／v3 只能產生 `LEGACY_UNVERIFIED` preview，不可冒充 Decision v4 品質通過。
...
```json
{
  "schemaVersion": "jaa-html-report-template-v5",
  "templateId": "JAA-SKILL-ANALYSIS-GOLDEN-HTML",
  "templateVersion": "1.4.0",
  "locale": "zh-TW",
  "timeZone": "Asia/Taipei",
  "minimumRendererVersion": "JAA-LOCAL-HTML-RENDERER-1.4.0",
  ...
  "input": {
    ...
    "requiredDecisionContract": "jaa-ai-analysis-decisions-v4",
  ...
  "diagnosticMode": {
    "bannerLevel": "DANGER",
    "bannerText": "非正式診斷預覽：此分析未通過 JAA 正式驗證，不可匯入資料庫，亦不可視為正式分析成果。",
...
- 正式 Classification／Skill Finding：Formal Package 中由 Decision v4 + Canonical Assembly 帶入的 immutable projection。
...
| Evidence Segment ID | `evidenceQuotes[].evidenceSegmentId` | Frozen Segment Catalog 中的 deterministic 定位鍵 | Model reference + JAA Segmenter |
| Evidence Role | `evidenceQuotes[].evidenceRole` | 本次可歸因變更或僅供佐證的 context | Model claim + JAA validation |
| Source Trace | `evidenceSegment.sourceJsonPointer` | Quote 回到 raw source、Diff hunk 與 line 的定位 | JAA Segmenter |
...
7. Frozen Evidence Segment Inventory。
...
- v2／v3 Preview 的 Quality 統計必須誠實顯示，不得宣稱 Decision v4 PASSED。
...
### 14.4 Decision v4 Evidence Role

- 每個 `CLASSIFIED` Skill Finding 至少顯示一個有效 `PRIMARY_CHANGE`。
- `SUPPORTING_CONTEXT` 必須視覺分離且不得標示成主要變更。
- Context-only finding 必須出現在 Aggregated Findings，不能只顯示第一筆錯誤。
- Evidence Segment ID 可定位到 source pointer／Diff hunk／line。

### 14.5 Diagnostic Non-canonical

- Artifact 可解析但 Schema／Identity／Segment／Quote／Attribution／Semantic／Quality 任一 blocking gate 失敗時，產生診斷 HTML。
- 固定顯示紅色非正式警示、`Canonical: NOT CREATED` 與 `SQLite: DENIED`。
...
- 同一 Formal Package 使用 v1.4.0 兩次，HTML 資料結果一致且 receipt hash 可追溯；檔名不得覆寫。
...
- Offline import 的 External Report Package 只能檢視與渲染，v0.3.24 不可直接寫入正式 SQLite。
