...
本版延續 v1.2.0 的權重、計分公式、multi-skill 與 deterministic assembly 邊界，補上 v0.3.20 真實 117 筆 Run 驗證後確認的 Status／Decision 欄位契約：

1. Status 必須先依 Evidence 語意決定，再依 Status 寫入合法欄位；不得為通過格式驗證而改變分類。
2. `unknownReasons` 只有 `UNKNOWN` 可以非空；所有其他 Status 必須為 `[]`。
3. 明定 `EXCLUDED`、`UNKNOWN`、`NEEDS_REVIEW` 與 `CLASSIFIED` 的判斷順序，避免把明確非技能 Evidence 全部降為 UNKNOWN。
4. `EXCLUDED` 的排除證據寫入 `negativeChecks` 與 `rationale`；`NEEDS_REVIEW` 的人工覆核原因寫入 `rationale`。
5. Report Template 是 JAA 本機 deterministic renderer 的版本化依據，不加入 ChatGPT 的 `1 JSON + 3 MD` 模型輸入。
6. 不設定任何預期狀態分布或固定 Skill 數量；同一輸入跨 Run 的差異必須保留為可比較證據，不得硬編碼單次輸出為 Golden label。
...
- 本機 Golden HTML 的版面、欄位、標籤、色彩 token、篩選與排序規格
...
| 狀態 | 使用條件 | `skillIds` | `positiveEvidence` | `unknownReasons` | 原因主要存放處 |
|---|---|---|---|---|---|
| `CLASSIFIED` | 至少一個有效 Skill，且每個 Skill 都有足夠 Evidence-specific 支持與負面檢查 | 非空 | 非空 | `[]` | `positiveEvidence`、`negativeChecks`、`rationale` |
| `UNKNOWN` | 不是明確非技能內容，但技術資訊不足，無法建立合理候選 | `[]` | `[]` | 至少一筆 | `unknownReasons`、`rationale`；`negativeChecks` 只寫實際檢查 |
| `NEEDS_REVIEW` | 有合理候選，但作者歸屬、技能界線、內容完整性或矛盾證據需要人工判斷 | 可有候選；無候選時應重新判斷是否為 UNKNOWN | 有候選 Skill 時必須有對應證據 | `[]` | `rationale`、`negativeChecks` |
| `EXCLUDED` | 可明確判定為 automation、workflow-only、request-only、completion-only、context-link-only、模板或其他非技能 Evidence | `[]` | `[]` | `[]` | `negativeChecks`、`rationale` |
| `CATALOG_DETAIL_MISSING` | Evidence 有技術內容，但綁定 Catalog 缺少足以正式判斷的 Skill Detail；不得由模型補寫正式定義 | 依契約與實際候選 | 依實際證據 | `[]` | `rationale` |
| `FAILED` | 本筆因解析、完整性或不可恢復的技術錯誤未完成分析；不得用來包裝一般 UNKNOWN | `[]` | `[]` | `[]` | `rationale` 與結構化 error evidence |
...
每筆 Evidence 必須依下列順序判斷：

```text
明確屬於非技能 Evidence
→ EXCLUDED

不是明確非技能，但技術資訊不足且沒有合理 Skill 候選
→ UNKNOWN

存在合理 Skill 候選，但需要人工判斷歸屬、邊界、完整性或矛盾
→ NEEDS_REVIEW

至少一個 Skill 有充分、獨立、可追溯的證據且通過負面檢查
→ CLASSIFIED

有技術證據但 Catalog Detail 不足
...
Status 一旦依 Evidence 語意決定，欄位必須服從該 Status。禁止為了讓 `unknownReasons` 合法、提高分類率、降低 warning、貼近範例或通過 Schema，而把 `EXCLUDED` 改成 `UNKNOWN`、把 `NEEDS_REVIEW` 改成 `UNKNOWN`，或把 `UNKNOWN` 強制改成 `CLASSIFIED`。

### 11.4 Decision Transport 欄位規則

1. `unknownReasons` 只有 `status = UNKNOWN` 時允許非空。
2. 所有非 UNKNOWN Decision 的 `unknownReasons` 必須嚴格為 `[]`。
3. `EXCLUDED` 的排除原因不得寫入 `unknownReasons`。
4. `NEEDS_REVIEW` 的覆核原因不得寫入 `unknownReasons`。
5. 沒有實際執行的排除檢查不得捏造為 `negativeChecks`；可依契約使用空陣列。
6. JAA 的 Dynamic Tool JSON Schema、Prompt、TypeScript type、semantic validator 與 canonical assembly 必須實作相同矩陣。
7. 模型送出前應完成全陣列 self-check；正式流程仍只允許一次 Artifact submission，不因此導入自動 Repair。
...
本節說明 v1.2.1 如何套用至 JiraActivityAnalyzer。Evidence 權重與計分公式維持 v1.1.0；本版補齊模型 Decision 欄位矩陣、Status 穩定性、本機 HTML renderer 與跨 Run 共存邊界。
...
- Golden HTML 必須由 JAA 本機使用 Manifest 綁定的 `Skill_Analysis_HTML_Report_Template_v1.0.0.md` 與已通過驗證的 Canonical Result 產生，不接受模型產生的 HTML 作為正式報告。
