...
assert.equal(bytes.length, manifest.artifactBytes); assert.equal(hash, manifest.artifactSha256); assert.equal(manifest.bridgeIdentity, "0.3.31-bridge-v13");
...
const names = fs.readdirSync(resourceDirectory); assert.deepEqual(names.sort(), ["analysis-bridge-manifest-v0331.json", "analysis-bridge-v0331.cjs"]);
...
const appAsarCurrentBridgeCount = asarBridgeEntries.filter((entry) => /analysis-bridge-v0331\.cjs$/i.test(entry)).length;
const staleAsarBridgeEntries = asarEntries.filter((entry) => /analysis-bridge-v\d+\.cjs$/i.test(entry) && !/analysis-bridge-v0331\.cjs$/i.test(entry));
...
const portable = diagnostic(path.join(release, "Jira Activity Analyzer Portable 0.3.31.exe"), "portable");
const report = { schemaVersion: "jaa-v0331-packaged-bridge-verification-v1", status: "PASS", bridgeIdentity: manifest.bridgeIdentity, bridgeBytes: bytes.length, bridgeSha256: hash, inventory: { appAsarCurrentBridgeCount, appAsarBridgeEntries: asarBridgeEntries, externalCurrentBridgeCount: 1, externalManifestCount: 1, staleBridgeCount: staleAsarBridgeEntries.length, staleAsarBridgeEntries, resourceDirectory, files: names }, diagnostics: { winUnpacked: win, portable }, installerInventory: { status: "BUILT_FROM_SAME_ELECTRON_BUILDER_RESOURCE_CONFIG", guiValidation: "MANUAL_VALIDATION_PENDING" }, verifiedAtUtc: new Date().toISOString() };
fs.writeFileSync(path.join(release, "v0.3.31-packaged-bridge-verification.json"), JSON.stringify(report, null, 2) + "\n"); console.log(JSON.stringify(report, null, 2));
