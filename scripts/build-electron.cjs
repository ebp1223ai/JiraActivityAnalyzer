...
fs.writeFileSync(frozenSharedContractPath, fs.readFileSync(frozenSharedContractPath, "utf8").replace("0.3.35-bridge-v15", "0.3.31-bridge-v13"), "utf8");
try {
...
const bridgeOutput = path.join(bridgeResourceDir, "analysis-bridge-v0335.cjs");
esbuild.buildSync({
  ...common,
  entryPoints: [path.resolve(__dirname, "../electron/analysisBridgeRuntimeV0335.ts")],
...
fs.writeFileSync(path.join(bridgeResourceDir, "analysis-bridge-manifest-v0335.json"), JSON.stringify({
  schemaVersion: "jaa-packaged-analysis-bridge-manifest-v1", bridgeIdentity: "0.3.35-bridge-v15", artifactFileName: "analysis-bridge-v0335.cjs",
...
console.log(`  analysis bridge manifest  identity=0.3.35-bridge-v15 bytes=${bridgeBytes} sha256=${bridgeSha256}`);
