...
const artifact = path.join(resourceDirectory, "analysis-bridge-v0334.cjs");
const manifestFile = path.join(resourceDirectory, "analysis-bridge-manifest-v0334.json");
const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
const bytes = fs.readFileSync(artifact); const hash = crypto.createHash("sha256").update(bytes).digest("hex");
assert.equal(bytes.length, manifest.artifactBytes); assert.equal(hash, manifest.artifactSha256); assert.equal(manifest.bridgeIdentity, "0.3.34-bridge-v14");
...
const names = fs.readdirSync(resourceDirectory); assert.deepEqual(names.sort(), ["analysis-bridge-manifest-v0334.json", "analysis-bridge-v0334.cjs"]);
...
const appAsarCurrentBridgeCount = asarBridgeEntries.filter((entry) => /analysis-bridge-v0334\.cjs$/i.test(entry)).length;
const staleAsarBridgeEntries = asarEntries.filter((entry) => /analysis-bridge-v\d+\.cjs$/i.test(entry) && !/analysis-bridge-v0334\.cjs$/i.test(entry));
