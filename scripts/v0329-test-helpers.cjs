...
const artifacts = path.join(root, "test-artifacts", "v0329");
function moduleApi() { fs.mkdirSync(artifacts, { recursive: true }); const target = path.join(artifacts, "v0329-modules.cjs"); esbuild.buildSync({ entryPoints: [path.join(root, "electron", "v0329TestExports.ts")], outfile: target, bundle: true, platform: "node", format: "cjs", target: "node20", logLevel: "silent" }); delete require.cache[target]; return require(target); }
...
module.exports = { root, artifacts, moduleApi, sha256, archiveEntries, archiveText };
