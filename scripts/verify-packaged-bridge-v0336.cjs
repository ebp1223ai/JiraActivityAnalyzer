...
const artifact = path.join(resourceDirectory, "analysis-bridge-v0336.cjs");
const manifestFile = path.join(resourceDirectory, "analysis-bridge-manifest-v0336.json");
...
const names = fs.readdirSync(resourceDirectory); assert.deepEqual(names.sort(), ["analysis-bridge-manifest-v0336.json", "analysis-bridge-v0336.cjs"]);
...
const appAsarCurrentBridgeCount = asarBridgeEntries.filter((entry) => /analysis-bridge-v0336\.cjs$/i.test(entry)).length;
const staleAsarBridgeEntries = asarEntries.filter((entry) => /analysis-bridge-v\d+\.cjs$/i.test(entry) && !/analysis-bridge-v0336\.cjs$/i.test(entry));
