...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const [capacityPrompt, setCapacityPrompt] = useState<{ run: AiAnalysisRun; payload: { mode: AiAnalyzerMode; datasetId: string; selectedDiffIds: string[]; service: AiServiceKey } } | null>(null);
  ...
  async function start() {
    ...
    const request = { mode, datasetId: dataset.datasetId, selectedDiffIds: selected, service };
    setBusy(true);
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.12">
