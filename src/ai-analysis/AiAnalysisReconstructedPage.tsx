...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  async function useBundledRules() { const result = await api?.useBundledRules(); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Bundled v0.3.35 Rule Set 已原子啟用。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  ...
  return <div className="space-y-4">
    ...
    {workspaceRun ? <Section eyebrow="Artifact lifecycle v0.3.35" title="分析產物生命週期" description="Provider、提交、驗證與正式產物分開顯示；診斷產物不會取代 Active Result。">
