...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  async function useBundledRules() { const result = await api?.useBundledRules(); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Bundled v0.3.32 Rule Set 已原子啟用。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  ...
  async function start() {
    ...
    notify(result.run?.status === "completed_with_persistence_error" ? "分析產物與 HTML 已完成，但 SQLite 寫入失敗；可在結果頁重試。" : result.ok ? "分析完成，Canonical JSON、HTML 與 SQLite commit 已建立。" : resultErrorText({ errorCode: result.errorCode ?? result.run?.progress.errorCode, message: result.message ?? result.run?.progress.message }), result.run?.status === "completed_with_persistence_error" ? "amber" : result.ok ? "green" : "red");
  }
  ...
  return <div className="space-y-4">
    ...
    {workspaceRun ? <Section eyebrow="Artifact lifecycle v0.3.32" title="分析產物生命週期" description="Provider、提交、驗證與正式產物分開顯示；診斷產物不會取代 Active Result。">
...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  async function report(format: "csv" | "html") { if (!run) return; const result = await api?.exportRun({ runId: run.runId, format }); notify(result?.canceled ? "Report export cancelled." : result?.filePath ? "Report saved: " + result.filePath : resultErrorText(result), result?.filePath ? "green" : "red"); }
