...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  async function useBundledRules() { const result = await api?.useBundledRules(); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Bundled v0.3.25 Rule Set 已原子啟用。" : (result?.errorCode ?? "AI_RULE_SELECTION_FAILED") + ": " + (result?.message ?? ""), result?.ok ? "green" : "red"); }
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.25">
