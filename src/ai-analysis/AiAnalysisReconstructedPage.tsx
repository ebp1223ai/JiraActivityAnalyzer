...
function resultErrorText(value: unknown) {
  const result = value && typeof value === "object" ? value as Record<string, unknown> : {};
  const normalized = result.normalizedError && typeof result.normalizedError === "object" ? result.normalizedError as Record<string, unknown> : {};
  const code = String(normalized.errorCode ?? result.errorCode ?? "UNEXPECTED_ERROR_SHAPE");
  const message = String(normalized.messageZhTw ?? result.message ?? "收到非標準錯誤物件。");
  return code + ": " + message;
}
...
function AiNexusConnection({ snapshot, update, notify }: { snapshot: AiAnalysisSnapshot; update: (value: AiAnalysisSnapshot) => void; notify: (message: string, tone?: Tone) => void }) {
  ...
  async function save() {
    ...
    notify(result.ok ? "AI Nexus 設定已安全寫入執行檔同目錄的 .env。" : resultErrorText(result), result.ok ? "green" : "red");
  }
  return (
    <div className="min-w-0 space-y-4">
...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  const instructionPreview = instructionMode === "STANDARD_FORMAL" ? "JAA Safety Wrapper + Artifact Token + bridge-resumable-v4 + Standard Formal Contract + Final Response Contract" : instructionMode === "STANDARD_PLUS_USER_INSTRUCTION" ? "JAA Safety Wrapper + Artifact Token + bridge-resumable-v4 + Standard Formal Contract\n\nUser additional instruction:\n" + (supplementalInstruction || "（尚未輸入）") : "JAA Safety Wrapper + Artifact Token + bridge-resumable-v4\n\nCUSTOM_DIAGNOSTIC: no canonical result, Golden HTML, or SQLite\n\nUser custom instruction:\n" + (supplementalInstruction || "（尚未輸入）");
  ...
  const readiness = [
    ...
    { id: "analysisBridge", label: `Analysis Bridge ${snapshot.analysisBridge?.expectedBridgeIdentity ?? "unavailable"}`, ok: snapshot.analysisBridge?.status === "ready" },
    { id: "databaseReady", label: "AI Analysis SQLite 可建立", ok: true },
    { id: "noActiveRun", label: "沒有其他分析執行中", ok: !snapshot.activeRunId }
  ].filter((item) => mode === "CHATGPT" || item.id !== "analysisBridge").filter((item) => mode !== "OFFLINE_RULE" || !["providerConfigured", "providerAuthenticated", "providerConnected", "modelAvailable"].includes(item.id));
  ...
  async function chooseRuleRole(role: "manifest" | "common_rules" | "catalog" | "html_template") { const result = await api?.chooseRuleRole(role); if (result?.snapshot) update(result.snapshot); if (result && !result.canceled && !result.ok) notify(resultErrorText(result), "red"); }
  async function cancelRuleDraft() { const result = await api?.cancelRuleDraft(); if (result?.snapshot) update(result.snapshot); }
  async function useBundledRules() { const result = await api?.useBundledRules(); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Bundled v0.3.37 Rule Set 已原子啟用。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  async function reloadRules() { const result = await api?.loadRules(); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "分析依據已重新讀檔並計算 SHA-256。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  async function choosePending() { const result = await api?.choosePending(); if (result?.snapshot) update(result.snapshot); if (result && !result.canceled && !result.ok) notify(resultErrorText(result), "red"); }
  async function verifyPending() { if (!dataset) return; const result = await api?.verifyPending(dataset.datasetId); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Pending Dataset 完整性已重新驗證。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  async function start() {
    ...
    notify(result.run?.status === "completed_with_persistence_error" ? "Canonical 已完成，但 SQLite 寫入失敗；HTML 狀態請見結果頁。" : result.run?.status === "completed_with_report_error" ? "Canonical 與 SQLite 已完成，但 HTML 產生失敗；可在結果頁重新產生。" : result.run?.status === "completed_with_quality_warnings" ? "Canonical 與報告產物已完成；SQLite 等待人工接受 Quality warnings。" : result.ok ? "分析完成，Canonical、HTML 與 SQLite 均已完成。" : resultErrorText({ errorCode: result.errorCode ?? result.run?.progress.errorCode, message: result.message ?? result.run?.progress.message }), result.run?.status === "completed_with_persistence_error" || result.run?.status === "completed_with_report_error" || result.run?.status === "completed_with_quality_warnings" ? "amber" : result.ok ? "green" : "red");
  }
  ...
  async function reprocessPersisted() {
    if (!api || !workspaceRun?.postArtifactRecovery?.eligible || busy) return;
    const confirmed = window.confirm("重新處理已保存的分析結果？\n\n不會再次聯絡 ChatGPT、不會消耗 Token、不會修改原提交，也不會自動取代較新的 Active Result。");
    if (!confirmed) return;
    setBusy(true); const result = await api.reprocessPersisted(workspaceRun.runId); setBusy(false);
    if (result.snapshot) update(result.snapshot);
    notify(result.ok ? "已保存的分析結果已完成冪等重新處理。" : resultErrorText(result), result.ok ? "green" : "red");
  ...
  return <div className="space-y-4">
    ...
    {mode === "CHATGPT" && snapshot.analysisBridge ? <div className={`rounded border p-3 text-xs font-bold ${snapshot.analysisBridge.status === "ready" ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-red-300 bg-red-50 text-red-950"}`} data-testid="analysis-bridge-readiness"><div>{snapshot.analysisBridge.status === "ready" ? `Analysis Bridge ${snapshot.analysisBridge.expectedBridgeIdentity} 已通過 bytes、SHA-256 與 loadability 驗證。` : `${snapshot.analysisBridge.rootErrorCode ?? "AI_BRIDGE_UNAVAILABLE"}: ${snapshot.analysisBridge.rootErrorMessage ?? "Bridge preflight failed."}`}</div><div className="mt-1 break-all font-mono text-[10px]">{snapshot.analysisBridge.resolvedAbsolutePath ?? "No Bridge artifact resolved"}</div></div> : null}
    ...
    {workspaceRun ? <Section eyebrow="Artifact lifecycle v0.3.37" title="分析產物生命週期" description="Provider、提交、驗證、Recovery 與正式產物分開顯示；歷史 terminal 保持 append-only。" actions={workspaceRun.postArtifactRecovery?.eligible ? <button className="btn" disabled={busy} onClick={() => void reprocessPersisted()}><RefreshCw size={15} />重新處理已保存的分析結果</button> : undefined}>
      {workspaceRun.bridgeEvidence?.providerDispatchGate ? <div className={`mb-4 rounded border p-4 text-xs font-bold ${workspaceRun.bridgeEvidence.providerDispatchGate.providerDispatchAllowed ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-red-300 bg-red-50 text-red-950"}`}><div>{workspaceRun.bridgeEvidence.providerDispatchGate.providerDispatchAllowed ? "Runtime Manifest 驗證通過，已允許聯絡 ChatGPT。" : "尚未聯絡 ChatGPT：Runtime Manifest 驗證失敗。"}</div><div className="mt-1 break-words font-mono text-[10px]">expected={String(workspaceRun.bridgeEvidence.providerDispatchGate.expectedProtocol ?? "unknown")} · observed={String(workspaceRun.bridgeEvidence.providerDispatchGate.observedProtocol ?? "unknown")}</div></div> : null}
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
        <Metric label="Provider Adapter" value={workspaceRun.providerAdapterDisplayName ?? snapshot.providerAdapters?.[0]?.displayName ?? "ChatGPT / Bundled Codex"} />
        <Metric label="Durable Artifact" value={workspaceRun.durableArtifactTruth?.artifactSha256 ? `PERSISTED · ${workspaceRun.durableArtifactTruth.artifactSha256.slice(0, 12)}…` : workspaceRun.durableArtifactTruth?.status ?? "NOT OBSERVED"} />
        <Metric label="Resolver" value={workspaceRun.durableArtifactTruth?.status ?? "NOT RUN"} />
        <Metric label="Post-Artifact" value={workspaceRun.postArtifactRecovery?.nextStage ?? (workspaceRun.postArtifactRecovery?.status === "NOT_REQUIRED" ? "COMPLETE" : "NOT STARTED")} />
        <Metric label="Recovery" value={workspaceRun.postArtifactRecovery?.status ?? "NOT REQUIRED"} />
        <Metric label="Provider" value={workspaceRun.lifecycle?.providerTurnStatus ?? workspaceRun.status} />
        <Metric label="Input Delivery" value={workspaceRun.bridgeEvidence?.modelDeliveryReceipt ? "DELIVERED / INPUT_READY" : workspaceRun.lifecycle?.inputStatus ?? "PENDING"} />
        <Metric label="Model Analysis" value={String(workspaceRun.bridgeEvidence?.analysisTelemetry?.state ?? workspaceRun.lifecycle?.analysisStatus ?? "NOT_REPORTED")} />
        <Metric label="Artifact" value={workspaceRun.lifecycle?.artifactStatus === "submission_rejected" ? "PERSISTED / REJECTED" : workspaceRun.bridgeEvidence?.artifactReceipt ? `RECEIVED / ${workspaceRun.bridgeEvidence.artifactReceipt.decisionCount ?? 0} DECISIONS` : workspaceRun.lifecycle?.artifactStatus ?? "NOT_RECEIVED"} />
        <Metric label="Content Validation" value={workspaceRun.lifecycle?.validationStatus ?? "NOT_STARTED"} />
        <Metric label="Formal Publish" value={workspaceRun.analyzedFilePath ? "PUBLISHED" : "NOT_PUBLISHED"} />
        <Metric label="Canonical / Active" value={`${workspaceRun.analyzedFilePath ? "CREATED" : "NOT_CREATED"} / ${snapshot.activeResult?.runId === workspaceRun.runId ? "COMMITTED" : "UNCHANGED"}`} />
...
<div className="mt-4 rounded border border-blue-200 bg-blue-50 p-3 text-xs font-bold leading-relaxed text-blue-900">Recovery 只使用已保存的 Artifact 與本機 receipts；不會再次聯絡 ChatGPT、不會消耗 Token，也不會修改原始 submission。</div>
            {workspaceRun.bridgeEvidence?.artifactReceipt && workspaceRun.lifecycle?.rootErrorCode && !workspaceRun.analyzedFilePath ? <div className="mt-4 rounded border border-amber-300 bg-amber-50 p-4 text-sm font-bold text-amber-950">ChatGPT 已完成 {String(workspaceRun.bridgeEvidence.artifactReceipt.decisionCount ?? workspaceRun.receivedDecisionCount ?? 0)} 筆分析並提交結果，但 JAA 在正式發布階段失敗。</div> : null}
      ...
      {workspaceRun.lifecycle?.artifactStatus === "submission_rejected" || workspaceRun.progress.errorCode === "AI_ARTIFACT_SUBMISSION_REJECTED" || workspaceRun.progress.errorCode === "AI_DECISION_QUALITY_GATE_BLOCKED" ? <div className="mt-4 rounded border border-amber-300 bg-amber-50 p-4 text-sm font-bold leading-relaxed text-amber-950"><div>{workspaceRun.progress.errorCode ?? "AI_DECISION_QUALITY_GATE_BLOCKED"} · {workspaceRun.lifecycle?.firstFailedValidationStage ?? "QUALITY_GATE"}</div><div className="mt-1 text-xs font-semibold">Submission 已保存；Formal Artifact 已拒絕；Canonical 未建立。Diagnostic Package／HTML 已建立，可開啟聚合 Quality findings 檢查 blocker 數量、受影響 records 與第一筆 recordIndex／skillId。後續 Validation stages 顯示 NOT RUN，不會冒充 PASS。</div><button className="btn mt-3" onClick={() => void api?.openFolder(workspaceRun.runDirectory!)}><FolderOpen size={15} />開啟 Submission / Findings / Diagnostic</button></div> : null}
    </Section> : null}
...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  async function chooseAnalyzed() { const result = await api?.chooseAnalyzed(); if (result?.snapshot) update(result.snapshot); if (result && !result.canceled && !result.ok) notify(resultErrorText(result), "red"); }
  async function activate(activeResultId: string) { const result = await api?.activateResult(activeResultId); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Active Result 已切換。" : resultErrorText(result), result?.ok ? "green" : "red"); }
  async function review(resultId: string, nextStatus: "CONFIRMED" | "REJECTED") { if (!run) return; const result = await api?.review({ runId: run.runId, resultId, status: nextStatus, note: reviewNotes[resultId] ?? "" }); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "Review saved: " + nextStatus : resultErrorText(result), result?.ok ? "green" : "red"); }
  async function report(format: "csv" | "html") { if (!run) return; if (format === "html") { const result = await api?.rerenderHtml(run.runId); if (result?.snapshot) update(result.snapshot); notify(result?.canceled ? "HTML regeneration cancelled." : result?.ok ? "HTML 已由共用 local renderer 重新產生。" : resultErrorText(result), result?.ok ? "green" : result?.canceled ? "amber" : "red"); return; } const result = await api?.exportRun({ runId: run.runId, format }); notify(result?.canceled ? "Report export cancelled." : result?.filePath ? "Report saved: " + result.filePath : resultErrorText(result), result?.filePath ? "green" : "red"); }
  async function openHtml() { if (!run) return; const result = await api?.openHtml(run.runId); notify(result?.ok ? "HTML opened: " + result.filePath : resultErrorText({ errorCode: result?.errorCode ?? "AI_HTML_RENDER_FAILED", message: result?.message ?? result?.error }), result?.ok ? "green" : "red"); }
  return <div className="space-y-4">
    <Section eyebrow="Active Analysis Result v1" title="Activity Events 分析結果" description="此頁只顯示已通過 durable commit 的 Active Dataset 與成功歷史；失敗診斷與對話保留在分析工作區。" actions={<button className="btn" onClick={() => void chooseAnalyzed()}><FileJson size={15} />匯入已分析 JSON</button>}>
      {!active || !run ? <div className="flex min-h-44 items-center justify-center rounded border border-dashed border-slate-300 text-sm font-semibold text-slate-500">尚無可顯示的成功 Active Result</div> : <div className="space-y-4">
        ...
        {run.htmlRenderStatus === "failed" ? <div className="rounded border border-amber-300 bg-amber-50 p-3 text-sm font-bold text-amber-950">Canonical Result 已有效建立，但自動 HTML 產生失敗。可使用下方「重新產生 HTML」按鈕；此狀態不會撤銷 Active Result。</div> : null}
        ...
        <div className="flex flex-wrap gap-2"><button className="btn" disabled={!active.htmlPath} onClick={() => void openHtml()}><FolderOpen size={15} />開啟 HTML</button><button className="btn" onClick={() => void report("csv")}>CSV</button><button className="btn btn-primary" onClick={() => void report("html")}>重新產生 HTML</button></div>
      </div>}
    </Section>
