...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  return <div className="space-y-4">
    {run ? <Section eyebrow={["completed", "completed_with_warnings"].includes(run.status) ? "Completed analysis dataset" : "Live analysis run"} title={run.status === "completed" ? "分析已完成，已自動產生資料檔" : run.status === "completed_with_warnings" ? "分析完成，但需人工確認異常" : "Activity Events 分析進度"} description={`${run.runId} · ${run.provider} · ${run.model}`} actions={run.status === "running" || run.status === "retrying" || run.status === "cancelling" ? <button className="btn btn-danger" onClick={() => void api?.cancel(run.runId)}><Square size={15} />取消分析</button> : <Badge tone={run.status === "completed" ? "green" : run.status === "completed_with_warnings" ? "amber" : run.status.includes("failed") || run.status === "provider_timeout" || run.status === "cancelled" ? "red" : "blue"}>{run.status}</Badge>}>
    ...
    {run ? <Section eyebrow="v0.3.17 canonical artifact validation" title="Result Validation" description="以下數字由 JAA 本機驗證與 canonical assembly 計算，不使用模型自述值。">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Cumulative Input" value={run.progress.usage.turnCumulative?.inputTokens ?? run.progress.usage.inputTokens ?? "Unavailable"} /><Metric label="Cached Input" value={run.progress.usage.turnCumulative?.cachedInputTokens ?? run.progress.usage.cachedInputTokens ?? "Unavailable"} /><Metric label="Cumulative Output" value={run.progress.usage.turnCumulative?.outputTokens ?? run.progress.usage.outputTokens ?? "Unavailable"} /><Metric label="Cumulative Reasoning" value={run.progress.usage.turnCumulative?.reasoningTokens ?? run.progress.usage.reasoningTokens ?? "Unavailable"} /><Metric label="Max Observed Single Call" value={run.progress.usage.maxObservedSingleCallTokens ?? "Unavailable"} /><Metric label="Max Context Utilization" value={run.progress.usage.maxObservedContextUtilizationPercent === null || run.progress.usage.maxObservedContextUtilizationPercent === undefined ? "Unavailable" : `${run.progress.usage.maxObservedContextUtilizationPercent.toFixed(1)}%`} /><Metric label="Usage Event Count" value={run.progress.usage.usageEventCount ?? 0} /><Metric label="Codex Write Probe" value={run.codexPreflight ? "Passed" : run.progress.errorCode?.startsWith("AI_OUTPUT_WRITE_PROBE") ? "Failed" : "Not run"} /></div>
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.17">
