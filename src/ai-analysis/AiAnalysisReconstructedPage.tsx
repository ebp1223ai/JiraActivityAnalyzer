...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  return <div className="space-y-4">
    ...
    {run ? <Section eyebrow="v0.3.20 canonical artifact validation" title="Result Validation" description="以下數字由 JAA 本機驗證與 canonical assembly 計算，不使用模型自述值。">
    ...
    {run ? <Section eyebrow="Review and reporting" title="Activity Events 分析結果" description="所有分類保留 Evidence reference；CONFIRMED／REJECTED 操作會寫入獨立 review audit。" actions={<><button className="btn" disabled={run.status !== "completed" && !run.warningAcceptance?.accepted} onClick={() => void report("csv")}>CSV</button><button className="btn btn-primary" disabled={run.status !== "completed" && !run.warningAcceptance?.accepted} onClick={() => void report("html")}>產生 HTML 報告</button></>}>
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.19">
