...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  async function rerenderHtml() { if (!run) return; const result = await api?.rerenderHtml(run.runId); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "HTML 已由既有 Canonical Result 重新產生；未呼叫 Provider。" : `${result?.errorCode}: ${result?.message}`, result?.ok ? "green" : "red"); }
  ...
  return <div className="space-y-4">
    ...
    {run ? <Section eyebrow="v0.3.22 verifiable analysis bridge" title="Analysis Lifecycle / 分析生命週期" description="以下狀態由本機 Bridge、驗證器與持久化流程產生，不使用模型自述值。">
    ...
    {run ? <Section eyebrow="v0.3.22 Decision v3 canonical validation" title="Result Validation" description="以下數字由 JAA 本機驗證與 canonical assembly 計算，不使用模型自述值。">
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.21">
