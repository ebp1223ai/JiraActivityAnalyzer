...
function WorkspacePage({ snapshot, update, notify, goResults }: PageProps & { goResults: () => void }) {
  ...
  const instructionPreview = instructionMode === "STANDARD_FORMAL" ? "JAA Safety Wrapper + Run Identity + bridge-resumable-v2 + Standard Formal Contract + Final Response Contract" : instructionMode === "STANDARD_PLUS_USER_INSTRUCTION" ? "JAA Safety Wrapper + Run Identity + bridge-resumable-v2 + Standard Formal Contract\n\nUser additional instruction:\n" + (supplementalInstruction || "（尚未輸入）") : "JAA Safety Wrapper + Run Identity + bridge-resumable-v2\n\nCUSTOM_DIAGNOSTIC: no canonical result, Golden HTML, or SQLite\n\nUser custom instruction:\n" + (supplementalInstruction || "（尚未輸入）");
  ...
  return <div className="space-y-4">
    <Section eyebrow="Shared analysis foundation" title="共用分析依據" description="四個角色逐一綁定並核對版本與 SHA-256；Bundled Default 與 Manual Explicit 不混用，HTML Template 僅供本機 renderer。" actions={<><button className="btn" onClick={() => void chooseRules()}><FolderOpen size={15} />明確選擇四份文件</button><button className="btn" disabled={!snapshot.rules} onClick={() => void reloadRules()}><RefreshCw size={15} />重新驗證</button></>}>
      {snapshot.rules ? <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><Metric label="Selection Mode" value={snapshot.rules.selectionMode ?? "Legacy"} /><Metric label="Rule Set ID" value={snapshot.rules.ruleSetId} /></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Validation" value={snapshot.rules.valid ? "Verified" : "Invalid"} detail={formatDate(snapshot.rules.validatedAt)} /><Metric label="Catalog records" value={snapshot.rules.catalogCount} /><Metric label="Unique Skill IDs" value={snapshot.rules.uniqueSkillIdCount ?? snapshot.rules.catalogCount} /><Metric label="Duplicate Skill IDs" value={snapshot.rules.duplicateSkillIdCount ?? 0} /></div><div className="overflow-x-auto rounded border border-slate-200"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-slate-50"><tr><th className="p-3">Role / File</th><th className="p-3">Version</th><th className="p-3">Size</th><th className="p-3">Provider</th><th className="p-3">SHA-256</th><th className="p-3">Status</th></tr></thead><tbody>{ruleDocuments.map((file) => <tr key={file.role} className="border-t border-slate-200"><td className="p-3"><div className="font-bold">{file.role}</div><div className="mt-1 text-[11px] font-semibold text-slate-600">{file.fileName}</div><code className="mt-1 block max-w-[420px] whitespace-normal break-all text-[10px] text-slate-500" title={file.fullPath}>{file.fullPath}</code></td><td className="p-3 font-mono">{file.version}</td><td className="p-3">{formatBytes(file.sizeBytes)}</td><td className="p-3"><Badge tone={file.providerVisible ? "blue" : "neutral"}>{file.providerVisible ? "Model-visible" : "Local only"}</Badge></td><td className="p-3"><code className="block max-w-52 break-all text-[10px]">{file.sha256}</code></td><td className="p-3"><Badge tone={file.status === "verified" ? "green" : "red"}>{file.status}</Badge></td></tr>)}</tbody></table></div>{snapshot.rules.htmlReportTemplate ? <div className="rounded border border-blue-200 bg-blue-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-xs font-black text-blue-950">本機 HTML Renderer（不送給 ChatGPT）</div><div className="mt-1 break-all text-xs font-semibold text-blue-800">{snapshot.rules.htmlReportTemplate.fileName}</div></div><Badge tone="green">Manifest bound / Verified</Badge></div><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Template ID" value={snapshot.rules.htmlReportTemplate.templateId} /><Metric label="Template Version" value={snapshot.rules.htmlReportTemplate.version} /><Metric label="Required Renderer" value={snapshot.rules.htmlReportTemplate.minimumRendererVersion} /><Metric label="Template sent" value="No" /></div><PathValue label="TEMPLATE FULL PATH" value={snapshot.rules.htmlReportTemplate.fullPath} /><div className="mt-2 break-all font-mono text-[10px] text-blue-800">SHA-256 {snapshot.rules.htmlReportTemplate.sha256} · {formatBytes(snapshot.rules.htmlReportTemplate.sizeBytes)}</div></div> : null}{snapshot.rules.duplicateDetails?.length ? <div className="rounded border border-red-200 bg-red-50 p-4"><div className="font-black text-red-900">ANALYSIS_RULES_DUPLICATE_SKILL_ID</div>{snapshot.rules.duplicateDetails.map((detail) => <div key={`${detail.comparison}-${detail.normalizedId}`} className="mt-3 rounded bg-white p-3 text-xs"><div className="font-black">{detail.skillId} · {detail.comparison} · {detail.severity}</div>{detail.definitions.map((definition) => <div className="mt-1 break-all font-mono text-[11px]" key={`${definition.fullPath}-${definition.lineNumber}`}>{definition.fullPath}:{definition.lineNumber} · record {definition.recordIndex} · {definition.loadSource}</div>)}</div>)}</div> : null}</div> : <div className="flex min-h-32 items-center justify-center rounded border border-dashed border-slate-300 text-sm font-semibold text-slate-500">尚未載入四角色分析依據</div>}
    </Section>
...
function ResultsPage({ snapshot, update, notify }: PageProps) {
  ...
  const [resultConversation, setResultConversation] = useState<AiAnalysisConversationEvent[]>([]);
  ...
  async function rerenderHtml() { if (!run) return; const result = await api?.rerenderHtml(run.runId); if (result?.snapshot) update(result.snapshot); notify(result?.canceled ? "離線重繪已取消。" : result?.ok ? "HTML 已由既有 Report Data Package 與所選 Template 離線重新產生；未呼叫 Provider、未讀取資料庫。" : `${result?.errorCode}: ${result?.message}`, result?.ok ? "green" : "red"); }
  async function renderExternalPackage() { const result = await api?.renderExternalPackage(); notify(result?.canceled ? "外部 Report Data Package 檢視已取消。" : result?.ok ? `外部 Package 已以 view/render-only 模式產生 HTML：${result.filePath}` : `${result?.errorCode}: ${result?.message}`, result?.canceled ? "amber" : result?.ok ? "green" : "red"); }
  async function retryDatabase() { if (!run) return; const result = await api?.retryDatabase(run.runId); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? "SQLite commit 已由既有 Canonical Result 完成；未呼叫 Provider。" : `${result?.errorCode}: ${result?.message}`, result?.ok ? "green" : "red"); }
  async function exportDebugFolder() { const result = await window.desktopApp?.appDebug?.saveBundle({ debugLog: "", currentPage: "ai-analysis" }); notify(result?.status === "failed" ? `Debug Folder export failed: ${result.errorCode ?? "unknown"}` : result?.canceled ? "Debug Folder export cancelled." : `Debug Folder exported: ${result?.folderPath ?? ""}`, result?.status === "failed" ? "red" : result?.canceled ? "amber" : "green"); }
  async function acceptWarnings(accept: boolean) { if (!run) return; const result = await api?.acceptWarnings({ runId: run.runId, accept }); if (result?.snapshot) update(result.snapshot); notify(result?.ok ? (accept ? "警告已接受，通過結構驗證的結果已寫入 SQLite。" : "已拒絕警告，本次結果維持不寫入 SQLite。") : `${result?.errorCode}: ${result?.message}`, result?.ok ? (accept ? "green" : "amber") : "red"); }
  ...
  const failed = Boolean(run && (run.status.includes("failed") || run.status === "provider_timeout" || run.status === "cancelled" || run.status === "interrupted" || run.status === "recovered_interrupted"));
  useEffect(() => {
    if (!api || !run?.runDirectory) { setResultConversation([]); return; }
    let live = true;
    void api.getConversation({ runId: run.runId, offset: 0, limit: 500 }).then((value) => {
      if (live && value.ok) setResultConversation(value.events ?? []);
    });
    return () => { live = false; };
  }, [api, run?.runId, run?.progress.stage, run?.progress.elapsedMs]);
  return <div className="space-y-4">
    {run ? <Section eyebrow={failed ? "Authoritative failure diagnostics" : ["completed", "completed_with_quality_warnings", "completed_with_persistence_error"].includes(run.status) ? "Completed analysis dataset" : "Live analysis run"} title={failed ? "AI 分析診斷結果" : "AI 分析結果"} description={`${run.runId} · ${run.provider} · ${run.model}`} actions={run.status === "running" || run.status === "retrying" || run.status === "cancelling" ? <button className="btn btn-danger" onClick={() => void api?.cancel(run.runId)}><Square size={15} />取消分析</button> : <Badge tone={run.status === "completed" ? "green" : run.status === "completed_with_quality_warnings" ? "amber" : failed ? "red" : "blue"}>{run.status}</Badge>}>
      <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Completed / Total" value={run.analyzerMode === "CHATGPT" && !run.lifecycle?.analysisCompleted ? "Awaiting authoritative checkpoint" : `${run.lifecycle?.completedCount ?? run.progress.completedDiffs} / ${run.progress.totalDiffs}`} /><Metric label="Stage" value={run.progress.stage ?? run.progress.status} /><Metric label="Duration" value={`${run.progress.elapsedMs} ms`} /><Metric label="Dispatch / Thread / Accepted / Completed" value={`${run.progress.providerDispatchCount ?? run.progress.requestCount} / ${run.progress.threadCreatedCount ?? run.progress.threadCount ?? "-"} / ${run.progress.acceptedTurnCount ?? run.progress.turnCount ?? "-"} / ${run.progress.turnCompletedCount ?? "-"}`} /><Metric label="Run Cumulative Tokens" value={run.progress.usage.turnCumulative?.totalTokens ?? run.progress.usage.totalTokens ?? "Unavailable"} /><Metric label="Last Model Call Tokens" value={run.progress.usage.lastModelCall?.totalTokens ?? "Unavailable"} /></div>{run.analyzerMode === "CHATGPT" ? <div className="rounded border border-blue-200 bg-blue-50 p-3 text-xs font-semibold text-blue-900"><div className="font-black">Authoritative stage checkpoint</div><div className="mt-1 break-words">{run.progress.message}</div></div> : <div><div className="mb-1 flex justify-between text-[11px] font-black text-slate-600"><span>{run.progress.message}</span><span>{progressPercent}%</span></div><div className="h-2 overflow-hidden rounded bg-slate-200"><div className="h-full bg-blue-600 transition-all" style={{ width: `${progressPercent}%` }} /></div></div>}<div className="grid gap-3 md:grid-cols-2"><PathValue label="SOURCE FULL PATH" value={run.sourceFilePath} /><PathValue label={run.status === "completed" ? "FINAL ANALYZED JSON FULL PATH" : "PLANNED OUTPUT JSON FULL PATH"} value={run.analyzedFilePath ?? run.plannedAnalyzedFilePath} /><PathValue label="RULES SNAPSHOT ID" value={run.rules.snapshotId ?? run.rules.ruleSetId} /><PathValue label="AI ANALYSIS SQLITE" value={run.databasePath ?? run.databaseWriteStatus ?? "Not started"} /><PathValue label="OUTPUT SCHEMA SHA-256" value={run.outputSchemaSha256 ?? "Not built"} /><PathValue label="OUTPUT SCHEMA PREFLIGHT" value={run.outputSchemaValidation ? `${run.outputSchemaValidation.isValid ? "Passed" : "Failed"} · ${run.outputSchemaValidation.findingCount} finding(s)` : "Not run"} /></div>{["completed", "completed_with_quality_warnings", "completed_with_persistence_error", "failed_validation"].includes(run.status) ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Provider / Parsed" value={`${run.providerReturnedRecordCount ?? "-"} / ${run.parsedRecordCount ?? "-"}`} /><Metric label="Schema / Semantic" value={`${run.schemaValidRecordCount ?? "-"} / ${run.semanticValidRecordCount ?? "-"}`} /><Metric label="Formal / SQLite" value={`${run.formalArtifactRecordCount ?? 0} / ${run.sqliteCommittedRecordCount ?? 0}`} /><Metric label="Candidates" value={run.results.reduce((sum, item) => sum + item.candidates.length, 0)} /><Metric label="File size" value={formatBytes(run.analyzedFileSizeBytes)} /><Metric label="Completed time" value={formatDate(run.completedAt)} /></div> : null}{run.analyzedFileSha256 ? <PathValue label="FINAL FILE SHA-256" value={run.analyzedFileSha256} /> : null}{run.progress.errorCode ? <div className="rounded border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-800">{run.progress.errorCode}: {run.progress.message}</div> : null}</div>
    </Section> : null}
    {run ? <Section eyebrow="v0.3.24 verifiable analysis bridge" title="Analysis Lifecycle / 分析生命週期" description="以下狀態由本機 Bridge、驗證器與持久化流程產生，不使用模型自述值。">
      <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Provider Turn" value={run.lifecycle?.providerTurnStatus ?? "not_started"} /><Metric label="Source Input" value={run.lifecycle?.sourceInputStatus ?? run.lifecycle?.inputStatus ?? "not_started"} /><Metric label="Model Delivery" value={run.lifecycle?.modelInputStatus ?? "not_started"} /><Metric label="Analysis" value={run.lifecycle?.analysisStatus ?? "not_started"} /><Metric label="Artifact" value={run.lifecycle?.artifactStatus ?? "not_started"} /><Metric label="Validation" value={run.lifecycle?.validationStatus ?? "not_started"} /><Metric label="Last successful stage" value={run.lifecycle?.lastSuccessfulStage ?? "RUN_CREATED"} /><Metric label="First failed stage" value={run.lifecycle?.firstFailedStage ?? "None"} /><Metric label="Root error code" value={run.lifecycle?.rootErrorCode ?? "None"} /><Metric label="Derived consequences" value={run.lifecycle?.derivedStatusCodes?.join(", ") || "None"} /><Metric label="Canonical" value={run.lifecycle?.canonicalStatus ?? "not_created"} /><Metric label="analysisStarted" value={String(run.lifecycle?.analysisStarted ?? false)} /><Metric label="analysisCompleted" value={String(run.lifecycle?.analysisCompleted ?? false)} /><Metric label="Completed / Decisions" value={`${run.lifecycle?.completedCount ?? 0} / ${run.lifecycle?.decisionPreparedCount ?? 0}`} /><Metric label="HTML Render" value={run.lifecycle?.htmlRenderStatus ?? run.htmlRenderStatus ?? "not_started"} /><Metric label="SQLite" value={run.lifecycle?.sqliteStatus ?? "blocked"} /></div><div className="grid gap-3 md:grid-cols-2"><PathValue label="BRIDGE VERSION / SHA-256" value={run.bridgeEvidence ? `${run.bridgeEvidence.version} · ${run.bridgeEvidence.sha256}` : "Not loaded"} /><PathValue label="BRIDGE PREFLIGHT" value={run.bridgeEvidence ? `${run.bridgeEvidence.integrity} · ${run.bridgeEvidence.transport} · localOnly=${run.bridgeEvidence.localOnly}` : "Not run"} /></div></div>
    </Section> : null}
    {failed && resultConversation.length ? <Section eyebrow="Durable run archive" title="完整分析對話" description="顯示失敗前已持久化的使用者指令、Bridge 事件、Provider 回應與 App 診斷。"><div className="max-h-[32rem] space-y-2 overflow-y-auto rounded border border-slate-200 bg-slate-50 p-3">{resultConversation.map((event) => <div key={event.eventId} className="rounded border border-slate-200 bg-white p-3 text-xs"><div className="flex flex-wrap justify-between gap-2 font-black text-slate-700"><span>{event.sequence}. {event.role} / {event.type}</span><span>{formatDate(event.atUtc)}</span></div><div className="mt-2 whitespace-pre-wrap break-words text-slate-700">{event.content}</div></div>)}</div></Section> : null}
    {run ? <Section eyebrow="v0.3.24 Decision v4 aggregated validation" title="Result Validation" description="以下數字由 JAA 本機驗證與 canonical assembly 計算，不使用模型自述值。">
      <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><Metric label="Expected records" value={run.expectedDecisionCount ?? run.selectedDiffIds.length} /><Metric label="AI decisions received" value={`${run.receivedDecisionCount ?? 0} / ${run.expectedDecisionCount ?? run.selectedDiffIds.length}`} /><Metric label="Canonical records assembled" value={run.canonicalRecordCount ?? 0} /><Metric label="Schema valid records" value={run.schemaValidRecordCount ?? 0} /><Metric label="Semantic valid records" value={run.semanticValidRecordCount ?? 0} /><Metric label="Final accepted records" value={run.finalAcceptedRecordCount ?? 0} /><Metric label="Unique Issues" value={run.uniqueIssueCount ?? "Unavailable"} /><Metric label="Decision Contract" value={run.decisionContractVersion ?? "Legacy / unavailable"} /><Metric label="Quality Gate" value={run.qualityGate?.status ?? "Not evaluated"} /></div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Cumulative Input" value={run.progress.usage.turnCumulative?.inputTokens ?? run.progress.usage.inputTokens ?? "Unavailable"} /><Metric label="Cached Input" value={run.progress.usage.turnCumulative?.cachedInputTokens ?? run.progress.usage.cachedInputTokens ?? "Unavailable"} /><Metric label="Cumulative Output" value={run.progress.usage.turnCumulative?.outputTokens ?? run.progress.usage.outputTokens ?? "Unavailable"} /><Metric label="Cumulative Reasoning" value={run.progress.usage.turnCumulative?.reasoningTokens ?? run.progress.usage.reasoningTokens ?? "Unavailable"} /><Metric label="Max Observed Single Call" value={run.progress.usage.maxObservedSingleCallTokens ?? "Unavailable"} /><Metric label="Max Context Utilization" value={run.progress.usage.maxObservedContextUtilizationPercent === null || run.progress.usage.maxObservedContextUtilizationPercent === undefined ? "Unavailable" : `${run.progress.usage.maxObservedContextUtilizationPercent.toFixed(1)}%`} /><Metric label="Usage Event Count" value={run.progress.usage.usageEventCount ?? 0} /><Metric label="Bridge Integrity" value={run.bridgeEvidence?.integrity ?? "Not run"} /></div>
      {run.anomalyWarnings?.length ? <div className="rounded border border-amber-300 bg-amber-50 p-4 text-xs text-amber-950"><div className="font-black">Anomaly warnings</div><div className="mt-2 flex flex-wrap gap-2">{run.anomalyWarnings.map((warning) => <Badge key={warning} tone="amber">{warning}</Badge>)}</div><p className="mt-2 font-semibold">結構驗證已通過，但重大異常預設阻擋 SQLite。人工確認只接受 anomaly，不會略過 schema、count、hash 或 identity 驗證。</p>{run.status === "completed_with_quality_warnings" && !run.warningAcceptance?.accepted ? <div className="mt-3 flex flex-wrap gap-2"><button className="btn btn-primary" onClick={() => void acceptWarnings(true)}><Check size={15} />接受警告並寫入 SQLite</button><button className="btn" onClick={() => void acceptWarnings(false)}><XCircle size={15} />拒絕本次寫入</button></div> : null}</div> : null}
      <div className="grid gap-3 md:grid-cols-2"><PathValue label="AI DECISIONS" value={run.artifactPaths ? `${run.artifactPaths.output}\\ai-analysis-decisions.json` : null} /><PathValue label="ANALYSIS REPORT" value={run.analysisReportPath} /><PathValue label="VALIDATION REPORT" value={run.validationReportPath} /><PathValue label="COMPLETION MANIFEST" value={run.completionManifestPath} /><PathValue label="GOLDEN HTML" value={run.reportFilePath} /><PathValue label="HTML RENDER RECEIPT" value={run.htmlRenderReceiptPath} /><PathValue label="DATABASE COMMIT RECEIPT" value={run.databaseCommitReceiptPath} /><PathValue label="DATABASE FAILURE EVIDENCE" value={run.databaseCommitFailurePath} /><PathValue label="QUALITY GATE REPORT" value={run.qualityGateReportPath} /><PathValue label="EVIDENCE NORMALIZATION RECEIPT" value={run.evidenceNormalizationReceiptPath} /><PathValue label="ISSUE SNAPSHOT" value={run.issueSnapshotPath} /><PathValue label="ISSUE SNAPSHOT RECEIPT" value={run.issueSnapshotReceiptPath} /><PathValue label="EVIDENCE SEGMENT CATALOG" value={run.evidenceSegmentCatalogPath} /><PathValue label="FORMAL REPORT DATA PACKAGE" value={run.reportDataPackagePath} /><PathValue label="DIAGNOSTIC REPORT DATA PACKAGE" value={run.diagnosticReportDataPackagePath} /><PathValue label="DIAGNOSTIC HTML" value={run.diagnosticReportFilePath} /></div><div className="flex flex-wrap gap-2"><button className="btn btn-primary" disabled={!run.reportFilePath || run.htmlRenderStatus !== "completed"} onClick={() => void openHtml()}><FolderOpen size={15} />開啟 HTML</button><button className="btn" disabled={!run.reportDataPackagePath} onClick={() => void rerenderHtml()}><RefreshCw size={15} />選擇 Template 離線重繪</button><button className="btn" disabled={!run.reportFilePath} onClick={() => void navigator.clipboard.writeText(run.reportFilePath ?? "")}><FileCheck2 size={15} />複製 HTML 路徑</button><button className="btn" disabled={run.lifecycle?.canonicalStatus !== "created" || !run.validationGate?.passed} onClick={() => void retryDatabase()}><Database size={15} />重試資料庫寫入</button><button className="btn" disabled={!run.htmlRenderDiagnosticsPath && !run.runDirectory} onClick={() => void api?.openFolder(run.htmlRenderDiagnosticsPath ?? run.runDirectory ?? undefined)}><AlertTriangle size={15} />查看診斷</button></div>{run.status === "completed_with_persistence_error" ? <div className="rounded border border-amber-300 bg-amber-50 p-3 text-xs font-bold text-amber-950">分析產物與 Canonical Result 有效且可檢視，但資料庫寫入失敗。可在不呼叫 Provider 的情況下重試資料庫寫入。</div> : null}<div className="rounded border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-black text-slate-800">SQLite write gate</div><div className="mt-1 break-words text-xs font-semibold text-slate-600">{run.databaseWriteStatus ?? "Not eligible"}</div></div></div>
    </Section> : null}
    {run?.analysisReportContent ? <Section eyebrow="AI-authored artifact" title="Analysis Report" description="安全純文字顯示；不執行 embedded HTML、script、remote content 或 URI。"><pre className="max-h-[34rem] overflow-auto whitespace-pre-wrap break-words rounded border border-slate-200 bg-slate-50 p-4 text-xs leading-relaxed text-slate-800">{run.analysisReportContent}</pre>{run.finalAssistantMessage ? <div className="mt-3 rounded border border-blue-200 bg-blue-50 p-3 text-xs font-semibold text-blue-900"><div className="mb-1 font-black">Final assistant summary</div>{run.finalAssistantMessage}</div> : null}</Section> : null}
    <Section eyebrow="Analyzed activity events dataset" title="選擇資料檔" description="匯入既有 completed analyzed JSON 供檢視；Partial、Cancelled、Failed 與 schema 不符資料會被拒絕。" actions={<><button className="btn" onClick={() => void chooseAnalyzed()}><FileJson size={15} />匯入已分析 JSON</button><button className="btn" onClick={() => void renderExternalPackage()}><RefreshCw size={15} />外部 Package 檢視／重繪</button></>}>
      <div className="grid min-w-0 gap-4 xl:grid-cols-[300px_minmax(0,1fr)]"><div className="max-h-72 space-y-2 overflow-y-auto">{snapshot.runs.map((item) => <button key={item.runId} className={`w-full min-w-0 rounded border p-3 text-left ${item.runId === snapshot.selectedRunId ? "border-blue-500 bg-blue-50" : "border-slate-200"}`} onClick={async () => { const result = await api?.selectRun(item.runId); if (result?.snapshot) update(result.snapshot); }}><div className="truncate text-xs font-black" title={item.analyzedFileName}>{item.analyzedFileName}</div><div className="mt-1 text-[11px] font-semibold text-slate-500">{item.provider} · {item.results.length} results</div><div className="mt-1 text-[10px] text-slate-400">{formatDate(item.completedAt)}</div></button>)}</div>{run ? <div className="space-y-3"><PathValue label="ANALYZED DATASET FULL PATH" value={run.analyzedFilePath ?? run.plannedAnalyzedFilePath} /><PathValue label="RUN ARCHIVE" value={run.runDirectory} /><div className="flex flex-wrap gap-2"><button className="btn" disabled={!run.runDirectory} onClick={() => void api?.openFolder(run.runDirectory!)}><FolderOpen size={15} />開啟 Run 資料夾</button><button className="btn" onClick={() => void exportDebugFolder()}><FileCheck2 size={15} />匯出 Debug Folder</button></div><button className="btn btn-danger" disabled={!run.runDirectory || ["running", "queued", "retrying", "cancelling"].includes(run.status)} onClick={async () => { const deleted = await api?.deleteRun(run.runId); if (deleted?.snapshot) update(deleted.snapshot); }}>Delete local Run archive</button><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Source count" value={run.selectedDiffIds.length} /><Metric label="Result count" value={run.results.length} /><Metric label="Review" value={counts.review} /><Metric label="Excluded / Unknown" value={`${counts.excluded} / ${counts.unknown}`} /></div></div> : <div className="flex min-h-40 items-center justify-center rounded border border-dashed border-slate-300 text-sm font-semibold text-slate-500">尚無已分析資料</div>}</div>
    </Section>
    {run ? <Section eyebrow="Review and reporting" title="Activity Events 分析結果" description="所有分類保留 Evidence reference；CONFIRMED／REJECTED 操作會寫入獨立 review audit。" actions={<><button className="btn" disabled={run.lifecycle?.canonicalStatus !== "created"} onClick={() => void report("csv")}>CSV</button><button className="btn btn-primary" disabled={run.lifecycle?.canonicalStatus !== "created"} onClick={() => void report("html")}>產生 HTML 報告</button></>}>
      ...
      <div className="overflow-x-auto rounded border border-slate-200"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-slate-50"><tr><th className="p-3">Evidence / Source Diff</th><th className="p-3">Status</th><th className="p-3">Skill candidates</th><th className="p-3">Evidence refs</th><th className="w-72 p-3">Review</th></tr></thead><tbody>{results.map((result) => <tr className="border-t border-slate-200 align-top" key={result.resultId}><td className="p-3"><code className="block max-w-64 break-all text-[10px] font-bold text-blue-700">{result.sourceDiffId}</code></td><td className="p-3"><Badge tone={resultTone(result.status)}>{result.status}</Badge></td><td className="p-3">{result.candidates.length ? result.candidates.map((candidate) => <div className="mb-2 rounded bg-slate-50 p-2 last:mb-0" key={candidate.skillId}><div className="font-black">{candidate.skillId} · {candidate.skillName} <span className="text-blue-700">{candidate.score ?? "-"}</span></div><div className="mt-1 break-words text-[11px] text-slate-600">{candidate.reason}</div>{candidate.evidenceExplanation ? <div className="mt-1 break-words text-[11px] text-slate-700"><b>Evidence：</b>{candidate.evidenceExplanation}</div> : null}{candidate.evidenceQuotes?.map((evidence) => <blockquote key={`${candidate.skillId}-${evidence.evidenceRef}-${evidence.quote}`} className="mt-1 border-l-2 border-blue-300 pl-2 text-[11px] text-slate-600">{evidence.quote}</blockquote>)}</div>) : <span className="text-slate-500">No candidate</span>}</td><td className="p-3">{result.evidenceRefs.map((reference) => <code className="mb-1 block max-w-56 break-all text-[10px]" key={reference}>{reference}</code>)}</td><td className="p-3"><input className="field py-2 text-xs" value={reviewNotes[result.resultId] ?? result.reviewNote} onChange={(event) => setReviewNotes((current) => ({ ...current, [result.resultId]: event.target.value }))} placeholder="Review note" /><div className="mt-2 flex gap-2"><button className="btn flex-1 py-2" onClick={() => void review(result.resultId, "CONFIRMED")}><Check size={13} />Confirm</button><button className="btn flex-1 py-2" onClick={() => void review(result.resultId, "REJECTED")}><XCircle size={13} />Reject</button></div></td></tr>)}</tbody></table></div>{!results.length ? <div className="py-12 text-center text-sm font-semibold text-slate-500">No matching analysis results</div> : null}
    </Section> : null}
  </div>;
}
...
export function AiAnalysisReconstructedPage() {
  ...
  return (
    <div className="min-w-0 pb-8" data-ui-version="0.3.24">
