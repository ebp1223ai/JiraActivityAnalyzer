...
export function DebugLogPanel({ page }: Props) {
  ...
  return (
    <aside className="flex h-screen w-[320px] min-w-[320px] max-w-[320px] shrink-0 flex-col overflow-hidden border-l border-line bg-white p-4">
      <h2 className="truncate text-lg font-black text-ink" title="除錯紀錄 / Debug Log">除錯紀錄 / Debug Log</h2>
      {page === "timeline" ? (
        <div className="mt-4 w-fit rounded-full bg-green-50 px-3 py-2 text-sm font-bold text-green-700">
          sessionStorage enabled
        </div>
      ) : null}
      <div className="mt-4 flex flex-wrap gap-2">
        <button className="btn min-w-[72px] flex-1 px-3" title="Copy logs">
          <Copy size={16} />
          <span>Copy</span>
        </button>
        <button className="btn min-w-[46px] px-3" title="Download TXT">
          <Download size={16} />
          <span className="sr-only">Download TXT</span>
        </button>
        <button className="btn btn-danger min-w-[46px] px-3" title="Clear logs">
          <Trash2 size={16} />
          <span>Clear</span>
        </button>
      </div>
      <div className="thin-scroll mt-4 min-h-0 flex-1 overflow-auto rounded-lg border border-line p-3">
        {logs.concat(logs.slice(0, 4)).map((log, index) => {
          ...
          return (
            <div key={`${log}-${index}`} className="grid min-w-0 grid-cols-[72px_44px_minmax(0,1fr)] gap-2 py-2 text-xs">
              <span className="font-mono text-slate-500">15:4{index}:21.{120 + index}</span>
              <span className={`h-fit rounded px-1.5 py-0.5 text-[10px] font-black ${level === "INFO" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                {level}
              </span>
      ...
      <div className="mt-4 rounded-lg border border-line p-4 text-xs">
        ...
        <div className="space-y-2 font-semibold text-muted">
          <div><span className="text-red-500">● ERROR</span> 輸入或系統錯誤</div>
      ...
      <div className="mt-5 flex min-w-0 items-center justify-between gap-3 text-sm font-black text-ink">
