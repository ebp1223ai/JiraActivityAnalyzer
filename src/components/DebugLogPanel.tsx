...
export function DebugLogPanel({ page }: Props) {
  ...
  return (
    <aside className="flex h-screen w-[320px] shrink-0 flex-col overflow-hidden border-l border-line bg-white p-4">
      ...
      <div className="thin-scroll mt-4 min-h-0 flex-1 overflow-auto rounded-lg border border-line p-3">
        {logs.concat(logs.slice(0, 4)).map((log, index) => {
          ...
          return (
            <div key={`${log}-${index}`} className="grid grid-cols-[72px_44px_minmax(0,1fr)] gap-2 py-2 text-xs">
