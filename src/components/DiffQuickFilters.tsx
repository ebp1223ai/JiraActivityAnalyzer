...
export function DiffQuickFilters({ value, disabled = false, onChange }: { value: DiffQuickFilterState; disabled?: boolean; onChange: (value: DiffQuickFilterState) => void }) {
  return <fieldset className="min-w-0 rounded-md border border-line bg-slate-50 p-3" disabled={disabled} data-testid="diff-quick-filters">
    <legend className="px-1 text-xs font-black text-muted">Diff Quick Filters / 差異快速篩選</legend>
    <div className="flex flex-wrap gap-x-5 gap-y-2">
      {controls.map(([key, label, sub]) => <label key={key} className="flex items-center gap-2 text-sm font-bold">
