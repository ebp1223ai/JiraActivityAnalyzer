  ...
  interface Window {
    desktopApp?: {
      ...
      userAnalysis?: {
        ...
        fullFetch: (payload: { connection: JiraConnection; fetchQueue: unknown[]; fetchLimit: number; batchSize: number | "all"; rawDataMode: "summary_only" | "auto_save_raw_per_issue" | "full_raw_in_memory"; selectedUser: string; startDate: string; endDate: string; directIssueKeys: string[]; resumeStagingId?: string }) => Promise<Record<string, unknown>>;
        ...
        scanFullFetchStaging: () => Promise<{ found: boolean; state?: Record<string, unknown>; checkpoint?: Record<string, unknown>; preview?: Record<string, unknown> }>;
        fullFetchStagingAction: (payload: { stagingId: string; action: "resume" | "export_completed" | "discard" | "decide_later" }) => Promise<Record<string, unknown>>;
