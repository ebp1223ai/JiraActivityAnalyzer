...
const activityEventColumns: SqliteTableColumn[] = [
  ...
  { id: "before", label: "Before", kind: "text", width: 320, render: (row, context) => isDescriptionDiffRow(row) ? <DescriptionOriginalPreviewCell row={row} side="before" onViewFull={() => context.setExpanded(true)} /> : <ReadableContentCell value={activityEventBefore(row)} missing="No previous value" /> },
  { id: "after", label: "After", kind: "text", width: 320, render: (row, context) => isDescriptionDiffRow(row) ? <DescriptionOriginalPreviewCell row={row} side="after" onViewFull={() => context.setExpanded(true)} /> : <ReadableContentCell value={activityEventAfter(row)} formatHint={String(row.commentBodyFormat ?? "")} /> },
...
const changelogColumns: SqliteTableColumn[] = [
  ...
  { id: "before", label: "Before", kind: "text", width: 320, render: (row, context) => isDescriptionDiffRow(row) ? <DescriptionOriginalPreviewCell row={row} side="before" onViewFull={() => context.setExpanded(true)} /> : <ReadableContentCell value={row.before} missing={row.beforeAvailable === false ? "Before unavailable" : "Empty"} /> },
  { id: "after", label: "After", kind: "text", width: 320, render: (row, context) => isDescriptionDiffRow(row) ? <DescriptionOriginalPreviewCell row={row} side="after" onViewFull={() => context.setExpanded(true)} /> : <ReadableContentCell value={row.after} missing={row.afterAvailable === false ? "After unavailable" : "Empty"} /> },
