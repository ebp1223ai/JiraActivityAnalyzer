...
export function PrecisionProbePage() {
  ...
  async function runActivityStreamOnly() {
    ...
    try {
      const response = await window.desktopApp?.userAnalysis?.activityStreamProbe?.({
        connection: smokeConnection()!, activityStreamUser: userAnalysis.activityStreamUser.trim(), startDate: userAnalysis.startDate,
      ...
      patchState({ activityStream, precisionProbeStatus: activityStream.status === "success" ? "completed" : "failed", precisionProbeErrors: activityStream.error ? [activityStream.error] : [], notice: `Activity Stream Probe completed: ${activityStream.parsedActivityCount} activities. / Activity Stream 測試完成。` });
    } catch (error) {
  ...
  function addToFetchQueue() {
    ...
    setUserAnalysis((current) => {
      ...
      return { ...current, candidateIssues: Array.from(issues.values()), selectedForFetch: Array.from(new Set([...current.selectedForFetch, ...current.uniquePreciseIssueKeys])), notice: `${current.uniquePreciseIssueKeys.length} candidate(s) added to User Analysis Fetch Queue. Full Fetch was not started. / 已加入使用者分析抓取佇列，未自動執行完整抓取。` };
    });
  }
  ...
  return <>
    ...
    <SectionCard title="Activity Stream Result" subtitle="Activity Stream 結果" className="mb-4">
      <div className="mb-4 grid min-w-0 grid-cols-[repeat(auto-fit,minmax(170px,1fr))] gap-3"><MiniStat label="Supported / 是否支援" value={stream.supported} /><MiniStat label="HTTP Status / HTTP 狀態" value={stream.httpStatus} /><MiniStat label="Content Type / 內容類型" value={stream.contentType || "-"} /><MiniStat label="Parsed Activities / 活動數" value={stream.parsedActivityCount} /><MiniStat label="Parsed Issue Keys / Jira 數" value={stream.parsedIssueKeys.length} /></div>
      ...
      <ResponsiveTableContainer><table className="table min-w-[900px]" data-testid="activity-stream-results"><thead><tr>{["Time / 時間", "Issue Key / Jira", "Activity Type / 活動類型", "Title / 標題", "Author / 作者", "Source / 來源"].map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{stream.entriesSanitized.map((entry, index) => <tr key={`${entry.issueKey}-${entry.activityTime}-${index}`}><td>{entry.activityTime || "-"}</td><td className="font-black text-blue-700">{entry.issueKey || "-"}</td><td>{entry.activityType}</td><td><span className="block max-w-[360px] truncate" title={entry.activityTitle} data-allow-truncate="true">{entry.activityTitle || "-"}</span></td><td>{entry.activityAuthor || "-"}</td><td>{entry.source}</td></tr>)}{stream.entriesSanitized.length === 0 ? <tr><td colSpan={6} className="text-center text-muted">No parsed Activity Stream entries / 尚無解析結果</td></tr> : null}</tbody></table></ResponsiveTableContainer>
