...
export function PrecisionProbePage() {
  ...
  const [probeMode, setProbeMode] = useState<"precision" | "stability" | "roundComparison" | "windowDiagnostics" | "raw">(new URLSearchParams(location.search).get("mode") === "stability" ? "stability" : "precision");
  const [probeRoundExecutionMode, setProbeRoundExecutionMode] = useState<RoundExecutionMode>("force_all_rounds");
  const [stabilityRun, setStabilityRun] = useState<any>(null);
  ...
  const [stabilitySort, setStabilitySort] = useState<"round" | "window" | "attempt" | "duration">("round");
  const [stabilityColumnsOpen, setStabilityColumnsOpen] = useState(false);
  const [stabilityVisibleColumns, setStabilityVisibleColumns] = useState(["round", "status", "duration", "raw", "unique", "primary", "referenced", "newPrevious", "missingPrevious", "eventFingerprint", "primaryFingerprint", "consistency", "coldStart"]);
  const [windowColumnsOpen, setWindowColumnsOpen] = useState(false);
  const [windowSort, setWindowSort] = useState<"round_window" | "duration" | "date">("round_window");
  const [windowVisibleColumns, setWindowVisibleColumns] = useState(["round", "window", "date", "raw", "unique", "primary", "referenced", "newEvents", "missingEvents", "duration", "status"]);
  ...
  const stabilityTabs = <div data-testid="precision-probe-modes" className="mb-4 flex min-w-0 flex-wrap gap-2 border-b border-line pb-3">{([ ["precision", "Precision Discovery / 精準探索"], ["stability", "Activity Stream Stability / Activity Stream 穩定性"], ["roundComparison", "Round Comparison / 輪次比較"], ["windowDiagnostics", "Window Diagnostics / 視窗診斷"], ["raw", "Raw Results / Diagnostics / 原始結果與診斷"] ] as const).map(([value, label]) => <button key={value} data-testid={`probe-mode-${value}`} className={`btn ${probeMode === value ? "btn-primary" : ""}`} type="button" onClick={() => setProbeMode(value)}>{label}</button>)}</div>;
  ...
  function WindowDiagnosticsV2() {
    const labels: Record<string, string> = { round: "Round / 輪次", window: "Window / 視窗", date: "Date Range / 日期範圍", raw: "Raw Events / 原始事件", unique: "Unique Events / 不重複事件", primary: "Primary Jira Keys / 主要 Jira", referenced: "Referenced Jira Keys / 參照 Jira", newEvents: "New Events / 新增事件", missingEvents: "Missing Events / 缺少事件", duration: "Duration / 耗時", status: "Status / 狀態" };
