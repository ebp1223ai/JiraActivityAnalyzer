...
import type { ActivityStreamMergeStrategy, ActivityStreamRequestWindowType } from "../../electron/activityStreamStability";
import type { ActivityStreamProbeRunV2, RoundExecutionMode } from "../../electron/activityStreamRoundStability";
...
function roundStatusLabel(value: string) {
  return value === "completed" ? "Completed / 已完成" : value === "partial" ? "Partial / 部分完成" : value === "cancelled" ? "Cancelled / 已取消" : value === "failed" ? "Failed / 失敗" : value;
}

function stabilityLabel(value: string) {
  return value === "stable" ? "Stable / 穩定" : value === "probably_stable" ? "Probably Stable / 可能穩定" : value === "unstable" ? "Unstable / 不穩定" : value === "insufficient_rounds" ? "Insufficient Rounds / 輪次不足" : value;
}
...
export function PrecisionProbePage() {
  ...
  const probeRoundExecutionMode = userAnalysis.stabilityRoundExecutionMode as RoundExecutionMode;
  const stabilityRun: any = userAnalysis.stabilityProbeRun;
  // Legacy v1 controls remain read-only implementation details until old export rendering is removed.
  const [stabilityRetryDelayMs, setStabilityRetryDelayMs] = useState(1000);
  ...
  const stabilityProgress = userAnalysis.stabilityProgress;
  const stabilityRunning = userAnalysis.stabilityRunning;
  const stabilityStatusFilter = userAnalysis.stabilityStatusFilter;
  const stabilitySort = userAnalysis.stabilitySort;
  const [stabilityColumnsOpen, setStabilityColumnsOpen] = useState(false);
  const stabilityVisibleColumns = userAnalysis.stabilityVisibleColumns;
  const [windowColumnsOpen, setWindowColumnsOpen] = useState(false);
  const windowSort = userAnalysis.windowSort;
  const windowVisibleColumns = userAnalysis.windowVisibleColumns;
  ...
  const setProbeMode = (value: typeof userAnalysis.stabilityActiveTab) => patchState({ stabilityActiveTab: value });
  const setProbeRoundExecutionMode = (value: RoundExecutionMode) => patchState({ stabilityRoundExecutionMode: value });
  const setStabilityRun = (value: ActivityStreamProbeRunV2 | null) => patchState({ stabilityProbeRun: value });
  const setStabilityProgress = (value: Record<string, unknown>) => patchState({ stabilityProgress: value });
  const setStabilityRunning = (value: boolean) => patchState({ stabilityRunning: value });
  const setStabilityStatusFilter = (value: string) => patchState({ stabilityStatusFilter: value });
  const setStabilitySort = (value: typeof userAnalysis.stabilitySort) => patchState({ stabilitySort: value });
  const setStabilityVisibleColumns = (value: string[] | ((current: string[]) => string[])) => setUserAnalysis((current) => ({ ...current, stabilityVisibleColumns: typeof value === "function" ? value(current.stabilityVisibleColumns) : value }));
  const setWindowSort = (value: typeof userAnalysis.windowSort) => patchState({ windowSort: value });
  const setWindowVisibleColumns = (value: string[] | ((current: string[]) => string[])) => setUserAnalysis((current) => ({ ...current, windowVisibleColumns: typeof value === "function" ? value(current.windowVisibleColumns) : value }));
  ...
  useEffect(() => window.desktopApp?.userAnalysis?.onStabilityProbeProgress?.((progress) => {
    ...
    appendDebugLog("precision", [`[${level}][stability-probe] runId=${String(progress.probeRunId || "-")} round=${String(progress.currentRound || 0)}/${String(progress.totalRounds || 0)} window=${String(progress.currentWindow || 0)}/${String(progress.totalWindows || 0)} stage=${String(progress.stage || "-")} duration=${String(progress.currentApiDurationMs || 0)}ms`]);
  }), [appendDebugLog]);

  useEffect(() => window.desktopApp?.userAnalysis?.onActivityStreamBenchmarkProgress?.((progress) => {
    patchState({ activityStreamBenchmarkProgress: progress });
    appendDebugLog("precision", [`[INFO][benchmark] benchmarkRunId=${String(progress.benchmarkRunId || "-")} stage=${String(progress.status || "running")} run=${String(progress.currentRun || 0)}/${String(progress.totalRuns || 0)}`]);
  }), [appendDebugLog]);

  useEffect(() => {
    void window.desktopApp?.userAnalysis?.updateStabilityUiState?.({ activeTab: probeMode, filters: { status: stabilityStatusFilter }, sort: { rounds: stabilitySort, windows: windowSort }, visibleColumns: { rounds: stabilityVisibleColumns, windows: windowVisibleColumns }, latestProbeRunId: userAnalysis.stabilityProbeRun?.probeRunId ?? "" });
  }, [probeMode, stabilityStatusFilter, stabilitySort, windowSort, stabilityVisibleColumns, windowVisibleColumns, userAnalysis.stabilityProbeRun?.probeRunId]);

  async function runBenchmark() {
    if (!activeConnection || userAnalysis.activityStreamBenchmarkRunning) return;
    patchState({ activityStreamBenchmarkRunning: true, activityStreamBenchmarkProgress: { status: "starting", completedRuns: 0 } });
    logAction("USER_ACTION", "Button clicked: Run Activity Stream Benchmark / 執行 Activity Stream 效能基準");
    try {
      const result = await window.desktopApp?.userAnalysis?.activityStreamBenchmark?.({ connection: activeConnection, config: userAnalysis.activityStreamBenchmarkConfig });
      if (!result) throw new Error("Benchmark IPC is unavailable. / 效能基準 IPC 無法使用。");
      patchState({ activityStreamBenchmarkRun: result as unknown as ActivityStreamBenchmarkRun, activityStreamBenchmarkRunning: false });
      appendDebugLog("precision", Array.isArray(result.logs) ? result.logs.map(String) : ["[INFO] Activity Stream Benchmark completed / 效能基準完成"]);
    } catch (error) {
      patchState({ activityStreamBenchmarkRunning: false });
      appendDebugLog("precision", [`[ERROR][benchmark] ${error instanceof Error ? error.message : String(error)}`]);
    }
  }

  async function cancelBenchmark() {
    logAction("USER_ACTION", "Button clicked: Cancel Benchmark / 取消效能基準");
    await window.desktopApp?.userAnalysis?.cancelActivityStreamBenchmark?.();
  }
  ...
  async function executeStabilityProbe(confirmedLargeRun = false) {
    ...
    logAction("USER_ACTION", `Button clicked: Run Stability Probe window=${userAnalysis.activityStreamRequestWindow} rounds=${userAnalysis.activityStreamFullScanRoundCount} mode=${probeRoundExecutionMode} merge=${userAnalysis.activityStreamMergeStrategy}`);
    try {
  ...
  function applyStabilityRecommendation() {
    if (!stabilityRun) return;
    patchState({ activityStreamRequestWindow: stabilityRun.recommendation.recommendedRequestWindow, activityStreamFullScanRoundCount: stabilityRun.recommendation.recommendedRoundCount, activityStreamRoundExecutionMode: stabilityRun.recommendation.recommendedExecutionMode, activityStreamMergeStrategy: "union" });
    logAction("USER_ACTION", `Applied Stability Probe recommendation: window=${stabilityRun.recommendation.recommendedRequestWindow} rounds=${stabilityRun.recommendation.recommendedRoundCount} merge=union`);
  }
  ...
  const stabilityTabs = <div data-testid="precision-probe-modes" className="mb-4 flex min-w-0 flex-wrap gap-2 border-b border-line pb-3">{([ ["precision", "Precision Discovery / 精準探索"], ["stability", "Activity Stream Stability / Activity Stream 穩定性"], ["roundComparison", "Round Comparison / 輪次比較"], ["windowDiagnostics", "Window Diagnostics / 視窗診斷"], ["raw", "Raw Results / Diagnostics / 原始結果與診斷"], ["benchmark", "Activity Stream Benchmark / Activity Stream 效能基準"] ] as const).map(([value, label]) => <button key={value} data-testid={`probe-mode-${value}`} className={`btn ${probeMode === value ? "btn-primary" : ""}`} type="button" onClick={() => setProbeMode(value)}>{label}</button>)}</div>;
  const stabilityWindowCount = estimatedStabilityWindows();
  const stabilityTotalRequests = stabilityWindowCount * userAnalysis.activityStreamFullScanRoundCount;
  ...
  function StabilitySummary() {
    ...
    return <><SectionCard title="Window Summary" subtitle="視窗穩定度摘要" className="mb-4"><ResponsiveTableContainer><table data-testid="stability-window-summary" className="table min-w-[1100px]"><thead><tr>{["Window", "Attempts", "First Stable", "Union Events", "Union Jira Keys", "Intersection", "Stability", "Recommended Retry"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{stabilityRun.windows.map((window: any) => <tr key={window.windowId}><td>{window.start} ~ {window.end}</td><td>{window.attempts.length}</td><td>{window.stability.firstStableAttempt ?? "-"}</td><td>{window.finalUnionEventCount}</td><td>{window.finalUnionJiraKeyCount}</td><td>{window.finalIntersectionEventCount}</td><td><StatusBadge>{window.stability.classification}</StatusBadge></td><td>{window.recommendedRetryCount}</td></tr>)}</tbody></table></ResponsiveTableContainer></SectionCard><SectionCard title="Overall Recommendation" subtitle="整體建議" className="mb-4"><div data-testid="stability-recommendation" className="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-3"><MiniStat label="Request Window" value={stabilityRun.recommendation.recommendedRequestWindow} /><MiniStat label="Retry Count" value={stabilityRun.recommendation.recommendedRetryCount} /><MiniStat label="Stable Attempt P50" value={stabilityRun.recommendation.observedStableAttemptP50 ?? "N/A"} /><MiniStat label="Stable Attempt Max" value={stabilityRun.recommendation.observedStableAttemptMax ?? "N/A"} /><MiniStat label="Unstable Windows" value={stabilityRun.recommendation.unstableWindowCount} /><MiniStat label="Cold Start Windows" value={stabilityRun.recommendation.coldStartAffectedWindowCount} /></div><div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-semibold text-blue-950">{stabilityRun.recommendation.recommendationReason}</div>{stabilityRun.mergeFallbackReason ? <div data-testid="stability-fallback-warning" className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm font-bold text-amber-900">{stabilityRun.mergeFallbackReason}</div> : null}<button data-testid="apply-stability-recommendation" className="btn btn-primary mt-4" type="button" onClick={applyStabilityRecommendation}>Apply Recommendation to User Analysis</button></SectionCard></>;
  }

  const roundRun = stabilityRun as (ActivityStreamProbeRunV2 & { files?: Record<string, string> }) | null;
  const completedRounds = roundRun?.rounds.filter((round) => round.status === "completed") ?? [];
  const roundConsistency = (roundNumber: number) => roundNumber < 2 ? "N/A" : roundRun?.rounds[roundNumber - 2]?.roundEventSetFingerprint === roundRun?.rounds[roundNumber - 1]?.roundEventSetFingerprint ? "100%" : "Changed / 有變更";

  function RoundSetupV2() {
    const modeText = probeRoundExecutionMode === "force_all_rounds" ? "Force All Rounds / 強制執行全部輪次" : "Stop When Stable / 穩定後提前停止";
    const modeHelp = probeRoundExecutionMode === "force_all_rounds" ? "Runs every configured round even after stability is reached. / 即使已達穩定條件，仍執行全部設定輪次。" : "Stops only after two complete consecutive rounds have identical event sets. / 僅在連續兩個完整輪次事件集合相同後停止。";
    return <SectionCard title="Activity Stream Stability Setup" subtitle="Activity Stream 穩定性設定" className="mb-4"><div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
      <div><FieldLabel label="Selected User" sub="選定使用者" /><input className="field" value={userAnalysis.selectedUsersText} onChange={(event) => patchState({ selectedUsersText: event.target.value.replace(/[\n,;].*$/, "") })} /></div>
      <div className="grid grid-cols-2 gap-2"><div><FieldLabel label="Start Date" sub="開始日期" /><input data-testid="stability-start-date" className="field" type="date" value={userAnalysis.startDate} onChange={(event) => patchState({ startDate: event.target.value })} /></div><div><FieldLabel label="End Date" sub="結束日期" /><input data-testid="stability-end-date" className="field" type="date" value={userAnalysis.endDate} onChange={(event) => patchState({ endDate: event.target.value })} /></div></div>
      <div><FieldLabel label="Request Window" sub="請求視窗" /><select data-testid="stability-request-window" className="field" value={userAnalysis.activityStreamRequestWindow} onChange={(event) => patchState({ activityStreamRequestWindow: event.target.value as ActivityStreamRequestWindowType })}><option value="1_day">1 Day / 1 天</option><option value="7_days">7 Days / 7 天</option><option value="14_days">14 Days / 14 天</option><option value="calendar_month">Calendar Month / 日曆月</option><option value="custom_days">Custom / 自訂</option></select>{userAnalysis.activityStreamRequestWindow === "custom_days" ? <input data-testid="stability-custom-window-days" className="field mt-2" type="number" min={1} max={31} value={userAnalysis.activityStreamCustomWindowDays} onChange={(event) => patchState({ activityStreamCustomWindowDays: Math.max(1, Math.min(31, Math.trunc(Number(event.target.value)))) })} /> : null}</div>
      <div><FieldLabel label="Full Scan Round Count" sub="完整掃描輪數（1-32）" /><input data-testid="stability-round-count" className="field" type="number" min={1} max={32} value={userAnalysis.activityStreamFullScanRoundCount} onChange={(event) => patchState({ activityStreamFullScanRoundCount: Math.max(1, Math.min(32, Math.trunc(Number(event.target.value)))) })} /></div>
      <div><FieldLabel label="Delay Between Rounds" sub="輪次間隔" /><select data-testid="stability-round-delay" className="field" value={userAnalysis.activityStreamDelayBetweenRoundsMs} onChange={(event) => patchState({ activityStreamDelayBetweenRoundsMs: Number(event.target.value) })}>{[0,1000,2000,3000,5000].map((value) => <option key={value} value={value}>{value / 1000} sec / 秒</option>)}</select></div>
      <div><FieldLabel label="Merge Strategy" sub="合併策略" /><select data-testid="stability-merge-strategy" className="field" value={userAnalysis.activityStreamMergeStrategy} onChange={(event) => patchState({ activityStreamMergeStrategy: event.target.value as ActivityStreamMergeStrategy })}><option value="union">Union / 聯集</option><option value="last_stable">Last Complete Round / 最後完整輪次</option></select></div>
    </div><div className="mt-4"><FieldLabel label="Round Execution Mode" sub="輪次執行模式" /><div className="flex flex-wrap gap-2">{(["stop_when_stable", "force_all_rounds"] as RoundExecutionMode[]).map((mode) => <button key={mode} data-testid={`round-mode-${mode}`} type="button" className={`btn ${probeRoundExecutionMode === mode ? "btn-primary" : ""}`} onClick={() => setProbeRoundExecutionMode(mode)}>{mode === "stop_when_stable" ? "Stop When Stable / 穩定後提前停止" : "Force All Rounds / 強制執行全部輪次"}</button>)}</div><div data-testid="current-round-mode" className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-semibold leading-relaxed text-blue-950"><b>Current Mode / 目前模式：{modeText}</b><br />{modeHelp}</div></div>
    <div data-testid="stability-request-estimate" className={`mt-4 rounded-lg border p-3 text-sm font-bold ${stabilityTotalRequests > 500 ? "border-red-300 bg-red-50 text-red-900" : "border-blue-200 bg-blue-50 text-blue-900"}`}>Sequential requests / 依序請求：{userAnalysis.activityStreamFullScanRoundCount} rounds / 輪 × {stabilityWindowCount} windows / 視窗 = {stabilityTotalRequests} requests / 請求。</div>
    <div className="mt-4 flex flex-wrap gap-2"><button data-testid="run-stability-probe" className="btn btn-primary" type="button" disabled={!connectionReady || selectedUsers.length !== 1 || stabilityRunning} onClick={() => stabilityTotalRequests > 500 ? setStabilityConfirmOpen(true) : void executeStabilityProbe()}><Play size={16} />Run Stability Probe / 執行穩定性測試</button><button data-testid="cancel-stability-probe" className="btn" type="button" disabled={!stabilityRunning} onClick={() => void cancelStabilityProbe()}><Square size={16} />Cancel / 取消</button><button className="btn" type="button" disabled={stabilityRunning} onClick={() => { setStabilityRun(null); setStabilityProgress({}); }}><RotateCcw size={16} />Reset / 重設</button><button className="btn" type="button" disabled={!roundRun} onClick={() => void exportStabilityResult()}><Download size={16} />Export Result / 匯出結果</button></div></SectionCard>;
  }

  function RoundProgressV2() {
    if (!stabilityRunning && !roundRun) return null;
    const p = stabilityProgress;
    const metricHelp = "Current run, completed requests only; round delay, export, and UI render excluded. / 僅統計目前執行與已完成請求；不含輪次間隔、匯出與 UI 呈現。";
    return <SectionCard title="Progress and ETA" subtitle="進度與預估完成時間" className="mb-4"><div data-testid="stability-progress-v2" className="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-3"><MiniStat label="Current Round / Total Rounds / 目前輪次" value={`${String(p.currentRound ?? roundRun?.rounds.length ?? 0)} / ${String(p.totalRounds ?? userAnalysis.activityStreamFullScanRoundCount)}`} /><MiniStat label="Current Window / Total Windows / 目前視窗" value={`${String(p.currentWindow ?? 0)} / ${String(p.totalWindows ?? stabilityWindowCount)}`} /><MiniStat label="Completed Requests / Total Requests / 已完成請求" value={`${String(p.completedRequests ?? 0)} / ${String(p.totalRequests ?? stabilityTotalRequests)}`} /><MiniStat label="Current Date Window / 目前日期視窗" value={`${String(p.requestWindowStart ?? "-")} ~ ${String(p.requestWindowEnd ?? "-")}`} /><div title={metricHelp}><MiniStat label="Average Logical Window Fetch Duration / 平均邏輯視窗查詢時間" value={`${String(p.averageLogicalFetchDurationMs ?? p.averageApiDurationMs ?? 0)} ms`} /></div><div title={metricHelp}><MiniStat label="Average Physical HTTP Duration / 平均實體 HTTP 請求時間" value={`${String(p.averagePhysicalHttpDurationMs ?? 0)} ms`} /></div><div title={metricHelp}><MiniStat label="Average Processing Duration / 平均本機處理時間" value={`${String(p.averageProcessingDurationMs ?? 0)} ms`} /></div><MiniStat label="Elapsed Time / 已用時間" value={`${String(p.elapsedMs ?? 0)} ms`} /><MiniStat label="Estimated Remaining Time / 預估剩餘時間" value={`${String(p.estimatedRemainingMs ?? 0)} ms`} /><MiniStat label="Estimated Completion Time / 預估完成時間" value={String(p.estimatedCompletionTime ?? "-")} /><MiniStat label="Current Stability / 目前穩定度" value={String(p.currentStability ?? roundRun?.comparison.stability ?? "insufficient_rounds")} /></div></SectionCard>;
  }

  function RoundComparisonV2() {
    const rows = [...(roundRun?.rounds ?? [])].filter((round) => stabilityStatusFilter === "all" || round.status === stabilityStatusFilter).sort((a, b) => stabilitySort === "duration" ? b.durationMs - a.durationMs : a.roundNumber - b.roundNumber).map((round) => ({ ...round, status: roundStatusLabel(round.status) }));
    const labels: Record<string, string> = { round: "Round / 輪次", status: "Status / 狀態", duration: "Duration / 耗時", raw: "Raw Events / 原始事件", unique: "Unique Events / 不重複事件", primary: "Primary Jira Keys / 主要 Jira", referenced: "Referenced Jira Keys / 參照 Jira", newPrevious: "New Events vs Previous / 較前輪新增", missingPrevious: "Missing Events vs Previous / 較前輪缺少", eventFingerprint: "Event Fingerprint / 事件指紋", primaryFingerprint: "Primary Jira Fingerprint / 主要 Jira 指紋", consistency: "Consistency / 一致率", coldStart: "Cold Start / 冷啟動" };
    return <SectionCard title="Round Comparison" subtitle="輪次比較" className="mb-4"><div className="mb-3 flex flex-wrap gap-2"><select data-testid="round-status-filter" className="field max-w-48" value={stabilityStatusFilter} onChange={(event) => setStabilityStatusFilter(event.target.value)}><option value="all">All Status / 全部狀態</option><option value="completed">Completed / 已完成</option><option value="partial">Partial / 部分完成</option><option value="cancelled">Cancelled / 已取消</option></select><select data-testid="round-sort" className="field max-w-48" value={stabilitySort} onChange={(event) => setStabilitySort(event.target.value as typeof stabilitySort)}><option value="round">Sort by Round / 依輪次</option><option value="duration">Sort by Duration / 依耗時</option></select><button data-testid="round-columns" className="btn" type="button" onClick={() => setStabilityColumnsOpen((open) => !open)}>Columns / 欄位</button><button data-testid="copy-round-comparison" className="btn" type="button" onClick={() => void navigator.clipboard?.writeText(JSON.stringify(rows, null, 2))}><Copy size={16} />Copy / 複製</button></div>{stabilityColumnsOpen ? <div className="mb-3 flex flex-wrap gap-3 rounded-lg border border-line p-3">{Object.entries(labels).map(([key,label]) => <label key={key} className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={stabilityVisibleColumns.includes(key)} onChange={() => setStabilityVisibleColumns((current) => current.includes(key) ? current.filter((value) => value !== key) : [...current,key])} />{label}</label>)}</div> : null}<ResponsiveTableContainer><table data-testid="stability-round-comparison" className="table min-w-[1800px]"><thead><tr>{Object.entries(labels).filter(([key]) => stabilityVisibleColumns.includes(key)).map(([key,label]) => <th key={key}>{label}</th>)}</tr></thead><tbody>{rows.map((round) => { const values: Record<string, unknown> = { round: `${round.roundNumber}/${round.totalRounds}`, status: round.status, duration: `${round.durationMs} ms`, raw: round.rawEventCount, unique: round.uniqueEventCount, primary: round.primaryJiraKeyCount, referenced: round.referencedJiraKeyCount, newPrevious: round.newEventsComparedWithPreviousRound, missingPrevious: round.missingEventsComparedWithPreviousRound, eventFingerprint: round.roundEventSetFingerprint, primaryFingerprint: round.roundPrimaryJiraKeySetFingerprint, consistency: roundConsistency(round.roundNumber), coldStart: round.coldStartSuspected ? "Yes / 是" : "No / 否" }; return <tr key={round.roundId}>{Object.keys(labels).filter((key) => stabilityVisibleColumns.includes(key)).map((key) => <td key={key} className={key.includes("Fingerprint") ? "max-w-64 truncate" : ""} title={String(values[key] ?? "")}>{String(values[key] ?? "-")}</td>)}</tr>;})}</tbody></table></ResponsiveTableContainer><div className="mt-3 text-xs font-semibold text-muted">CSV: activity-stream-round-comparison.csv / CSV 已自動匯出。</div></SectionCard>;
  }

  function WindowDiagnosticsLegacyV2() {
    const rows = (roundRun?.windowDiagnostics ?? []).filter((row) => stabilityStatusFilter === "all" || row.status === stabilityStatusFilter);
    return <SectionCard title="Window Diagnostics" subtitle="視窗診斷" className="mb-4"><div className="mb-3 flex flex-wrap gap-2"><select data-testid="window-status-filter" className="field max-w-48" value={stabilityStatusFilter} onChange={(event) => setStabilityStatusFilter(event.target.value)}><option value="all">All Status / 全部狀態</option><option value="completed">Completed / 已完成</option><option value="failed">Failed / 失敗</option></select><button data-testid="window-columns" className="btn" type="button" onClick={() => setStabilityColumnsOpen((open) => !open)}>Columns / 欄位</button><button data-testid="copy-window-diagnostics" className="btn" type="button" onClick={() => void navigator.clipboard?.writeText(JSON.stringify(rows, null, 2))}><Copy size={16} />Copy / 複製</button></div><ResponsiveTableContainer><table data-testid="stability-window-diagnostics" className="table min-w-[1500px]"><thead><tr>{["Round / 輪次","Window / 視窗","Date Range / 日期範圍","Raw Events / 原始事件","Unique Events / 不重複事件","Primary Jira Keys / 主要 Jira","Referenced Jira Keys / 參照 Jira","New Events / 新增事件","Missing Events / 缺少事件","Duration / 耗時","Status / 狀態"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={`${row.roundId}-${row.windowId}`}><td>{row.roundNumber}</td><td>{row.windowNumber}/{row.windowCount}</td><td>{row.requestWindowStart} ~ {row.requestWindowEnd}</td><td>{row.rawEventCount}</td><td>{row.uniqueEventCount}</td><td>{row.primaryJiraKeyCount}</td><td>{row.referencedJiraKeyCount}</td><td>{row.newEventsComparedWithPreviousRound}</td><td>{row.missingEventsComparedWithPreviousRound}</td><td>{row.totalRequestDurationMs} ms</td><td><StatusBadge>{row.status}</StatusBadge></td></tr>)}</tbody></table></ResponsiveTableContainer><div className="mt-3 text-xs font-semibold text-muted">CSV: activity-stream-window-diagnostics.csv / CSV 已自動匯出。</div></SectionCard>;
  }

  function WindowDiagnosticsV2() {
    const labels: Record<string, string> = { round: "Round / 輪次", window: "Window / 視窗", date: "Date Range / 日期範圍", classification: "Classification / 結果分類", logical: "Logical Fetch / 邏輯查詢", physical: "Physical Requests / 實體請求", http: "HTTP Duration / HTTP 耗時", processing: "Processing / 本機處理", raw: "Raw Events / 原始事件", unique: "Unique Events / 不重複事件", primary: "Primary Jira Keys / 主要 Jira", referenced: "Referenced Jira Keys / 參照 Jira", status: "Status / 狀態" };
    const rows = [...(roundRun?.windowDiagnostics ?? [])].filter((row) => stabilityStatusFilter === "all" || row.status === stabilityStatusFilter).sort((a, b) => windowSort === "duration" ? b.totalRequestDurationMs - a.totalRequestDurationMs : windowSort === "date" ? a.requestWindowStart.localeCompare(b.requestWindowStart) : a.roundNumber - b.roundNumber || a.windowNumber - b.windowNumber);
    return <SectionCard title="Window Diagnostics" subtitle="視窗診斷" className="mb-4"><div className="mb-3 flex flex-wrap gap-2"><select data-testid="window-status-filter" className="field max-w-48" value={stabilityStatusFilter} onChange={(event) => setStabilityStatusFilter(event.target.value)}><option value="all">All Status / 全部狀態</option><option value="completed">Completed / 已完成</option><option value="failed">Failed / 失敗</option></select><select data-testid="window-sort" className="field max-w-56" value={windowSort} onChange={(event) => setWindowSort(event.target.value as typeof windowSort)}><option value="round_window">Round and Window / 輪次與視窗</option><option value="date">Date / 日期</option><option value="duration">Duration / 耗時</option></select><button data-testid="window-columns" className="btn" type="button" onClick={() => setWindowColumnsOpen((open) => !open)}>Columns / 欄位</button><button data-testid="copy-window-diagnostics" className="btn" type="button" onClick={() => void navigator.clipboard?.writeText(JSON.stringify(rows, null, 2))}><Copy size={16} />Copy / 複製</button></div>{windowColumnsOpen ? <div data-testid="window-column-settings" className="mb-3 flex flex-wrap gap-3 rounded-lg border border-line p-3">{Object.entries(labels).map(([key,label]) => <label key={key} className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={windowVisibleColumns.includes(key)} onChange={() => setWindowVisibleColumns((current) => current.includes(key) ? current.filter((value) => value !== key) : [...current,key])} />{label}</label>)}</div> : null}<ResponsiveTableContainer><table data-testid="stability-window-diagnostics" className="table min-w-[1800px]"><thead><tr>{Object.entries(labels).filter(([key]) => windowVisibleColumns.includes(key)).map(([key,label]) => <th key={key}>{label}</th>)}</tr></thead><tbody>{rows.map((row) => { const values: Record<string, unknown> = { round: row.roundNumber, window: `${row.windowNumber}/${row.windowCount}`, date: `${row.requestWindowStart} ~ ${row.requestWindowEnd}`, classification: row.classification, logical: `${row.logicalFetchDurationMs} ms`, physical: row.physicalHttpRequestCount, http: `${row.physicalRequests.reduce((sum, request) => sum + request.httpDurationMs, 0)} ms`, processing: row.processingTiming.totalProcessingMs < 1 ? "<1 ms" : `${row.processingTiming.totalProcessingMs} ms`, raw: row.rawEventCount, unique: row.uniqueEventCount, primary: row.primaryJiraKeyCount, referenced: row.referencedJiraKeyCount, status: roundStatusLabel(row.status) }; return <tr key={`${row.roundId}-${row.windowId}`}>{Object.keys(labels).filter((key) => windowVisibleColumns.includes(key)).map((key) => <td key={key}>{String(values[key] ?? "-")}</td>)}</tr>; })}</tbody></table></ResponsiveTableContainer><div className="mt-3 text-xs font-semibold text-muted">CSV: activity-stream-window-diagnostics.csv / CSV 已自動匯出。</div></SectionCard>;
  }

  function RoundSummaryV2() {
    if (!roundRun) return <SectionCard title="No Result" subtitle="尚無結果" className="mb-4"><div className="text-sm font-semibold text-muted">Run the Stability Probe to create round diagnostics. / 請執行穩定性測試以產生輪次診斷。</div></SectionCard>;
    const c = { ...roundRun.comparison, stability: stabilityLabel(roundRun.comparison.stability) }; const r = roundRun.recommendation;
    return <SectionCard title="Round Stability Recommendation" subtitle="輪次穩定性建議" className="mb-4"><div className="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(170px,1fr))] gap-3"><MiniStat label="Stability / 穩定度" value={c.stability} /><MiniStat label="Round Union Events / 輪次聯集事件" value={c.roundUnionEventCount} /><MiniStat label="Round Intersection Events / 輪次交集事件" value={c.roundIntersectionEventCount} /><MiniStat label="Variable Events / 非穩定事件" value={c.variableEventCount} /><MiniStat label="Consistency Rate / 一致率" value={c.consistencyRate === null ? "N/A" : `${(c.consistencyRate * 100).toFixed(1)}%`} /><MiniStat label="Recommended Round Count / 建議輪數" value={r.recommendedRoundCount} /><MiniStat label="First Stable Round / 首次穩定輪次" value={r.firstStableRound ?? "N/A"} /><MiniStat label="Cold Start Rounds / 冷啟動輪次" value={r.coldStartAffectedRoundCount} /></div><div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-semibold leading-relaxed text-blue-950">{r.recommendationReason}</div><button className="btn btn-primary mt-4" type="button" onClick={applyStabilityRecommendation}>Apply Recommendation / 套用建議</button></SectionCard>;
  }

  function BenchmarkView() {
    const config = userAnalysis.activityStreamBenchmarkConfig;
    const run = userAnalysis.activityStreamBenchmarkRun;
    const summary = run?.summary;
    const updateConfig = (patch: Partial<typeof config>) => patchState({ activityStreamBenchmarkConfig: { ...config, ...patch, variant: "escaped_username", concurrency: 1 } });
    const duration = (value: number | null | undefined) => value === null || value === undefined ? "N/A" : value < 1 ? "<1 ms" : `${value} ms`;
    return <>
      <SectionCard title="Activity Stream Benchmark" subtitle="Activity Stream 效能基準" className="mb-4">
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-semibold leading-relaxed text-blue-950">Benchmark samples are isolated from Timeline, Candidate Set, and Full Fetch. Read-only GET requests only. / 效能基準樣本與 Timeline、Candidate Set、Full Fetch 完全隔離，且只執行唯讀 GET。</div>
        <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div><FieldLabel label="User" sub="使用者" /><input data-testid="benchmark-user" className="field" value={config.selectedUser} onChange={(event) => updateConfig({ selectedUser: event.target.value })} /></div>
          <div><FieldLabel label="Start Date" sub="開始日期" /><input data-testid="benchmark-start-date" className="field" type="date" value={config.startDate} onChange={(event) => updateConfig({ startDate: event.target.value })} /></div>
          <div><FieldLabel label="End Date" sub="結束日期" /><input data-testid="benchmark-end-date" className="field" type="date" value={config.endDate} onChange={(event) => updateConfig({ endDate: event.target.value })} /></div>
          <div><FieldLabel label="Runs (1-20)" sub="執行次數（1-20）" /><input data-testid="benchmark-runs" className="field" type="number" min={1} max={20} value={config.runs} onChange={(event) => updateConfig({ runs: Math.max(1, Math.min(20, Math.trunc(Number(event.target.value)))) })} /></div>
          <div><FieldLabel label="Variant" sub="查詢變體" /><input className="field" readOnly value="escaped_username" /></div>
          <div><FieldLabel label="Concurrency" sub="並行數" /><input className="field" readOnly value="1" /></div>
        </div>
        <div className="mt-4 flex flex-wrap gap-2"><button data-testid="run-benchmark" className="btn btn-primary" type="button" disabled={!connectionReady || userAnalysis.activityStreamBenchmarkRunning} onClick={() => void runBenchmark()}><Play size={16} />Run Benchmark / 執行效能基準</button><button data-testid="cancel-benchmark" className="btn" type="button" disabled={!userAnalysis.activityStreamBenchmarkRunning} onClick={() => void cancelBenchmark()}><Square size={16} />Cancel Benchmark / 取消效能基準</button></div>
      </SectionCard>
      {userAnalysis.activityStreamBenchmarkRunning || run ? <SectionCard title="Benchmark Progress and Summary" subtitle="效能基準進度與摘要" className="mb-4"><div className="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(170px,1fr))] gap-3"><MiniStat label="Completed Runs / 已完成執行" value={`${String(userAnalysis.activityStreamBenchmarkProgress.completedRuns ?? run?.samples.length ?? 0)} / ${config.runs}`} /><MiniStat label="Status / 狀態" value={String(userAnalysis.activityStreamBenchmarkProgress.status ?? run?.status ?? "running")} /><MiniStat label="Run Count / 執行數" value={summary?.runCount ?? 0} /><MiniStat label="Success Count / 成功數" value={summary?.successCount ?? 0} /><MiniStat label="Timeout Count / 逾時數" value={summary?.timeoutCount ?? 0} /><MiniStat label="HTTP Error Count / HTTP 錯誤數" value={summary?.httpErrorCount ?? 0} /><MiniStat label="Logical Min / Median / Avg / P90 / Max / 邏輯查詢" value={summary ? [summary.logicalWindow.min, summary.logicalWindow.median, summary.logicalWindow.average, summary.logicalWindow.p90, summary.logicalWindow.max].map(duration).join(" / ") : "N/A"} /><MiniStat label="Physical HTTP Min / Median / Avg / P90 / Max / 實體 HTTP" value={summary ? [summary.physicalHttp.min, summary.physicalHttp.median, summary.physicalHttp.average, summary.physicalHttp.p90, summary.physicalHttp.max].map(duration).join(" / ") : "N/A"} /><MiniStat label="Processing Min / Median / Avg / P90 / Max / 本機處理" value={summary ? [summary.processing.min, summary.processing.median, summary.processing.average, summary.processing.p90, summary.processing.max].map(duration).join(" / ") : "N/A"} /><MiniStat label="Average Response Bytes / 平均回應位元組" value={summary?.averageResponseBytes ?? 0} /><MiniStat label="Average Event Count / 平均事件數" value={summary?.averageEventCount ?? 0} /><MiniStat label="Pagination / Retry / 分頁與重試" value={`${summary?.paginationCount ?? 0} / ${summary?.retryCount ?? 0}`} /></div></SectionCard> : null}
      <SectionCard title="Benchmark Samples" subtitle="效能基準明細" className="mb-4"><ResponsiveTableContainer><table data-testid="benchmark-samples" className="table min-w-[1500px]"><thead><tr>{["Run / 次數","Start / 開始","End / 結束","Logical / 邏輯","Physical Requests / 實體請求","HTTP / HTTP 耗時","Processing / 本機處理","Status / 狀態","Bytes / 位元組","Atom Entries / Atom 項目","Parsed Events / 解析事件","Timeout / 逾時","Classification / 分類"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{run?.samples.map((sample) => <tr key={sample.runNumber}><td>{sample.runNumber}</td><td>{sample.startTime}</td><td>{sample.endTime}</td><td>{duration(sample.logicalDurationMs)}</td><td>{sample.physicalHttpRequestCount}</td><td>{duration(sample.httpDurationMs)}</td><td>{duration(sample.processingDurationMs)}</td><td>{sample.httpStatus}</td><td>{sample.responseBytes}</td><td>{sample.atomEntryCount}</td><td>{sample.parsedEventCount}</td><td>{sample.timeout ? "Yes / 是" : "No / 否"}</td><td>{sample.resultClassification}</td></tr>)}{!run ? <tr><td colSpan={13} className="text-center text-muted">No benchmark result yet. / 尚無效能基準結果。</td></tr> : null}</tbody></table></ResponsiveTableContainer><div className="mt-3 text-xs font-semibold text-muted">Exports / 匯出：activity-stream-benchmark.json, activity-stream-benchmark.csv, activity-stream-benchmark-summary.json</div></SectionCard>
    </>;
  }

  if (probeMode !== "precision") return <><PageHeader title="User Activity Precision Probe" subtitle="使用者活動精準查詢測試" />{stabilityTabs}{stabilityConfirmOpen ? <MockModal title="Confirm Large Sequential Probe / 確認大型依序測試" onClose={() => setStabilityConfirmOpen(false)} footer={<><button className="btn" type="button" onClick={() => setStabilityConfirmOpen(false)}>Cancel / 取消</button><button data-testid="confirm-stability-run" className="btn btn-primary" type="button" onClick={() => { if (stabilityConfirmText === "CONFIRM") { setStabilityConfirmOpen(false); void executeStabilityProbe(true); } }}>Confirm and Run / 確認並執行</button></>}><p className="mb-3 text-sm font-semibold">This probe will execute {stabilityTotalRequests} sequential requests. Type CONFIRM to continue.<br />此測試將依序執行 {stabilityTotalRequests} 個請求，請輸入 CONFIRM 繼續。</p><input data-testid="stability-confirm-input" className="field" value={stabilityConfirmText} onChange={(event) => setStabilityConfirmText(event.target.value)} /></MockModal> : null}{probeMode === "stability" ? <><RoundSetupV2 /><RoundProgressV2 /><RoundSummaryV2 /></> : probeMode === "roundComparison" ? <><RoundComparisonV2 /><RoundSummaryV2 /></> : probeMode === "windowDiagnostics" ? <WindowDiagnosticsV2 /> : probeMode === "benchmark" ? <BenchmarkView /> : <SectionCard title="Raw Results / Diagnostics" subtitle="原始結果與診斷" className="mb-4"><div data-testid="stability-raw-results" className="space-y-3">{roundRun?.rounds.map((round) => <details key={round.roundId} className="rounded-lg border border-line p-3"><summary className="cursor-pointer font-bold">Round {round.roundNumber} / 輪次 {round.roundNumber} · {round.windows.length} windows / 視窗</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-all rounded bg-slate-950 p-3 text-xs text-slate-100">{JSON.stringify(round, null, 2)}</pre></details>)}{!roundRun ? <div className="text-sm font-semibold text-muted">No Stability Probe result yet. / 尚無穩定性測試結果。</div> : null}</div></SectionCard>}</>;
  ...
  return <>{stabilityTabs}
    <PageHeader title="活動串流測試" subtitle="Activity Stream Probe" />
