...
export const debugLogs: Record<string, string[]> = {
  ...
  analysis: ["User Analysis Stage 1 initialized", "Data Source Mode: Live Jira API", "Connection Source: Current .env Jira Connection", "API Version: Jira Server/Data Center v2", "Auth Type: Bearer Token / PAT", "Authorization: [masked]", "JQL Strategy: base search without updatedBy", "updatedBy status: disabled", "Candidate Discovery will use read-only Jira JQL search", "No database write performed"],
  precision: ["User Activity Precision Probe ready", "Activity Stream is the preferred validation source", "Authorization: [masked]", "No database write performed"],
