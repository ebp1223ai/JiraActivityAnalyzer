...
export const BRIDGE_IDENTITY_V0336 = "0.3.36-bridge-v16" as const;
export const BRIDGE_ARTIFACT_V0336 = "analysis-bridge-v0336.cjs" as const;
export const BRIDGE_MANIFEST_V0336 = "analysis-bridge-manifest-v0336.json" as const;
...
export type PackagedBridgeManifestV0336 = {
  schemaVersion: typeof PACKAGED_BRIDGE_SCHEMA;
  bridgeIdentity: typeof BRIDGE_IDENTITY_V0336;
  artifactFileName: typeof BRIDGE_ARTIFACT_V0336;
  artifactBytes: number;
  artifactSha256: string;
  runtimeContractRegistry: "jaa-runtime-contract-registry-v2";
...
export type BridgePreflightReceiptV0336 = {
  ...
  expectedBridgeIdentity: typeof BRIDGE_IDENTITY_V0336;
...
export type BridgeRuntimeContextV0336 = {
  appIsPackaged: boolean;
  processResourcesPath: string;
  developmentResourcesPath?: string;
  distributionKind?: BridgePreflightReceiptV0336["distributionKind"];
  phase: BridgePreflightReceiptV0336["phase"];
  attemptId?: string | null;
  expectedBytes?: number;
  expectedSha256?: string;
};
...
export function bridgeDistributionKindV0336(appIsPackaged: boolean): BridgePreflightReceiptV0336["distributionKind"] {
...
export function resolveAnalysisBridgeArtifactV0336(context: BridgeRuntimeContextV0336) {
  ...
  const artifactPath = path.join(resourceDirectory, BRIDGE_ARTIFACT_V0336);
  const manifestPath = path.join(resourceDirectory, BRIDGE_MANIFEST_V0336);
  ...
  const receipt: BridgePreflightReceiptV0336 = {
    ...
    distributionKind: context.distributionKind ?? bridgeDistributionKindV0336(context.appIsPackaged),
    processResourcesPath: context.processResourcesPath,
    expectedRelativePath: `${BRIDGE_RESOURCE_DIRECTORY}/${BRIDGE_ARTIFACT_V0336}`,
    resolvedAbsolutePath: null, manifestAbsolutePath: manifestPath,
    expectedBridgeIdentity: BRIDGE_IDENTITY_V0336, observedBridgeIdentity: null,
  ...
  try {
    ...
    let manifest: PackagedBridgeManifestV0336;
    try { manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PackagedBridgeManifestV0336; }
    ...
    if (manifest.schemaVersion !== PACKAGED_BRIDGE_SCHEMA || manifest.bridgeIdentity !== BRIDGE_IDENTITY_V0336 || manifest.artifactFileName !== BRIDGE_ARTIFACT_V0336) throw failure("AI_BRIDGE_VERSION_MISMATCH", "Bridge manifest identity or artifact filename does not match v0.3.36.");
    if (manifest.runtimeContractRegistry !== "jaa-runtime-contract-registry-v2" || manifest.transport !== "bridge-resumable-v4" || manifest.decisionContract !== "jaa-ai-analysis-decisions-v5" || manifest.segmentSchema !== "jaa-model-input-segment-v3" || manifest.segmentPlanner !== "jaa-protected-token-safe-segment-planner-v1" || manifest.boundaryReceipt !== "jaa-segment-boundary-safety-receipt-v1" || manifest.localOnly !== true || manifest.externalFallback !== false) throw failure("AI_BRIDGE_CONTRACT_MISMATCH", "Bridge manifest runtime contract does not match v0.3.36.");
