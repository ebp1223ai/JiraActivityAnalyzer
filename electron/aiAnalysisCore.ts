...
import { loadV0322RulesSnapshot as loadBoundV0322RulesSnapshot } from "./aiAnalysisRulesV0322.js";
...
export function loadV0322RulesSnapshot(directory: string, allowedRoots: string[]): AiRulesSnapshot {
  return loadBoundV0322RulesSnapshot(directory, allowedRoots);
}
...
export function classifyOffline(dataset: AiPendingDataset, selectedIds: string[], rules: AiRulesSnapshot) {
  ...
  return dataset.diffs.filter((diff) => selected.has(diff.sourceDiffId)).map((diff) => {
    ...
    return {
      ...
      sourceDiffId: diff.sourceDiffId, sourceContentHash: diff.sourceContentHash, evidenceRefs: [diff.evidenceId], recordIndex: dataset.diffs.findIndex((item) => item.sourceDiffId === diff.sourceDiffId), sourceRecordStableId: diff.sourceDiffId, activityEventId: diff.activityEventId, evidenceId: diff.evidenceId, classificationStatus: candidates.length ? "MATCHED" as const : "UNKNOWN" as const, reviewStatus: "PENDING_REVIEW" as const, reviewAttention: candidates.length ? "STANDARD_REVIEW" as const : "NEEDS_REVIEW" as const, dispositionReason: candidates.length ? "Deterministic rule evidence matched exact Catalog IDs." : "No sufficient deterministic evidence matched an exact Catalog ID.", exclusionReason: null, unknownReason: candidates.length ? null : "Insufficient deterministic evidence.", matchedRuleIds: candidates.flatMap((candidate) => candidate.matchedRuleIds), negativeChecks: [{ ruleId: "OFFLINE_EVIDENCE_SUFFICIENCY", passed: candidates.length > 0, detail: candidates.length ? "Sufficient deterministic evidence was found." : "Evidence was insufficient for a deterministic match.", evidenceRefs: [diff.evidenceId] }], candidates,
