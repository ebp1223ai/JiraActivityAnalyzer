...
export function singleRunOutputSchema() {
  return createSingleRunOutputSchema();
}
...
function parseNegative(value: unknown, expectedEvidence: string): AiNegativeCheck[] {
  if (!Array.isArray(value)) throw new AiAnalysisError("AI_RESPONSE_SEMANTIC_VALIDATION_FAILED", "negativeChecks must be an array.");
...
export function parseSingleRunResponse(responseText: string, compact: CompactPayload, rules: AiRulesSnapshot, analyzerVersion: string, requestTraceId: string | null, usage: AiDiffAnalysisResult["usage"]) {
  let parsed: unknown;
  try { parsed = JSON.parse(responseText.replace(/^\x60\x60\x60json\s*|\s*\x60\x60\x60$/g, "")); }
  catch { throw new AiAnalysisError("AI_PROVIDER_RESPONSE_INVALID_JSON", "Single-run provider response is not valid JSON. No repair Turn was issued."); }
  const schemaValidation = validateValueAgainstOutputSchema(parsed, createSingleRunOutputSchema());
  if (!schemaValidation.isValid) throw new AiAnalysisError("AI_PROVIDER_RESPONSE_SCHEMA_MISMATCH", `Provider response does not match the canonical schema: ${schemaValidation.findings[0]?.jsonPointer ?? "/"} ${schemaValidation.findings[0]?.message ?? "invalid"}`);
  const semanticValidation = validateCanonicalResponseSemantics(parsed);
  if (!semanticValidation.valid) { const finding = semanticValidation.findings[0]; throw new AiAnalysisError("AI_RESPONSE_SEMANTIC_VALIDATION_FAILED", `${finding.path}: ${finding.message}`); }
  const root = object(parsed);
  if (root.schemaVersion !== AI_ANALYSIS_OUTPUT_SCHEMA_VERSION || !Array.isArray(root.records)) throw new AiAnalysisError("AI_PROVIDER_RESPONSE_SCHEMA_MISMATCH", "Single-run response schema version or records array is invalid.");
  if (root.records.length !== compact.records.length) throw new AiAnalysisError("AI_RESULT_IDENTITY_VALIDATION_FAILED", `Record conservation failed: expected ${compact.records.length}, received ${root.records.length}.`);
  ...
  return root.records.map((raw, outputIndex): AiDiffAnalysisResult => {
    ...
    if (recordIndex === null || !Number.isInteger(recordIndex) || seen.has(recordIndex) || recordIndex !== expected.recordIndex) throw new AiAnalysisError("AI_RESULT_IDENTITY_VALIDATION_FAILED", "recordIndex order/uniqueness conservation failed.");
    seen.add(recordIndex);
    for (const key of ["sourceRecordStableId", "activityEventId", "evidenceId", "sourceContentHash"] as const) if (text(item[key]) !== expected[key]) throw new AiAnalysisError("AI_RESULT_IDENTITY_VALIDATION_FAILED", `${key} conservation failed at record ${recordIndex}.`);
    const classification = text(item.classificationStatus);
    if (!(["MATCHED", "EXCLUDED", "UNKNOWN", "CATALOG_DETAIL_MISSING", "NEEDS_REVIEW"] as string[]).includes(classification)) throw new AiAnalysisError("AI_RESPONSE_SEMANTIC_VALIDATION_FAILED", `Invalid classificationStatus at record ${recordIndex}.`);
    ...
    if (["EXCLUDED", "UNKNOWN"].includes(classification) && item.analyses.length !== 0) throw new AiAnalysisError("AI_RESPONSE_SEMANTIC_VALIDATION_FAILED", `${classification} record ${recordIndex} must not contain Skill candidates.`);
    const candidates = item.analyses.map((rawCandidate): AiAnalysisCandidate => {
      ...
      if (!skill) throw new AiAnalysisError("AI_RESULT_CATALOG_VALIDATION_FAILED", `Skill ID ${skillId || "<empty>"} is not an exact Catalog ID.`);
      ...
      const componentKeys = Array.isArray(candidate.scoreComponents) ? candidate.scoreComponents.map((component) => text(object(component).componentKey)) : [];
      if (componentKeys.some((key) => !key) || new Set(componentKeys).size !== componentKeys.length) throw new AiAnalysisError("AI_PROVIDER_RESPONSE_SCHEMA_MISMATCH", `scoreComponents.componentKey must be present and unique at record ${recordIndex}.`);
      ...
      return {
        ...
        scoreComponents: Array.isArray(candidate.scoreComponents) ? candidate.scoreComponents.map((rawComponent) => {
          const component = object(rawComponent);
          const componentKey = text(component.componentKey);
          const componentScore = number(component.score);
          if (!componentKey || componentScore === null || (component.explanation !== null && typeof component.explanation !== "string")) throw new AiAnalysisError("AI_PROVIDER_RESPONSE_SCHEMA_MISMATCH", `Invalid score component at record ${recordIndex}.`);
          return { componentKey, score: componentScore, explanation: component.explanation === null ? null : text(component.explanation) };
        }) : (() => { throw new AiAnalysisError("AI_PROVIDER_RESPONSE_SCHEMA_MISMATCH", `scoreComponents must be an array at record ${recordIndex}.`); })(),
        ...
        rejectedNearSkills, evidenceQuote: text(candidate.evidenceQuote), matchedRuleIds: strings(item.matchedRuleIds), reason: text(candidate.confidenceReason), status: "PENDING_REVIEW", candidateStatus: text(candidate.candidateStatus) as AiAnalysisCandidate["candidateStatus"], statusReason: text(candidate.statusReason), catalogDetailAvailable: candidate.catalogDetailAvailable === true
      };
    });
    ...
    return {
      ...
      classificationStatus: classification as AiDiffAnalysisResult["classificationStatus"], reviewStatus: "PENDING_REVIEW", reviewAttention: item.reviewAttention as "STANDARD_REVIEW" | "NEEDS_REVIEW",
      dispositionReason, exclusionReason: item.exclusionReason === null ? null : text(item.exclusionReason) || null, unknownReason: item.unknownReason === null ? null : text(item.unknownReason) || null, reviewReason: item.reviewReason === null ? null : text(item.reviewReason) || null,
...
export function stageVisibleProviderResponse(folder: string, responseText: string) {
  ...
  const rawFilePath = path.join(folder, "provider-response.raw.json");
  fs.writeFileSync(rawFilePath, responseText, { encoding: "utf8", flag: "wx", mode: 0o600 });
  if (sha256(fs.readFileSync(rawFilePath)) !== rawSha256) throw new AiAnalysisError("AI_PROVIDER_RESPONSE_EVIDENCE_MISMATCH", "Provider raw response read-back hash mismatch.");
  ...
  return { filePath, rawFilePath, rawSizeBytes: Buffer.byteLength(responseText), rawSha256, gzipSizeBytes: reopened.length, gzipSha256: sha256(reopened) };
}
...
export function goldenHtmlForRun(run: AiAnalysisRun, dataset?: AiPendingDataset | null) {
  ...
  const rows = run.results.map((result) => {
    ...
    return {
      ...
      skills: result.candidates.map((candidate) => ({ id: candidate.skillId, name: candidate.skillName, group: candidate.group, status: candidate.candidateStatus ?? candidate.status, statusReason: candidate.statusReason ?? candidate.reason, score: candidate.score, confidence: candidate.confidenceLabel ?? candidate.confidence, reason: candidate.confidenceReason ?? candidate.reason, quote: candidate.evidenceQuote ?? "Unavailable", positive: candidate.positiveSignals ?? candidate.positiveEvidenceRefs, negative: candidate.negativeChecks ?? [], near: candidate.rejectedNearSkills ?? [], components: candidate.scoreComponents ?? [] })),
