...
export const RUNTIME_CONTRACT_REGISTRY_VERSION_V0336 = "jaa-runtime-contract-registry-v2" as const;
const registryValue = {
  schemaVersion: RUNTIME_CONTRACT_REGISTRY_VERSION_V0336,
  applicationVersion: "0.3.36", promptIdentity: "JAA-CHATGPT-ZH-TW-0.3.36", promptTemplateVersion: "0.3.36-zh-TW-v17",
  pipelineIdentity: "JAA-ANALYSIS-PIPELINE-0.3.36", bridgeIdentity: "0.3.36-bridge-v16", bridgeSchemaVersion: "jaa-analysis-bridge-v11",
  ...
  manifestReceiptVersion: "jaa-runtime-manifest-receipt-v1", rules: Object.freeze({ manifestVersion: "0.8.5", commonRulesVersion: "1.6.5", catalogVersion: "0.3.1", htmlTemplateVersion: "1.5.1" })
} as const;
...
export const RUNTIME_CONTRACT_V0336 = deepFreeze({ ...registryValue, registryHash: crypto.createHash("sha256").update(JSON.stringify(registryValue)).digest("hex") });
export type RuntimeContractV0336 = typeof RUNTIME_CONTRACT_V0336;
export function validateDispatchIdentityV0336(input: { applicationVersion: string; promptIdentity: string; promptTemplateVersion: string; decisionContract: string; qualityContract: string; bridgeIdentity: string; providerTransport: string; outputSchemaSha256: string; expectedOutputSchemaSha256: string; rulesSnapshotId: string; requestRulesSnapshotId: string }) {
  const expected = RUNTIME_CONTRACT_V0336;
