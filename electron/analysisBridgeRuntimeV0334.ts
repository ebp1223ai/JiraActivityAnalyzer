...
import { buildIdentityReceiptV0334, createApprovedArtifactIdentityV0334, persistAiSubmittedArtifactV0334, type ApprovedArtifactIdentityV0334, type ArtifactSubmissionV0334 } from "./aiArtifactIdentityV0334.js";
...
import { RUNTIME_CONTRACT_V0334 } from "./runtimeContractRegistryV0334.js";
import { createModelInputManifestV3, createRuntimeManifestReceiptV0334, serializeAndValidateManifestResponseV0334, writeRuntimeManifestReceiptV0334, type RuntimeManifestReceiptV0334 } from "./runtimeManifestV0334.js";
...
import { ProviderLifecycleV0334 } from "./providerLifecycleV0334.js";
...
export class AnalysisBridgeV0334 {
  readonly version = "0.3.34-bridge-v14";
  ...
  private approvedIdentity: ApprovedArtifactIdentityV0334 | null = null;
  ...
  private providerDispatchGateReceipt: RuntimeManifestReceiptV0334 | null = null;
  ...
  private readonly providerLifecycle: ProviderLifecycleV0334;
  ...
  constructor(private readonly config: Config) {
    ...
    this.providerLifecycle = new ProviderLifecycleV0334(config.runDirectory);
  ...
  preflight() {
    ...
    const gateManifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0334, sourceManifest, { deliveryHandle: `dispatch_gate_${"0".repeat(32)}`, artifactSubmissionToken: null });
    const receipt = createRuntimeManifestReceiptV0334(gateManifest, true, RUNTIME_CONTRACT_V0334);
    this.providerDispatchGateReceipt = receipt;
    writeRuntimeManifestReceiptV0334(this.config.runDirectory, "provider-dispatch-gate-receipt.json", receipt);
    durableJson(path.join(this.config.runDirectory, "control", "runtime-contract-registry.json"), RUNTIME_CONTRACT_V0334);
    ...
    return { ...modelSafe(legacy), version: this.version, runtimeContract: RUNTIME_CONTRACT_V0334, providerDispatchGate: receipt, bridgeExecutionContext: "jaa-bridge-execution-context-v1", modelDeliveryHandle: RUNTIME_CONTRACT_V0334.modelDeliveryHandleContract, transportProtocol: RUNTIME_CONTRACT_V0334.providerTransport, decisionContract: RUNTIME_CONTRACT_V0334.decisionContract, artifactSubmissionTokenContract: RUNTIME_CONTRACT_V0334.artifactSubmissionTokenContract, modelVisibleIdentityFields: [], hostOwnedIdentity: true };
  }
  ...
  bindProviderTurn(threadId: string, turnId: string) {
    this.providerLifecycle.transition("PROVIDER_DISPATCHED", { threadBound: true, turnBound: true });
    this.executionContext.bindTurn(threadId, turnId);
    if (this.approvedIdentity) return;
    this.approvedIdentity = createApprovedArtifactIdentityV0334({
  ...
  async handle(tool: string, argsValue: unknown, context: AnalysisBridgeToolContext) {
    ...
    const providerStateBefore = this.providerLifecycle.snapshot().state;
    ...
    try {
      ...
      normalized = normalizeJaaError(error, { stage: this.stageForTool(tool), source: "AnalysisBridgeV0334", fallbackCode: "AI_INPUT_TOOL_FAILED", safeDetails: { callId: context.callId, toolName: tool, argumentNames: Object.keys(args).sort() } });
  ...
  private manifest(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const manifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0334, sourceManifest, { deliveryHandle, artifactSubmissionToken: this.config.instructionMode === "CUSTOM_DIAGNOSTIC" ? null : this.artifactToken });
    const receipt = createRuntimeManifestReceiptV0334(manifest, true, RUNTIME_CONTRACT_V0334);
    writeRuntimeManifestReceiptV0334(this.config.runDirectory, "runtime-manifest-receipt.json", receipt);
    writeRuntimeManifestReceiptV0334(this.config.runDirectory, "runtime-manifest-self-validation.json", receipt);
    writeRuntimeManifestReceiptV0334(this.config.runDirectory, "model-input-manifest-receipt.json", receipt);
  ...
  serializeToolResponse(tool: string, result: unknown) {
    if (tool === "jaa_get_input_manifest") {
      const response = serializeAndValidateManifestResponseV0334(result, RUNTIME_CONTRACT_V0334);
  ...
  private status(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    return { ...(this.legacy.handle("jaa_get_delivery_status", { runId: this.config.runId }, this.hostContext(context)) as JsonObject), providerLifecycle: this.providerLifecycle.snapshot() };
  }
  ...
  private finalize(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const receipt = { ...legacyReceipt, transportProtocol: RUNTIME_CONTRACT_V0334.providerTransport };
  ...
  private progress(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const progress = this.providerLifecycle.reportProgress({ phase: String(args.phase) as "INPUT_READY" | "ANALYSIS_STARTED" | "ANALYSIS_COMPLETED" | "ARTIFACT_SUBMISSION_STARTED", expectedCount: Number(args.expectedCount), completedCount: Number(args.completedCount), decisionPreparedCount: Number(args.decisionPreparedCount), message: String(args.message) });
    if (!progress.idempotentReplay && !progress.warning && args.phase !== "INPUT_READY") this.legacy.handle("jaa_report_analysis_progress", { runId: this.config.runId, stage: args.phase, expectedCount: args.expectedCount, completedCount: args.completedCount, decisionPreparedCount: args.decisionPreparedCount, statusDistribution: args.statusDistribution ?? null, message: args.message }, this.hostContext(context));
    return progress;
  }
  ...
  private publish(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const submission: ArtifactSubmissionV0334 = { artifactSubmissionToken: typeof args.artifactSubmissionToken === "string" ? args.artifactSubmissionToken : "", decisions: args.decisions, analysisReportMarkdown: String(args.analysisReportMarkdown ?? ""), finalSummaryZhTw: String(args.finalSummaryZhTw ?? "") };
    const persisted = persistAiSubmittedArtifactV0334(this.config.runDirectory, this.artifactAttempt, submission, identity);
    ...
    const reject = (code: string, message: string): never => {
      ...
      const receipt = { ...buildIdentityReceiptV0334(identity, identity, code), accepted: false, firstMismatchCode: code, tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
    ...
    const identityReceipt = { ...buildIdentityReceiptV0334(identity, identity, "MATCH"), tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
    ...
    if (decisions.length === identity.sourceRecordCount && decisions.length > 0 && decisions.every((decision) => decision.status === "FAILED")) {
      ...
      this.providerLifecycle.complete("FAILED", "AI_PROVIDER_RUN_LEVEL_FAILURE");
    ...
    this.providerLifecycle.transition("ARTIFACT_RECEIVED", { receivedDecisionCount: decisions.length });
    ...
    this.artifactReceipt = { ...legacyReceipt, schemaVersion: "jaa-analysis-artifact-receipt-v4", decisionContractVersion: RUNTIME_CONTRACT_V0334.decisionContract, approvedArtifactIdentity: identity, aiSubmittedArtifactSha256: persisted.artifact.sha256, identityReceiptPath: identityPath };
  ...
  private writeToolEvidence(tool: string, args: JsonObject, context: AnalysisBridgeToolContext, startedAt: number, success: boolean, error: NormalizedJaaError | null, providerStateBefore: string) {
    ...
    appendJsonl(this.toolCallEvidencePath, {
      ...
      success,
      providerStateBefore,
      providerStateAfter: this.providerLifecycle.snapshot().state,
      lifecycleReceiptId: this.providerLifecycle.snapshot().finalized?.receiptId ?? null,
  ...
  setHtmlRenderStatus(status: AnalysisLifecycleSummary["htmlRenderStatus"]) { this.htmlRenderStatus = status; if (status === "completed") this.providerLifecycle.transition("HTML_RENDERED"); }
  setSqliteStatus(status: AnalysisLifecycleSummary["sqliteStatus"], code?: string) { this.legacy.setSqliteStatus(status); this.providerLifecycle.transition("SQLITE_GATE", { status, code: code ?? null }); if (status === "commit_failed" && code) this.markDerivedError(code, "SQLite commit failed after analysis.", "RUN_COMPLETED"); }
  setPostBridgeStage(stage: "VALIDATION_COMPLETED" | "CANONICAL_ASSEMBLY_COMPLETED" | "RUN_COMPLETED", status: "completed" | "failed") { this.legacy.setPostBridgeStage(stage, status); if (stage === "VALIDATION_COMPLETED" && status === "completed") this.providerLifecycle.transition("VALIDATION_COMPLETED"); else if (stage === "CANONICAL_ASSEMBLY_COMPLETED" && status === "completed") this.providerLifecycle.transition("CANONICAL_CREATED"); else if (stage === "RUN_COMPLETED") this.providerLifecycle.complete(status === "completed" ? "SUCCEEDED" : "FAILED", status === "failed" ? "AI_RUN_FAILED" : null); }
  fail(stage: AnalysisLifecycleStage, code: string, message = code) { this.providerLifecycle.complete("FAILED", code); const normalized = normalizeJaaError(Object.assign(new Error(`${code}:${message}`), { code }), { stage, source: "AnalysisBridgeV0334", fallbackCode: code }); this.captureRootError(normalized); }
  ...
  snapshot() {
    ...
    return {
      ...
      transportProtocol: RUNTIME_CONTRACT_V0334.providerTransport,
      runtimeContract: RUNTIME_CONTRACT_V0334,
      providerDispatchGate: this.providerDispatchGateReceipt,
      decisionContract: RUNTIME_CONTRACT_V0334.decisionContract,
      bridgeExecutionContext: this.executionContext.snapshot(),
      providerLifecycle: this.providerLifecycle.snapshot(),
...
export function createAnalysisBridge(config: Config) { return new AnalysisBridgeV0334(config); }
