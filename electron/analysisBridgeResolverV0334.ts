...
export const BRIDGE_IDENTITY_V0334 = "0.3.34-bridge-v14" as const;
export const BRIDGE_ARTIFACT_V0334 = "analysis-bridge-v0334.cjs" as const;
export const BRIDGE_MANIFEST_V0334 = "analysis-bridge-manifest-v0334.json" as const;
...
export type PackagedBridgeManifestV0334 = {
  schemaVersion: typeof PACKAGED_BRIDGE_SCHEMA;
  bridgeIdentity: typeof BRIDGE_IDENTITY_V0334;
  artifactFileName: typeof BRIDGE_ARTIFACT_V0334;
...
export type BridgePreflightReceiptV0334 = {
  ...
  expectedBridgeIdentity: typeof BRIDGE_IDENTITY_V0334;
...
export type BridgeRuntimeContextV0334 = {
  appIsPackaged: boolean;
  processResourcesPath: string;
  developmentResourcesPath?: string;
  distributionKind?: BridgePreflightReceiptV0334["distributionKind"];
  phase: BridgePreflightReceiptV0334["phase"];
  attemptId?: string | null;
  expectedBytes?: number;
  expectedSha256?: string;
};
...
export function bridgeDistributionKindV0334(appIsPackaged: boolean): BridgePreflightReceiptV0334["distributionKind"] {
...
export function resolveAnalysisBridgeArtifactV0334(context: BridgeRuntimeContextV0334) {
  ...
  const artifactPath = path.join(resourceDirectory, BRIDGE_ARTIFACT_V0334);
  const manifestPath = path.join(resourceDirectory, BRIDGE_MANIFEST_V0334);
  ...
  const receipt: BridgePreflightReceiptV0334 = {
    ...
    distributionKind: context.distributionKind ?? bridgeDistributionKindV0334(context.appIsPackaged),
    processResourcesPath: context.processResourcesPath,
    expectedRelativePath: `${BRIDGE_RESOURCE_DIRECTORY}/${BRIDGE_ARTIFACT_V0334}`,
    resolvedAbsolutePath: null, manifestAbsolutePath: manifestPath,
    expectedBridgeIdentity: BRIDGE_IDENTITY_V0334, observedBridgeIdentity: null,
  ...
  try {
    ...
    let manifest: PackagedBridgeManifestV0334;
    try { manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PackagedBridgeManifestV0334; }
