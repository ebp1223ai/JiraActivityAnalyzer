...
function resultDocument(run: StagingRun) {
  return {
    schemaVersion: "full_fetch_result_index_v5", appVersion: "0.2.34", runId: run.state.fullFetchRunId, stagingId: run.state.stagingId,
...
export function exportStaging(run: StagingRun, outputDir: string, _partial = false, exportedAt = now()) {
  ...
  const manifest = () => Buffer.from(`${JSON.stringify({ schemaVersion: "source_archive_import_package_v3", sectionMetadataVersion: "coverage_v2", appVersion: "0.2.34", exportedAt, packageStatus: "complete", stagingId: run.state.stagingId, fullFetchRunId: run.state.fullFetchRunId, eligible: run.state.eligible, countReconciliation: run.state.countReconciliation, verification, canonicalStorage: "per_issue_file_backed", futureImporterPolicy: "Import eligible records only and verify every canonical file reference before insertion." }, null, 2)}\n`, "utf8");
