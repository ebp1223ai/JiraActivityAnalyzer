...
function finding(findings: DecisionFinding[], code: string, jsonPointer: string, recordIndex: number | null, expected: unknown, observed: unknown, message: string) {
  findings.push({ code, jsonPointer, recordIndex, expected, observed, message });
}
...
export function validateDecisionArray(value: unknown, expected: ExpectedDecisionContract): DecisionValidation {
  ...
  const decisions = rawDecisions.map((raw, outputIndex): AiDecision => {
    ...
    if (status === "CLASSIFIED" && (!skillIds.length || !positiveEvidence.length)) finding(findings, "AI_DECISION_SEMANTIC_VALIDATION_FAILED", pointer, recordIndex, "skillIds and positiveEvidence >= 1", { skillIds: skillIds.length, positiveEvidence: positiveEvidence.length }, "CLASSIFIED requires a valid Skill ID and positive evidence.");
