...
type Segment = { index: number; startByte: number; endByteExclusive: number; bytes: Buffer; sha256: string; receiptToken: string; attempts: number; acknowledged: boolean };
type DeliveryFile = { fileId: string; role: string; fileName: string; bytes: Buffer; sha256: string; segments: Segment[] };

const INPUT_ROLES = ["PENDING_ANALYSIS_JSON", "COMMON_RULES", "SKILL_CATALOG", "RULE_SET_MANIFEST_OR_SCORING_RULES"];
export const MODEL_DELIVERY_PROTOCOL = "bridge-resumable-v2" as const;
...
function splitUtf8(bytes: Buffer, nonce: string, fileId: string) {
  const text = strictUtf8(bytes, fileId); const segments: Segment[] = []; let current = ""; let currentBytes = 0; let startByte = 0;
  const flush = () => { if (!currentBytes) return; const part = Buffer.from(current, "utf8"); const index = segments.length; segments.push({ index, startByte, endByteExclusive: startByte + part.length, bytes: part, sha256: sha256(part), receiptToken: sha256(`${nonce}:${fileId}:${index}:${startByte}:${sha256(part)}`), attempts: 0, acknowledged: false }); startByte += part.length; current = ""; currentBytes = 0; };
  for (const point of text) { const size = Buffer.byteLength(point); if (currentBytes && currentBytes + size > MODEL_VISIBLE_SEGMENT_BYTES) flush(); current += point; currentBytes += size; }
...
export class AnalysisBridgeV0319 {
  ...
  constructor(private readonly config: BridgeConfig) {
    ...
    this.files = documents.map((document, index) => {
      ...
      return { fileId, role: document.role, fileName: document.originalFileName, bytes, sha256: sha256(bytes), segments: splitUtf8(bytes, config.sessionNonce, fileId) };
    });
  ...
  preflight() { const legacy = this.legacy.preflight(); return { ...legacy, version: this.version, transportProtocol: MODEL_DELIVERY_PROTOCOL, modelVisibleSegmentBytes: MODEL_VISIBLE_SEGMENT_BYTES, sourceInputValidated: true }; }
  ...
  private read(args: JsonObject, context: AnalysisBridgeToolContext) { const { file, segment } = this.find(args); const firstMissing = file.segments.find((item) => !item.acknowledged); if (firstMissing && firstMissing.index !== segment.index) fail("AI_MODEL_INPUT_SEGMENT_INVALID", "Segments must be delivered in byte order; resume from the first missing segment."); if (Number(args.cursor) !== segment.startByte) fail("AI_MODEL_INPUT_CURSOR_MISMATCH", "Segment cursor does not match its byte offset."); if (segment.acknowledged) fail("AI_MODEL_INPUT_DUPLICATE_SEGMENT", "Acknowledged segments cannot be resent."); segment.attempts += 1; if (segment.attempts > MAX_SEGMENT_ATTEMPTS) { this.deliveryFailed("AI_MODEL_INPUT_RETRY_EXHAUSTED", file.fileId, segment.index); fail("AI_MODEL_INPUT_RETRY_EXHAUSTED", "Segment retry limit exhausted."); }
    ...
    const result = { schemaVersion: "jaa-model-input-segment-v2", runId: this.config.runId, threadId: context.threadId, turnId: context.turnId, fileId: file.fileId, role: file.role, segmentIndex: segment.index, segmentCount: file.segments.length, startByte: segment.startByte, endByteExclusive: segment.startByte + delivered.length, expectedBytes: segment.bytes.length, deliveredBytes: delivered.length, segmentSha256: sha256(delivered), expectedSegmentSha256: segment.sha256, receiptToken: segment.receiptToken, eof: segment.index === file.segments.length - 1 && delivered.length === segment.bytes.length, nextCursor: segment.startByte + delivered.length, content: strictUtf8(delivered, `${file.fileId}:${segment.index}`) };
