...
import { RunManifestWriterV0329 } from "./runManifestWriterV0329.js";
import { revalidateRunForQuarantineV0329 } from "./aiAnalysisQuarantineV0329.js";
...
export class AiAnalysisRunArchive {
  static reopen(runId: string, directory: string) {
    ...
    archive.conversationWriter = new BufferedDurableWriter(conversationPath); archive.providerWriter = new BufferedDurableWriter(providerPath); archive.manifestWriter = new RunManifestWriterV0329(path.join(archive.directory, "run-manifest.json")); archive.closed = false; activeArchives.add(archive);
    return archive;
  }
  readonly directory: string; readonly logsDirectory: string; private sequence = 0; private streamSequence = 0; private previousHash: string | null = null; private conversationWriter: BufferedDurableWriter; private providerWriter: BufferedDurableWriter; private manifestWriter: RunManifestWriterV0329; private closed = false;
  constructor(readonly runId: string, startedAt = new Date()) {
    ...
    this.conversationWriter = new BufferedDurableWriter(conversationPath); this.providerWriter = new BufferedDurableWriter(providerPath); this.manifestWriter = new RunManifestWriterV0329(path.join(this.directory, "run-manifest.json")); activeArchives.add(this);
  }
  ...
  writeManifest(run: AiAnalysisRun, extra: Record<string, unknown> = {}) {
    ...
    this.manifestWriter.write({ archiveVersion: RUN_ARCHIVE_VERSION, run, conversation: { eventCount: this.sequence, terminalHash: this.previousHash, ...metric("conversation.jsonl") }, providerStream: { eventCount: this.streamSequence, ...metric("provider-stream.jsonl") }, flush: this.flushStatus(), ...extra }, run.status);
