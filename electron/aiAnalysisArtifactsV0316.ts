...
import { AI_DECISION_CONTRACT_VERSION, AI_DECISION_STATUSES, validateDecisionArray, type DecisionValidation, type ValidatedDecisionDocument } from "./aiAnalysisDecisionContractV0320.js";
...
export type { AiDecision, DecisionFinding, DecisionStatus, DecisionValidation } from "./aiAnalysisDecisionContractV0320.js";
...
export function parseAndValidateDecisions(raw: string, expected: { runId: string; sourceSha256: string; rulesSnapshotId: string; recordCount: number; catalogSkillIds: string[] }): { document: AiDecisionDocument; validation: DecisionValidation } {
  ...
  if (legacyV1) {
    ...
    for (const [field, expectedValue, code] of identityChecks) if (legacyRoot?.[field] !== expectedValue) validation.findings.push({ code, jsonPointer: "/" + field, recordIndex: null, expected: expectedValue, observed: legacyRoot?.[field] ?? null, message: "Known legacy v1 wrapper identity mismatch: " + field });
