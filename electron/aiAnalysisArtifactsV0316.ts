...
import { AI_DECISION_CONTRACT_VERSION, AI_DECISION_STATUSES, validateDecisionArray, type DecisionValidation, type ValidatedDecisionDocument } from "./aiAnalysisDecisionContractV0321.js";

export const AI_DECISION_SCHEMA_VERSION = AI_DECISION_CONTRACT_VERSION;
export const AI_ARTIFACT_CONTRACT_VERSION = "ai-analysis-artifacts-v0.3.20" as const;
export const PROVIDER_PERFORMANCE_WARNING_MS = 855_000;
export const PROVIDER_HARD_TIMEOUT_MS = 1_200_000;
export type { AiDecision, DecisionFinding, DecisionStatus, DecisionValidation } from "./aiAnalysisDecisionContractV0321.js";
...
export function parseAndValidateDecisions(raw: string, expected: { runId: string; sourceSha256: string; rulesSnapshotId: string; recordCount: number; catalogSkillIds: string[] }): { document: AiDecisionDocument; validation: DecisionValidation } {
  ...
  const legacyRoot = parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed as Record<string, unknown> : null;
  const legacyV1 = legacyRoot?.schemaVersion === "ai-analysis-decisions-v1" && Array.isArray(legacyRoot.decisions);
  const decisionValue = legacyV1
    ? (legacyRoot!.decisions as unknown[]).map((item) => item && typeof item === "object" && !Array.isArray(item) && (item as Record<string, unknown>).status === "MATCHED" ? { ...(item as Record<string, unknown>), status: "CLASSIFIED" } : item)
    : parsed;
  const validation = validateDecisionArray(decisionValue, { recordCount: expected.recordCount, catalogSkillIds: expected.catalogSkillIds });
  if (legacyV1) {
    validation.warnings.push("LEGACY_DECISION_V1_NORMALIZED");
    const identityChecks = [["runId", expected.runId, "AI_ARTIFACT_RECEIPT_INVALID"], ["sourceSha256", expected.sourceSha256, "AI_DECISION_SOURCE_HASH_MISMATCH"], ["rulesSnapshotId", expected.rulesSnapshotId, "AI_DECISION_RULES_SNAPSHOT_MISMATCH"], ["expectedRecordCount", expected.recordCount, "AI_DECISION_COUNT_MISMATCH"]] as const;
    for (const [field, expectedValue, code] of identityChecks) if (legacyRoot?.[field] !== expectedValue) validation.findings.push({ code, status: null, jsonPointer: "/" + field, recordIndex: null, expected: expectedValue, observed: legacyRoot?.[field] ?? null, message: "Known legacy v1 wrapper identity mismatch: " + field });
