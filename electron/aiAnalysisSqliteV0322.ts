...
import { sqliteQualityGate, type QualityReport } from "./aiAnalysisQualityV0322.js";
...
export function persistCompletedRunV0322(databasePath: string, dataset: AiPendingDataset, run: AiAnalysisRun, options: { attemptNumber?: number; failCandidateInsertAt?: number; qualityReport?: QualityReport; warningAcceptance?: { accepted: boolean; canonicalSha256: string; runId: string; acceptedAt: string } } = {}) {
  if (!run.results.length || run.canonicalRecordCount !== run.results.length || !run.analyzedFilePath) throw new AiAnalysisError("AI_SQLITE_BLOCKED", "Validated Canonical Result is required before SQLite commit.");  if (options.qualityReport) {
    const gate = sqliteQualityGate(options.qualityReport, options.warningAcceptance);
    if (!gate.eligible) throw new AiAnalysisError("AI_SQLITE_BLOCKED", `SQLite blocked by Quality Gate: ${gate.reason}.`);
  }
