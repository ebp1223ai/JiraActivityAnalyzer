...
import { AI_DECISION_FIELDS, AI_DECISION_STATUSES, getDecisionContractDescriptor } from "./aiAnalysisDecisionContractV0321.js";
...
Status 欄位矩陣：
- CLASSIFIED：skillIds 與 positiveEvidence 必須 non-empty；unknownReasons 必須 []。
- UNKNOWN：skillIds 與 positiveEvidence 必須 []；unknownReasons 必須 non-empty。
- NEEDS_REVIEW：skillIds 可空；有 candidate 時 positiveEvidence 必須 non-empty；unknownReasons 必須 []；理由放 rationale／checks。
- EXCLUDED：skillIds、positiveEvidence、unknownReasons 必須 []；理由放 negativeChecks／rationale。
- CATALOG_DETAIL_MISSING：保留實際 skillIds 與 positiveEvidence；unknownReasons 必須 []；理由放 rationale。
- FAILED：skillIds、positiveEvidence、unknownReasons 必須 []；structured error 放 rationale。
