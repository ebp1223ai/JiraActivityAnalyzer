...
export function buildCompletionManifest(input: { run: AiAnalysisRun; paths: CanonicalRunPaths; validation: DecisionValidation; decisionSha256: string; canonicalResultSha256: string; canonicalCount: number; sqliteEligible: boolean }) { return { schemaVersion: "ai-analysis-completion-manifest-v2", runId: input.run.runId, decisionFile: "ai-output/ai-analysis-decisions.json", reportFile: "ai-output/analysis-report.md", finalAssistantMessageFile: "ai-output/artifact-final-summary.txt", canonicalResultFile: "canonical-output/analysis-result.json", expectedRecordCount: input.validation.expectedCount, actualDecisionCount: input.validation.actualCount, actualCanonicalCount: input.canonicalCount, decisionSha256: input.decisionSha256, canonicalResultSha256: input.canonicalResultSha256, schemaValidationPassed: input.validation.schemaValid, semanticValidationPassed: input.validation.semanticValid, warnings: input.validation.warnings, finalStatus: input.validation.warnings.length ? "completed_with_warnings" : "completed", sqliteEligible: input.sqliteEligible, generatedAt: new Date().toISOString() }; }