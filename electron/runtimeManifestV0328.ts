...
import { RUNTIME_CONTRACT_V0328, type RuntimeContractV0328 } from "./runtimeContractRegistryV0328.js";
...
export type RuntimeManifestReceiptV0328 = {
...
export function validateModelInputManifestV3(value: unknown, runtimeContract: RuntimeContractV0328 = RUNTIME_CONTRACT_V0328) {
  ...
  return { manifest: manifest as ModelInputManifestV0328, fileCount: files.length, recordCount: Number(manifest.expectedRecordCount), segmentCount: totalSegments, totalBytes };
}

export function serializeAndValidateManifestResponseV0328(value: unknown, runtimeContract: RuntimeContractV0328 = RUNTIME_CONTRACT_V0328) {
...
export function createRuntimeManifestReceiptV0328(value: unknown, providerDispatchAllowed: boolean, runtimeContract: RuntimeContractV0328 = RUNTIME_CONTRACT_V0328): RuntimeManifestReceiptV0328 {
  const source = object(value);
  const preHash = sha256(JSON.stringify(value));
  try {
    const response = serializeAndValidateManifestResponseV0328(value, runtimeContract);
...
export function writeRuntimeManifestReceiptV0328(runDirectory: string, fileName: string, receipt: RuntimeManifestReceiptV0328) {
