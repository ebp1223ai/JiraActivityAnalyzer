...
import { RUNTIME_CONTRACT_V0335 } from "./runtimeContractRegistryV0335.js";
import { createModelInputManifestV3, createRuntimeManifestReceiptV0335, serializeAndValidateManifestResponseV0335, writeRuntimeManifestReceiptV0335, type RuntimeManifestReceiptV0335 } from "./runtimeManifestV0335.js";
...
import { getHostControlLifecycleControllerV0335, releaseHostControlLifecycleControllerV0335, type HostControlStateV0335 } from "./hostControlLifecycleV0335.js";
...
export class AnalysisBridgeV0335 {
  readonly version = "0.3.35-bridge-v15";
  ...
  private providerDispatchGateReceipt: RuntimeManifestReceiptV0335 | null = null;
  ...
  private readonly control: ReturnType<typeof getHostControlLifecycleControllerV0335>;
  ...
  constructor(private readonly config: Config) {
    ...
    this.control = getHostControlLifecycleControllerV0335(config.runDirectory, { failFinalizePostcondition: Boolean(config.testHooks?.failFinalizePostcondition) });
  ...
  preflight() {
    ...
    const gateManifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0335, sourceManifest, { deliveryHandle: `dispatch_gate_${"0".repeat(32)}`, artifactSubmissionToken: null });
    const receipt = createRuntimeManifestReceiptV0335(gateManifest, true, RUNTIME_CONTRACT_V0335);
    this.providerDispatchGateReceipt = receipt;
    writeRuntimeManifestReceiptV0335(this.config.runDirectory, "provider-dispatch-gate-receipt.json", receipt);
    durableJson(path.join(this.config.runDirectory, "control", "runtime-contract-registry.json"), RUNTIME_CONTRACT_V0335);
    ...
    return { ...modelSafe(legacy), version: this.version, runtimeContract: RUNTIME_CONTRACT_V0335, providerDispatchGate: receipt, bridgeExecutionContext: "jaa-bridge-execution-context-v1", modelDeliveryHandle: RUNTIME_CONTRACT_V0335.modelDeliveryHandleContract, transportProtocol: RUNTIME_CONTRACT_V0335.providerTransport, decisionContract: RUNTIME_CONTRACT_V0335.decisionContract, artifactSubmissionTokenContract: RUNTIME_CONTRACT_V0335.artifactSubmissionTokenContract, modelVisibleIdentityFields: [], hostOwnedIdentity: true };
  }
  ...
  async handle(tool: string, argsValue: unknown, context: AnalysisBridgeToolContext) {
    ...
    try {
      ...
      normalized = normalizeJaaError(error, { stage: this.stageForTool(tool), source: "AnalysisBridgeV0335", fallbackCode: "AI_INPUT_TOOL_FAILED", safeDetails: { callId: context.callId, toolName: tool, argumentNames: Object.keys(args).sort() } });
  ...
  terminate() { if (this.recoverableCursorError) { this.captureRootError(this.recoverableCursorError); this.recoverableCursorError = null; } this.executionContext.terminate(); releaseHostControlLifecycleControllerV0335(this.config.runDirectory); }
  ...
  private manifest(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const manifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0335, sourceManifest, { deliveryHandle, artifactSubmissionToken: this.config.instructionMode === "CUSTOM_DIAGNOSTIC" ? null : this.artifactToken });
    const receipt = createRuntimeManifestReceiptV0335(manifest, true, RUNTIME_CONTRACT_V0335);
    writeRuntimeManifestReceiptV0335(this.config.runDirectory, "runtime-manifest-receipt.json", receipt);
    writeRuntimeManifestReceiptV0335(this.config.runDirectory, "runtime-manifest-self-validation.json", receipt);
    writeRuntimeManifestReceiptV0335(this.config.runDirectory, "model-input-manifest-receipt.json", receipt);
  ...
  serializeToolResponse(tool: string, result: unknown) {
    if (tool === "jaa_get_input_manifest") {
      const response = serializeAndValidateManifestResponseV0335(result, RUNTIME_CONTRACT_V0335);
  ...
  private finalize(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const receipt = this.control.finalize({ bindingHash, receipt: { ...legacyReceipt, transportProtocol: RUNTIME_CONTRACT_V0335.providerTransport } });
  ...
  private publish(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    if (decisions.length === identity.sourceRecordCount && decisions.length > 0 && decisions.every((decision) => decision.status === "FAILED")) {
      ...
      this.control.complete("FAILED", "AI_PROVIDER_RUN_LEVEL_FAILURE");
    ...
    const pipeline = runLayeredValidationV0335({ runDirectory: this.config.runDirectory, identity: identity as never, submission, quoteCatalog: this.quoteCatalog, catalogSkillIds: this.config.catalogSkillIds, tokenBindingReceipt: identityReceipt });
    ...
    const artifacts = [
      { role: "DECISIONS_JSON", ...durableText(path.join(outputDirectory, "ai-analysis-decisions.json"), JSON.stringify(v4) + "\n") },
      { role: "ANALYSIS_REPORT", ...durableText(path.join(outputDirectory, "analysis-report.md"), submission.analysisReportMarkdown.trim() + "\n") },
      { role: "FINAL_ASSISTANT_MESSAGE", ...durableText(path.join(outputDirectory, "final-assistant-message.txt"), submission.finalSummaryZhTw.trim() + "\n") }
    ];
    this.artifactReceipt = {
      schemaVersion: "jaa-analysis-artifact-receipt-v4",
      status: "published",
      decisionCount: pipeline.validation.actualCount,
      authoritativeDistribution: pipeline.validation.distribution,
      warnings: pipeline.validation.findings.filter((finding) => finding.severity !== "ERROR"),
      decisionContractVersion: RUNTIME_CONTRACT_V0335.decisionContract,
  ...
  setControlStage(state: HostControlStateV0335, reason: string, receiptId: string | null = null) { return this.control.transition(state, reason, receiptId); }
  ...
  setPostBridgeStage(stage: "VALIDATION_COMPLETED" | "CANONICAL_ASSEMBLY_COMPLETED" | "RUN_COMPLETED", status: "completed" | "failed") { this.legacy.setPostBridgeStage(stage, status); if (stage === "CANONICAL_ASSEMBLY_COMPLETED" && status === "completed") this.control.transition("CANONICAL_CREATED", "Canonical durable write completed."); else if (stage === "RUN_COMPLETED") this.control.complete(status === "completed" ? "SUCCEEDED" : "FAILED", status === "failed" ? "AI_RUN_FAILED" : null); }
  fail(stage: AnalysisLifecycleStage, code: string, message = code) { this.control.complete("FAILED", code); const normalized = normalizeJaaError(Object.assign(new Error(`${code}:${message}`), { code }), { stage, source: "AnalysisBridgeV0335", fallbackCode: code }); this.captureRootError(normalized); }
  ...
  snapshot() {
    ...
    const lifecycle = {
      ...
      analysisStatus: ["COMPLETED", "COMPLETED_BY_ARTIFACT"].includes(this.control.snapshot().analysisTelemetry.state) ? "completed" as const : this.control.snapshot().analysisTelemetry.state === "NOT_REPORTED" ? "not_started" as const : "running" as const,
      ...
      artifactStatus: this.submissionRejectedCode ? "rejected" as const : this.artifactReceipt ? "published" as const : this.artifactAttempt > 0 ? "received" as const : snapshot.lifecycle.artifactStatus,
      ...
      analysisCompleted: ["COMPLETED", "COMPLETED_BY_ARTIFACT"].includes(this.control.snapshot().analysisTelemetry.state),
    ...
    return {
      ...
      transportProtocol: RUNTIME_CONTRACT_V0335.providerTransport,
      runtimeContract: RUNTIME_CONTRACT_V0335,
      providerDispatchGate: this.providerDispatchGateReceipt,
      decisionContract: RUNTIME_CONTRACT_V0335.decisionContract,
...
export function createAnalysisBridge(config: Config) { return new AnalysisBridgeV0335(config); }
