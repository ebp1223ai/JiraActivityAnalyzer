...
export const HOST_CONTROL_LIFECYCLE_VERSION_V0337 = "jaa-host-control-lifecycle-v3" as const;
export const ANALYSIS_TELEMETRY_VERSION_V0337 = "jaa-analysis-telemetry-v1" as const;

export type HostControlStateV0337 =
...
export type AnalysisTelemetryStateV0337 =
...
type Transition = {
  sequence: number;
  stateBefore: HostControlStateV0337;
  stateAfter: HostControlStateV0337;
  reason: string;
...
const ORDER: HostControlStateV0337[] = [
  "ATTEMPT_CREATED", "PROVIDER_DISPATCHED", "INPUT_READING", "INPUT_READY",
  "ARTIFACT_RECEIVED", "VALIDATING", "CANONICAL_CREATED",
...
export class HostControlLifecycleControllerV0337 {
  private state: HostControlStateV0337 = "ATTEMPT_CREATED";
  private telemetry: AnalysisTelemetryStateV0337 = "NOT_REPORTED";
  private sequence = 0;
  ...
  transition(next: HostControlStateV0337, reason: string, receiptId: string | null = null, idempotencyKey = `${next}:${receiptId ?? reason}`) {
  ...
  snapshot() {
    return Object.freeze({
      schemaVersion: HOST_CONTROL_LIFECYCLE_VERSION_V0337,
      ...
      analysisTelemetry: { schemaVersion: ANALYSIS_TELEMETRY_VERSION_V0337, state: this.telemetry, lastProgress: this.lastProgress, warnings: [...this.warnings] },
...
const controllers = new Map<string, HostControlLifecycleControllerV0337>();

export function getHostControlLifecycleControllerV0337(runDirectory: string, hooks: { failFinalizePostcondition?: boolean } = {}) {
  const key = path.resolve(runDirectory).toLowerCase();
  const existing = controllers.get(key);
  if (existing) return existing;
  const controller = new HostControlLifecycleControllerV0337(runDirectory, hooks);
  controllers.set(key, controller);
  return controller;
}

export function releaseHostControlLifecycleControllerV0337(runDirectory: string) {
