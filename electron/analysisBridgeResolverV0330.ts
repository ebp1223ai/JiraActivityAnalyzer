...
export const BRIDGE_IDENTITY_V0330 = "0.3.30-bridge-v12" as const;
export const BRIDGE_ARTIFACT_V0330 = "analysis-bridge-v0330.cjs" as const;
export const BRIDGE_MANIFEST_V0330 = "analysis-bridge-manifest-v0330.json" as const;
...
export type PackagedBridgeManifestV0330 = {
  schemaVersion: typeof PACKAGED_BRIDGE_SCHEMA;
  bridgeIdentity: typeof BRIDGE_IDENTITY_V0330;
  artifactFileName: typeof BRIDGE_ARTIFACT_V0330;
...
export type BridgePreflightReceiptV0330 = {
  ...
  expectedBridgeIdentity: typeof BRIDGE_IDENTITY_V0330;
...
export type BridgeRuntimeContextV0330 = {
  appIsPackaged: boolean;
  processResourcesPath: string;
  developmentResourcesPath?: string;
  distributionKind?: BridgePreflightReceiptV0330["distributionKind"];
  phase: BridgePreflightReceiptV0330["phase"];
  attemptId?: string | null;
  expectedBytes?: number;
  expectedSha256?: string;
};
...
export function bridgeDistributionKindV0330(appIsPackaged: boolean): BridgePreflightReceiptV0330["distributionKind"] {
...
export function resolveAnalysisBridgeArtifactV0330(context: BridgeRuntimeContextV0330) {
  ...
  const artifactPath = path.join(resourceDirectory, BRIDGE_ARTIFACT_V0330);
  const manifestPath = path.join(resourceDirectory, BRIDGE_MANIFEST_V0330);
  ...
  const receipt: BridgePreflightReceiptV0330 = {
    ...
    distributionKind: context.distributionKind ?? bridgeDistributionKindV0330(context.appIsPackaged),
    processResourcesPath: context.processResourcesPath,
    expectedRelativePath: `${BRIDGE_RESOURCE_DIRECTORY}/${BRIDGE_ARTIFACT_V0330}`,
    resolvedAbsolutePath: null, manifestAbsolutePath: manifestPath,
    expectedBridgeIdentity: BRIDGE_IDENTITY_V0330, observedBridgeIdentity: null,
  ...
  try {
    ...
    let manifest: PackagedBridgeManifestV0330;
    try { manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PackagedBridgeManifestV0330; }
    ...
    if (manifest.schemaVersion !== PACKAGED_BRIDGE_SCHEMA || manifest.bridgeIdentity !== BRIDGE_IDENTITY_V0330 || manifest.artifactFileName !== BRIDGE_ARTIFACT_V0330) throw failure("AI_BRIDGE_VERSION_MISMATCH", "Bridge manifest identity or artifact filename does not match v0.3.30.");
    if (manifest.runtimeContractRegistry !== "jaa-runtime-contract-registry-v1" || manifest.transport !== "bridge-resumable-v3" || manifest.decisionContract !== "jaa-ai-analysis-decisions-v5" || manifest.localOnly !== true || manifest.externalFallback !== false) throw failure("AI_BRIDGE_CONTRACT_MISMATCH", "Bridge manifest runtime contract does not match v0.3.30.");
