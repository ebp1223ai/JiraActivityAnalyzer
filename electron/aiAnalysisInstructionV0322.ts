...
import { AI_DECISION_CONFIDENCE_VALUES, AI_DECISION_FIELDS, AI_DECISION_STATUSES, AI_SKILL_FINDING_FIELDS, getDecisionContractDescriptor } from "./aiAnalysisDecisionContractV0322.js";
...
export const DEFAULT_ANALYSIS_INSTRUCTION = `# Standard Formal 正式分析契約 JAA-CHATGPT-ZH-TW-0.3.22
...
Record 與每個 Skill Finding 的 confidence 都只能是 JSON number 0、0.3、0.6、0.9。每個 skillFindings[] 只能包含 skillId、confidence、evidenceQuotes、evidenceExplanation、negativeChecks、rationale。每個 Skill 必須有自己的 Evidence Quote、Explanation、Negative Checks 與 Rationale，不得把 Record 共用句複製成獨立分析。evidenceQuotes[] 只能包含 evidenceRef 與 quote；quote 必須是 JAA Evidence Normalizer 所產生可讀來源的 exact substring。
...
Model Delivery Receipt 完成後才可開始分析。jaa_publish_analysis_artifacts.decisionsDocument 必須是 direct array，共 ${input.recordCount} 筆。每筆僅能包含：${AI_DECISION_FIELDS.join(", ")}。每個 Skill Finding 僅能包含：${AI_SKILL_FINDING_FIELDS.join(", ")}。status 僅能是：${AI_DECISION_STATUSES.join(" | ")}。confidence 僅能是：${AI_DECISION_CONFIDENCE_VALUES.join(" | ")}。
decisionContractVersion=${decisionContract.schemaVersion}
decisionContractSha256=${decisionContract.sha256}`;
  const sections = [
