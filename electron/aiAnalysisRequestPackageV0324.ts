...
export const CORE_INSTRUCTION_VERSION = "0.3.37-zh-TW-v18" as const;
...
export function buildRequestPackage(input: { runId: string; runDirectory: string; dataset: AiPendingDataset; rules: AiRulesSnapshot; supplementalInstruction?: string; instructionMode?: AiInstructionMode; userAdditionalInstruction?: string; userCustomInstruction?: string; selectedRecordCount: number; modelInputJsonPath?: string }) {
  ...
  atomicExport(path.join(control, "bridge-contract.json"), JSON.stringify({ schemaVersion: "jaa-analysis-bridge-contract-v1", version: "0.3.37-bridge-v17", executionContext: "jaa-bridge-execution-context-v1", modelDeliveryHandle: "jaa-model-delivery-handle-v1", modelVisibleIdentityFields: [], transport: "codex_dynamic_tools_stdio", modelInputTransport: "bridge-resumable-v4", localOnly: true, tools: ["jaa_get_input_manifest", "jaa_read_and_ack_next_segment", "jaa_get_delivery_status", "jaa_finalize_input_delivery", "jaa_report_analysis_progress", "jaa_publish_analysis_artifacts_v5"], arbitraryPath: false, arbitraryCommand: false, externalFallback: false }, null, 2));
  const requestPackage: AiAnalysisRequestPackage = {
    requestPackageVersion: REQUEST_PACKAGE_VERSION, applicationVersion: "0.3.37", promptIdentity: "JAA-CHATGPT-ZH-TW-0.3.37", qualityContractVersion: input.rules.qualityContractVersion, bridgeIdentity: "0.3.37-bridge-v17", rulesSnapshotId: input.rules.snapshotId ?? input.rules.ruleSetId, instructionMode, instructionComposition: composition, modelInputTransport: "bridge-resumable-v4", promptLocale: "zh-TW", responseLocale: "zh-TW", promptTemplateVersion: CORE_INSTRUCTION_VERSION, decisionContractVersion: decisionContract.schemaVersion, decisionContractSha256: decisionContract.sha256, runId: input.runId, createdAtLocal: created.toLocaleString("sv-SE", { timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone }), createdAtUtc: created.toISOString(), localTimeZone: Intl.DateTimeFormat().resolvedOptions().timeZone,
