...
import { buildUserActivityTimeline, classifyTimelineSource, timelineCsv, timelineEventSchema, type UserActivityTimelineBuild } from "./userActivityTimeline.js";
...
async function runUiSmoke(window: BrowserWindow) {
  ...
  const timelineIssueFilterAudit = await window.webContents.executeJavaScript(`(async () => {
    const groups = ["filter-activity-type", "filter-confidence", "filter-role", "filter-source-system", "filter-project-key"];
    const sourceChecks = Array.from(document.querySelectorAll("[data-testid='filter-source-system'] input[type='checkbox']"));
    ...
    return { groups: groups.every((id) => Boolean(document.querySelector("[data-testid='" + id + "']"))), initial, defaultNote: Boolean(document.querySelector("[data-testid='jira-default-filter-note']")), sourceColumn: document.querySelector("[data-testid='timeline-issue-groups-table']")?.textContent?.includes("Source Systems"), summaryAfterMulti, sameCategoryOr: summaryAfterMulti.includes(" OR "), categoriesAnd: summaryAfterMulti.includes(" AND "), activityCleared, allCleared, rows, selected };
  })()`);
  if (!timelineIssueFilterAudit.groups || JSON.stringify(timelineIssueFilterAudit.initial) !== JSON.stringify([true, false, false, false]) || !timelineIssueFilterAudit.defaultNote || !timelineIssueFilterAudit.sourceColumn || !timelineIssueFilterAudit.sameCategoryOr || !timelineIssueFilterAudit.categoriesAnd || !timelineIssueFilterAudit.activityCleared || !timelineIssueFilterAudit.allCleared || timelineIssueFilterAudit.rows < 1 || timelineIssueFilterAudit.selected !== timelineIssueFilterAudit.rows) failures.push(`timeline issue multi-select filter audit failed ${JSON.stringify(timelineIssueFilterAudit)}`);
