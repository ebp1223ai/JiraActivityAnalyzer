...
ipcMain.handle("debug-log:save-bundle", async (_event, payload: { debugLog: string; currentPage: string }) => {
  ...
  const failureSummary = fullFetchFailureSummary(latestFullFetchFailedIssues);
  writeBundleJson(folderPath, "full-fetch-failed-issues.json", latestFullFetchFailedIssues);
...
async function runUiSmoke(window: BrowserWindow) {
  ...
  const timelineFixture = buildUserActivityTimeline({
    ...
    entries: [
      ...
      { issueKey: "COPGEN1-125695", extractedIssueKeysPerEntry: ["COPGEN1-125695", "COPGEN1-125806"], activityTime: "2026-07-02T05:44:09.000Z", activityType: "link", activityApplication: "Jira", activityTitle: "linked COPGEN1-125695 to COPGEN1-125806", activityAuthor: "謝正洪", activityAuthorEmail: "roger_hsieh", activityTypeClassifier: { matchedRule: "link_action", finalType: "link" }, entryFingerprint: "sha256:fixture-secondary", variant: "escaped_username", entryIndex: 3 },
  ...
  if (failedStepAudit.state !== "failed" || !failedStepAudit.reason || !failedStepAudit.style.includes("bg-red-50") || warningStepAudit.state !== "warning" || !warningStepAudit.reason || !warningStepAudit.style.includes("bg-amber-50")) failures.push(`User Analysis warning/failed state audit failed: ${JSON.stringify({ failedStepAudit, warningStepAudit })}`);
  ...
  await wait(2200);
  const timelineUiAudit = await window.webContents.executeJavaScript(`(() => { const table = document.querySelector("[data-testid='timeline-events-table']"); const text = document.body.innerText || ""; const rows = table?.querySelectorAll("tbody > tr").length || 0; table?.querySelector("tbody button")?.click(); return { rows, issue: text.includes("SMOKE-101"), link: text.includes("link"), page: text.includes("page"), sourceSystem: table?.textContent?.includes("Source Application") && table?.textContent?.includes("jira"), sourceDetail: table?.textContent?.includes("Source Detail") && table?.textContent?.includes("jira_activity_stream"), export: text.includes("user-activity-timeline-") && text.includes("timeline-build-summary-"), integrity: Boolean(document.querySelector("[data-testid='timeline-integrity-diagnostics']")), countWarning: Boolean(document.querySelector("[data-testid='timeline-integrity-count-warning']")), secondaryInfo: Boolean(document.querySelector("[data-testid='timeline-integrity-secondary-info']")) }; })()`);
  await wait(100);
  ...
  if (!timelineTabAudit.panel || !timelineTabAudit.button || timelineUiAudit.rows < 1 || !timelineUiAudit.issue || !timelineUiAudit.link || !timelineUiAudit.page || !timelineUiAudit.sourceSystem || !timelineUiAudit.sourceDetail || !timelineUiAudit.export || !timelineUiAudit.integrity || !timelineUiAudit.countWarning || !timelineUiAudit.secondaryInfo || !timelineDetailAudit.eventId || !timelineDetailAudit.fingerprint || !timelineDetailAudit.baseline || !timelineDetailAudit.retry || !timelineFilterAudit || timelineFilteredRows !== 1 || !timelineExportAudit?.json || !timelineExportAudit.csv || !timelineExportAudit.summary || !timelineExportAudit.bom || !timelineExportAudit.eventIds || !timelineExportAudit.sourceCounts || !timelineExportAudit.secondary || !timelineExportAudit.reconciliation || timelineExportAudit.sensitive) failures.push(`activity timeline UI/export audit failed ${JSON.stringify({ timelineTabAudit, timelineUiAudit, timelineDetailAudit, timelineFilteredRows, timelineExportAudit })}`);
  ...
  const timelineIssueFilterAudit = await window.webContents.executeJavaScript(`(async () => {
    const groups = ["filter-jira-relation", "filter-activity-type", "filter-confidence", "filter-role", "filter-source-application", "filter-project-key"];
    ...
    return { groups: groups.every((id) => Boolean(document.querySelector("[data-testid='" + id + "']"))), relationInitial, initial, defaultNote: Boolean(document.querySelector("[data-testid='jira-default-filter-note']")), sourceColumn: document.querySelector("[data-testid='timeline-issue-groups-table']")?.textContent?.includes("Source Applications"), relationColumn: document.querySelector("[data-testid='timeline-issue-groups-table']")?.textContent?.includes("Jira Relation"), summaryAfterMulti, sameCategoryOr: summaryAfterMulti.includes(" OR "), categoriesAnd: summaryAfterMulti.includes(" AND "), activityCleared, allCleared, rows, selected };
  })()`);
  if (!timelineIssueFilterAudit.groups || JSON.stringify(timelineIssueFilterAudit.relationInitial) !== JSON.stringify([true, false, false, false]) || JSON.stringify(timelineIssueFilterAudit.initial) !== JSON.stringify([true, true, false, false]) || !timelineIssueFilterAudit.defaultNote || !timelineIssueFilterAudit.sourceColumn || !timelineIssueFilterAudit.relationColumn || !timelineIssueFilterAudit.sameCategoryOr || !timelineIssueFilterAudit.categoriesAnd || !timelineIssueFilterAudit.activityCleared || !timelineIssueFilterAudit.allCleared || timelineIssueFilterAudit.rows < 1 || timelineIssueFilterAudit.selected !== timelineIssueFilterAudit.rows) failures.push(`timeline issue multi-select filter audit failed ${JSON.stringify(timelineIssueFilterAudit)}`);
  const timelineQueueUiAudit = await window.webContents.executeJavaScript(`(async () => {
    ...
    const add = Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.includes("Add Selected Issues, then go to Step 4"));
    add?.click();
  ...
  await wait(120);
  const recommendedQueueAudit = await window.webContents.executeJavaScript(`(() => { const text = document.body.innerText || ""; return { source: text.includes("recommended_related_issue"), summary: Boolean(document.querySelector("[data-testid='queue-add-summary']")) && text.includes("Run Related Issues in Step 4: Full Fetch") }; })()`);
  await window.webContents.executeJavaScript(`window.dispatchEvent(new CustomEvent("jaa:seed-related-scope"));`);
  await wait(120);
  ...
  await window.webContents.executeJavaScript(`document.querySelector("[data-testid='workflow-timeline']")?.click();`);
  await wait(100);
  await window.webContents.executeJavaScript(`window.desktopApp?.userAnalysis?.updateWorkflowSnapshot?.({ steps: { activityTimeline: "completed", timelineIssueSelection: "completed", fetchQueue: "ready", fullFetch: "completed", relatedIssues: "completed", exports: "not_run", advancedCandidateSearch: "not_run" }, timelineIssueGroups: [{ issueKey: "COPGEN1-125806", source: "activity_timeline", issueKeyRole: "secondary", isJiraRelated: true, hasJiraIssueKey: true, sourceSystemSummary: { jira: 1, confluence: 0, other: 0, unknown: 0 }, sourceDetails: { jira_activity_stream: 1 }, jiraRelationSummary: { jiraRelatedEventCount: 1, nonJiraRelatedEventCount: 0, hasJiraIssueKeyCount: 1, sourceApplicationCounts: { jira: 1, confluence: 0, other: 0, unknown: 0 }, relatedSystemsCounts: { jira: 1 }, jiraRelationReasons: { has_jira_issue_key: 1 } } }], timelineSelectedIssues: ["COPGEN1-125806"], fetchQueue: [{ key: "COPGEN1-125806", queueMetadata: { sources: ["activity_timeline"] } }], relatedCandidateIssues: [{ issueKey: "COPGEN1-69506", relationType: "parent_link", scope: "recommended", discoveredFromIssueKey: "COPGEN1-126606", source: "related_issue_expansion", field: "parent", reason: "parent_link discovered from COPGEN1-126606", confidence: "high", firstSeen: "2026-07-02T06:00:00.000Z", lastSeen: "2026-07-02T06:00:00.000Z", evidenceCount: 1, selected: false }], addedTimelineIssuesToFetchQueueCount: 1, addedRelatedIssuesToFetchQueueCount: 1, addedRecommendedRelatedIssuesToFetchQueueCount: 1, addedOptionalRelatedIssuesToFetchQueueCount: 0 });`);
  ...
  if (!failedIssuesUiAudit.panel || !failedIssuesUiAudit.issueKey || !failedIssuesUiAudit.status || !failedIssuesUiAudit.stage || !failedIssuesUiAudit.source || !failedIssuesUiAudit.reason || !failedIssuesUiAudit.message) failures.push(`full fetch failed issues UI audit failed ${JSON.stringify(failedIssuesUiAudit)}`);
