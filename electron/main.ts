...
ipcMain.handle("user-analysis:build-activity-timeline", async (_event, payload: { connection: AppConnection; selectedUser: string; startDate: string; endDate: string; projectScope?: string }) => {
  ...
  const guardedRun = run as unknown as { baselineComparison: ActivityStreamBaselineComparison; baselineGuardRetry: BaselineGuardRetrySummary };
  const comparison = guardedRun.baselineComparison;
  const retry = guardedRun.baselineGuardRetry;
  const entries = Array.isArray(run.clientDateFilteredEntriesSanitized) ? run.clientDateFilteredEntriesSanitized : run.activityStream.entriesSanitized;
...
async function runUiSmoke(window: BrowserWindow) {
  ...
  const timelineFixture = buildUserActivityTimeline({
    ...
    baseline: { classification: "accepted_equal", retryTriggered: true, retryRecovered: true, baselineBestParsedActivityCount: 17, currentParsedActivityCount: 17 },
    entries: [
  ...
  if (timelineFixture.events.length !== 3 || timelineFixture.events[1]?.eventType !== "link" || !timelineFixture.events.some((event) => event.eventType === "comment" && event.issueKey === "COPGEN1-138930" && event.sourceConfidence === "high") || !timelineFixture.events.some((event) => event.eventType === "page" && event.sourceConfidence === "medium") || timelineFixture.events.some((event) => !event.eventId.startsWith("sha256:") || !event.rawRef.entryFingerprint || event.evidence.baselineGuard.classification !== "accepted_equal") || !timelineFixtureCsv.startsWith("\uFEFF")) failures.push(`timeline mapping fixtures failed ${JSON.stringify(timelineFixture)}`);
  ...
  await wait(2200);
  const timelineUiAudit = await window.webContents.executeJavaScript(`(() => { const table = document.querySelector("[data-testid='timeline-events-table']"); const text = document.body.innerText || ""; const rows = table?.querySelectorAll("tbody > tr").length || 0; table?.querySelector("tbody button")?.click(); return { rows, issue: text.includes("SMOKE-101"), link: text.includes("link"), page: text.includes("page"), export: text.includes("user-activity-timeline-") && text.includes("timeline-build-summary-") }; })()`);
  await wait(100);
  ...
  const timelineExportAudit = latestUserActivityTimeline ? {
    ...
    eventIds: latestUserActivityTimeline.events.every((event) => event.eventId.startsWith("sha256:") && event.rawRef.entryFingerprint && event.evidence.baselineGuard.classification),
  ...
  if (!timelineTabAudit.panel || !timelineTabAudit.button || timelineUiAudit.rows < 1 || !timelineUiAudit.issue || !timelineUiAudit.link || !timelineUiAudit.page || !timelineUiAudit.export || !timelineDetailAudit.eventId || !timelineDetailAudit.fingerprint || !timelineDetailAudit.baseline || !timelineDetailAudit.retry || !timelineFilterAudit || timelineFilteredRows !== 1 || !timelineExportAudit?.json || !timelineExportAudit.csv || !timelineExportAudit.summary || !timelineExportAudit.bom || !timelineExportAudit.eventIds || timelineExportAudit.sensitive) failures.push(`activity timeline UI/export audit failed ${JSON.stringify({ timelineTabAudit, timelineUiAudit, timelineDetailAudit, timelineFilteredRows, timelineExportAudit })}`);
  ...
  if (debugBundlePath) {
    ...
    if (bundleUserTimelineSummary.available !== true || !String(bundleUserTimeline.timelineRunId).startsWith("tlrun-") || Number(bundleTimelineSummary.totalEvents) < 1 || Number(bundleTimelineSchema.schemaVersion) !== 1 || bundleUserTimelineEvents.some((event) => !String(event.eventId).startsWith("sha256:")) || !bundleTimelineCsv.subarray(0, 3).equals(Buffer.from([0xef, 0xbb, 0xbf])) || !bundleTimeline.some((entry) => entry.type === "user_activity_timeline_built") || !bundleText.includes("User Activity Timeline:")) failures.push(`debug bundle timeline audit failed: ${JSON.stringify({ bundleUserTimelineSummary, timelineRunId: bundleUserTimeline.timelineRunId, totalEvents: bundleTimelineSummary.totalEvents, schemaVersion: bundleTimelineSchema.schemaVersion })}`);
