...
import { resolveAnalysisBridgeArtifactV0337 } from "./analysisBridgeResolverV0337.js";
...
if (shouldVerifyAnalysisBridge) {
  const resolution = resolveAnalysisBridgeArtifactV0337({ appIsPackaged: app.isPackaged, processResourcesPath: process.resourcesPath, developmentResourcesPath: __dirname, phase: "diagnostic" });
...
ipcMain.handle("debug-log:save-bundle", async (_event, payload: { debugLog: string; currentPage: string; fullFetchIdentity?: Partial<FullFetchRunIdentity> }) => {
  ...
  let selectedRunStatus: string | null = null; let selectedInstructionMode: string | null = null; let selectedArtifactStatus: string | null = null; let selectedHtmlStatus: string | null = null; let selectedSqliteStatus: string | null = null;
  ...
  const contentCompleteness = !selectedAiRunPath ? "incomplete_unexpected" : trulyMissingFiles.length || aiHashMismatches.length || aiFlushFailed ? "incomplete_unexpected" : expectedAbsentForFailedRun.length ? "incomplete_expected_for_failed_run" : "complete";
