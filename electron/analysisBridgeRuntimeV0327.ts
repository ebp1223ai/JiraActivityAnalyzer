...
import { buildIdentityReceiptV0327, createApprovedArtifactIdentityV0327, persistAiSubmittedArtifactV0327, type ApprovedArtifactIdentityV0327, type ArtifactSubmissionV0327 } from "./aiArtifactIdentityV0327.js";
...
export class AnalysisBridgeV0327 {
  readonly version = "0.3.27-bridge-v9";
  readonly schemaVersion = "jaa-analysis-bridge-v9";
  ...
  private approvedIdentity: ApprovedArtifactIdentityV0327 | null = null;
  ...
  preflight() {
    ...
    return { ...modelSafe(legacy), version: this.version, bridgeExecutionContext: "jaa-bridge-execution-context-v1", modelDeliveryHandle: "jaa-model-delivery-handle-v1", transportProtocol: "bridge-resumable-v3", decisionContract: "jaa-ai-analysis-decisions-v5", artifactSubmissionTokenContract: "jaa-artifact-submission-token-v1", modelVisibleIdentityFields: [], hostOwnedIdentity: true };
  }
  ...
  bindProviderTurn(threadId: string, turnId: string) {
    ...
    this.approvedIdentity = createApprovedArtifactIdentityV0327({
  ...
  async handle(tool: string, argsValue: unknown, context: AnalysisBridgeToolContext) {
    ...
    try {
      ...
      normalized = normalizeJaaError(error, { stage: this.stageForTool(tool), source: "AnalysisBridgeV0327", fallbackCode: "AI_INPUT_TOOL_FAILED", safeDetails: { callId: context.callId, toolName: tool, argumentNames: Object.keys(args).sort() } });
  ...
  private manifest(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const safeManifest = modelSafe(this.manifestCache);
    const sourceReceipt = this.legacy.snapshot().sourceInputReceipt ?? {};
    const deliveryHandle = this.executionContext.issueDeliveryHandle(sha256(JSON.stringify(safeManifest)), sha256(JSON.stringify(sourceReceipt)));
    return { ...safeManifest, schemaVersion: "jaa-model-input-manifest-v3", deliveryHandleContract: "jaa-model-delivery-handle-v1", deliveryHandle, artifactSubmissionToken: this.config.instructionMode === "CUSTOM_DIAGNOSTIC" ? null : this.artifactToken, hostOwnedIdentity: true };
  }
  ...
  private publish(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const submission: ArtifactSubmissionV0327 = { artifactSubmissionToken: typeof args.artifactSubmissionToken === "string" ? args.artifactSubmissionToken : "", decisions: args.decisions, analysisReportMarkdown: String(args.analysisReportMarkdown ?? ""), finalSummaryZhTw: String(args.finalSummaryZhTw ?? "") };
    const persisted = persistAiSubmittedArtifactV0327(this.config.runDirectory, this.artifactAttempt, submission, identity);
    ...
    const reject = (code: string, message: string): never => {
      ...
      const receipt = { ...buildIdentityReceiptV0327(identity, identity, code), accepted: false, firstMismatchCode: code, tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
    ...
    const identityReceipt = { ...buildIdentityReceiptV0327(identity, identity, "MATCH"), tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
  ...
  fail(stage: AnalysisLifecycleStage, code: string, message = code) { const normalized = normalizeJaaError(Object.assign(new Error(`${code}:${message}`), { code }), { stage, source: "AnalysisBridgeV0327", fallbackCode: code }); this.captureRootError(normalized); }
...
export function createAnalysisBridge(config: Config) { return new AnalysisBridgeV0327(config); }
