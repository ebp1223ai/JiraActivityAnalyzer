...
import { resolveAnalysisBridgeArtifactV0337, type BridgePreflightReceiptV0337 } from "./analysisBridgeResolverV0337.js";
...
export function preflightAnalysisBridgeV0337(phase: BridgePreflightReceiptV0337["phase"], attemptId?: string | null) {
  return resolveAnalysisBridgeArtifactV0337({ appIsPackaged: app.isPackaged, processResourcesPath: process.resourcesPath, developmentResourcesPath: __dirname, phase, attemptId });
}

export function loadAnalysisBridge(input: BridgeInput) {
  const resolution = preflightAnalysisBridgeV0337("analysis_start", input.analysisAttemptId);
