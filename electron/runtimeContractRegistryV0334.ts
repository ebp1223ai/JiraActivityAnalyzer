...
export const RUNTIME_CONTRACT_REGISTRY_VERSION_V0334 = "jaa-runtime-contract-registry-v1" as const;

const registryValue = {
  schemaVersion: RUNTIME_CONTRACT_REGISTRY_VERSION_V0334,
...
export const RUNTIME_CONTRACT_V0334 = deepFreeze({
  ...registryValue,
  registryHash: crypto.createHash("sha256").update(JSON.stringify(registryValue)).digest("hex")
});

export type RuntimeContractV0334 = typeof RUNTIME_CONTRACT_V0334;

export function validateDispatchIdentityV0334(input: {
  ...
  const expected = RUNTIME_CONTRACT_V0334;
  const mismatches = [
