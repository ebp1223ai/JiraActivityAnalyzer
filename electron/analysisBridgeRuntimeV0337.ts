...
import { buildArtifactIdentityReceiptV0337, injectArtifactActualIdentityV0337, sealDispatchExpectedIdentityV0337, type ArtifactIdentityV0337, type ArtifactSubmissionV0337, type RunIdentityFieldsV0337 } from "./aiArtifactIdentityV0337.js";
...
import { RUNTIME_CONTRACT_V0337 } from "./runtimeContractRegistryV0337.js";
import { createModelInputManifestV3, createRuntimeManifestReceiptV0337, serializeAndValidateManifestResponseV0337, writeRuntimeManifestReceiptV0337, type RuntimeManifestReceiptV0337 } from "./runtimeManifestV0337.js";
...
import { getHostControlLifecycleControllerV0337, releaseHostControlLifecycleControllerV0337, type HostControlStateV0337 } from "./hostControlLifecycleV0337.js";
import { persistProviderArtifactV0337 } from "./artifactPersistenceV0337.js";
import { ReducerFactJournalV0337 } from "./reducerFactJournalV0337.js";
import { ArtifactLifecycleV0337, type ArtifactStateV0337 } from "./artifactLifecycleV0337.js";
...
export class AnalysisBridgeV0337 {
  readonly version = "0.3.37-bridge-v17";
  ...
  private approvedIdentity: ArtifactIdentityV0337 | null = null;
  private identityInput: RunIdentityFieldsV0337 | null = null;
  ...
  private providerDispatchGateReceipt: RuntimeManifestReceiptV0337 | null = null;
  ...
  private readonly control: ReturnType<typeof getHostControlLifecycleControllerV0337>;
  private readonly factJournal: ReducerFactJournalV0337;
  private readonly artifactLifecycle = new ArtifactLifecycleV0337();
  ...
  constructor(private readonly config: Config) {
    ...
    this.control = getHostControlLifecycleControllerV0337(config.runDirectory, { failFinalizePostcondition: Boolean(config.testHooks?.failFinalizePostcondition) });
    this.factJournal = new ReducerFactJournalV0337(config.runDirectory, config.runId);
  ...
  preflight() {
    ...
    const gateManifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0337, sourceManifest, { deliveryHandle: `dispatch_gate_${"0".repeat(32)}`, artifactSubmissionToken: null });
    const receipt = createRuntimeManifestReceiptV0337(gateManifest, true, RUNTIME_CONTRACT_V0337);
    this.providerDispatchGateReceipt = receipt;
    writeRuntimeManifestReceiptV0337(this.config.runDirectory, "provider-dispatch-gate-receipt.json", receipt);
    durableJson(path.join(this.config.runDirectory, "control", "runtime-contract-registry.json"), RUNTIME_CONTRACT_V0337);
    ...
    return { ...modelSafe(legacy), version: this.version, runtimeContract: RUNTIME_CONTRACT_V0337, providerDispatchGate: receipt, bridgeExecutionContext: "jaa-bridge-execution-context-v1", modelDeliveryHandle: RUNTIME_CONTRACT_V0337.modelDeliveryHandleContract, transportProtocol: RUNTIME_CONTRACT_V0337.providerTransport, decisionContract: RUNTIME_CONTRACT_V0337.decisionContract, artifactSubmissionTokenContract: RUNTIME_CONTRACT_V0337.artifactSubmissionTokenContract, modelVisibleIdentityFields: [], hostOwnedIdentity: true };
  }
  ...
  bindProviderTurn(threadId: string, turnId: string) {
    ...
    this.approvedIdentity = sealDispatchExpectedIdentityV0337(this.identityInput);
  ...
  async handle(tool: string, argsValue: unknown, context: AnalysisBridgeToolContext) {
    ...
    try {
      ...
      normalized = normalizeJaaError(error, { stage: this.stageForTool(tool), source: "AnalysisBridgeV0337", fallbackCode: "AI_INPUT_TOOL_FAILED", safeDetails: { callId: context.callId, toolName: tool, argumentNames: Object.keys(args).sort() } });
  ...
  terminate() { if (this.recoverableCursorError) { this.captureRootError(this.recoverableCursorError); this.recoverableCursorError = null; } this.executionContext.terminate(); releaseHostControlLifecycleControllerV0337(this.config.runDirectory); }
  ...
  private manifest(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const manifest = createModelInputManifestV3(RUNTIME_CONTRACT_V0337, sourceManifest, { deliveryHandle, artifactSubmissionToken: this.config.instructionMode === "CUSTOM_DIAGNOSTIC" ? null : this.artifactToken });
    const receipt = createRuntimeManifestReceiptV0337(manifest, true, RUNTIME_CONTRACT_V0337);
    writeRuntimeManifestReceiptV0337(this.config.runDirectory, "runtime-manifest-receipt.json", receipt);
    writeRuntimeManifestReceiptV0337(this.config.runDirectory, "runtime-manifest-self-validation.json", receipt);
    writeRuntimeManifestReceiptV0337(this.config.runDirectory, "model-input-manifest-receipt.json", receipt);
  ...
  serializeToolResponse(tool: string, result: unknown) {
    if (tool === "jaa_get_input_manifest") {
      const response = serializeAndValidateManifestResponseV0337(result, RUNTIME_CONTRACT_V0337);
  ...
  private finalize(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const receipt = this.control.finalize({ bindingHash, receipt: { ...legacyReceipt, transportProtocol: RUNTIME_CONTRACT_V0337.providerTransport } });
  ...
  private publish(args: JsonObject, context: AnalysisBridgeToolContext) {
    ...
    const submission: ArtifactSubmissionV0337 = { artifactSubmissionToken: typeof args.artifactSubmissionToken === "string" ? args.artifactSubmissionToken : "", decisions: args.decisions, analysisReportMarkdown: String(args.analysisReportMarkdown ?? ""), finalSummaryZhTw: String(args.finalSummaryZhTw ?? "") };
    const actualIdentity = injectArtifactActualIdentityV0337(this.identityInput!, identity.createdAtUtc);
    ...
    const reject = (code: string, message: string): never => {
      ...
      const receipt = { ...buildArtifactIdentityReceiptV0337(identity, actualIdentity, code), accepted: false, firstMismatchCode: code, tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
    ...
    const identityReceipt = { ...buildArtifactIdentityReceiptV0337(identity, actualIdentity, "MATCH"), tokenHash, tokenPrefix: this.artifactTokenPrefix, fullTokenPersisted: false };
    ...
    if (decisions.length === identity.sourceRecordCount && decisions.length > 0 && decisions.every((decision) => decision.status === "FAILED")) reject("AI_PROVIDER_RUN_LEVEL_FAILURE", "Provider returned an all-FAILED batch; treat this as a run-level failure.");
    if (decisions.length !== identity.sourceRecordCount || decisions.some((decision, index) => Number(decision.recordIndex) !== index)) reject("AI_ARTIFACT_DECISION_COUNT_INVALID", `Artifact requires exact ordered decisions expected=${identity.sourceRecordCount} actual=${decisions.length}.`);
    ...
    const safeSubmission = { schemaVersion: "jaa-ai-submitted-artifact-v1", state: "AI_SUBMITTED", receivedAtUtc: new Date().toISOString(), artifactSubmissionToken: "[masked]", artifactSubmissionTokenHash: tokenHash, approvedArtifactIdentity: identity, decisions, analysisReportMarkdown: submission.analysisReportMarkdown, finalSummaryZhTw: submission.finalSummaryZhTw };
    const artifactBytes = Buffer.from(JSON.stringify(safeSubmission, null, 2) + "\n", "utf8");
    const persisted = persistProviderArtifactV0337({
      runDirectory: this.config.runDirectory,
      runId: this.config.runId,
      artifactBytes,
      artifactFileName: `ai-submitted-artifact-attempt-${String(this.artifactAttempt).padStart(3, "0")}.json`,
      validateBinding: () => undefined,
      onLifecycleAdvance: (receipt) => { this.control.acceptArtifact(String(receipt.artifactSha256), decisions.length); },
      schedulePostArtifact: () => undefined
    });
    this.artifactLifecycle.transition("persisted");
    durableJson(path.join(this.config.runDirectory, "artifact-attempts", `ai-submitted-artifact-attempt-${String(this.artifactAttempt).padStart(3, "0")}.receipt.json`), { schemaVersion: "jaa-ai-submitted-artifact-receipt-v1", filePath: persisted.artifactPath, bytes: persisted.artifactBytes, sha256: persisted.artifactSha256, state: "AI_SUBMITTED", reopenHashVerified: true, createdAtUtc: persisted.persistedAtUtc });
    ...
    this.artifactReceipt = { schemaVersion: "jaa-artifact-submission-result-v3", status: "persisted", decisionCount: decisions.length, authoritativeDistribution: null, warnings: [], decisionContractVersion: RUNTIME_CONTRACT_V0337.decisionContract, approvedArtifactIdentity: identity, aiSubmittedArtifactSha256: persisted.artifactSha256, artifactBytes: persisted.artifactBytes, identityReceiptPath: identityPath, artifacts, persistedAt: persisted.persistedAtUtc };
  ...
  setProviderTurnStatus(status: AnalysisLifecycleSummary["providerTurnStatus"]) {
    ...
    if (status === "completed" || status === "failed" || status === "interrupted") this.factJournal.append(status === "completed" ? "PROVIDER_COMPLETED" : status === "failed" ? "PROVIDER_FAILED" : "RUN_CANCELLED", "analysis-bridge-v0337", { providerTerminal: status });
  }
  setControlStage(state: HostControlStateV0337, reason: string, receiptId: string | null = null) { return this.control.transition(state, reason, receiptId); }
  setArtifactStatus(status: Extract<ArtifactStateV0337, "content_validated" | "formally_published">) { return this.artifactLifecycle.transition(status); }
  ...
  setPostBridgeStage(stage: "VALIDATION_COMPLETED" | "CANONICAL_ASSEMBLY_COMPLETED" | "RUN_COMPLETED", status: "completed" | "failed") { this.legacy.setPostBridgeStage(stage, status); if (stage === "CANONICAL_ASSEMBLY_COMPLETED" && status === "completed") this.control.transition("CANONICAL_CREATED", "Canonical durable write completed."); else if (stage === "RUN_COMPLETED") { this.factJournal.append("VALIDATION_STAGE_COMPLETED", "analysis-bridge-v0337", { stage, status }); } }
  fail(stage: AnalysisLifecycleStage, code: string, message = code) { const normalized = normalizeJaaError(Object.assign(new Error(`${code}:${message}`), { code }), { stage, source: "AnalysisBridgeV0337", fallbackCode: code }); this.captureRootError(normalized); }
  ...
  snapshot() {
    ...
    return {
      ...
      transportProtocol: RUNTIME_CONTRACT_V0337.providerTransport,
      runtimeContract: RUNTIME_CONTRACT_V0337,
      providerDispatchGate: this.providerDispatchGateReceipt,
      decisionContract: RUNTIME_CONTRACT_V0337.decisionContract,
...
export function createAnalysisBridge(config: Config) { return new AnalysisBridgeV0337(config); }
