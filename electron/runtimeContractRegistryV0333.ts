...
export const RUNTIME_CONTRACT_REGISTRY_VERSION_V0333 = "jaa-runtime-contract-registry-v1" as const;

const registryValue = {
  schemaVersion: RUNTIME_CONTRACT_REGISTRY_VERSION_V0333,
...
export const RUNTIME_CONTRACT_V0333 = deepFreeze({
  ...registryValue,
  registryHash: crypto.createHash("sha256").update(JSON.stringify(registryValue)).digest("hex")
});

export type RuntimeContractV0333 = typeof RUNTIME_CONTRACT_V0333;

export function validateDispatchIdentityV0333(input: {
  ...
  const expected = RUNTIME_CONTRACT_V0333;
  const mismatches = [
