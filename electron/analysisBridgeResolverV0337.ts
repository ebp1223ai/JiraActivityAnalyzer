...
export const BRIDGE_IDENTITY_V0337 = "0.3.37-bridge-v17" as const;
export const BRIDGE_ARTIFACT_V0337 = "analysis-bridge-v0337.cjs" as const;
export const BRIDGE_MANIFEST_V0337 = "analysis-bridge-manifest-v0337.json" as const;
...
export type PackagedBridgeManifestV0337 = {
  schemaVersion: typeof PACKAGED_BRIDGE_SCHEMA;
  bridgeIdentity: typeof BRIDGE_IDENTITY_V0337;
  artifactFileName: typeof BRIDGE_ARTIFACT_V0337;
...
export type BridgePreflightReceiptV0337 = {
  ...
  expectedBridgeIdentity: typeof BRIDGE_IDENTITY_V0337;
...
export type BridgeRuntimeContextV0337 = {
  appIsPackaged: boolean;
  processResourcesPath: string;
  developmentResourcesPath?: string;
  distributionKind?: BridgePreflightReceiptV0337["distributionKind"];
  phase: BridgePreflightReceiptV0337["phase"];
  attemptId?: string | null;
  expectedBytes?: number;
  expectedSha256?: string;
};
...
export function bridgeDistributionKindV0337(appIsPackaged: boolean): BridgePreflightReceiptV0337["distributionKind"] {
...
export function resolveAnalysisBridgeArtifactV0337(context: BridgeRuntimeContextV0337) {
  ...
  const artifactPath = path.join(resourceDirectory, BRIDGE_ARTIFACT_V0337);
  const manifestPath = path.join(resourceDirectory, BRIDGE_MANIFEST_V0337);
  ...
  const receipt: BridgePreflightReceiptV0337 = {
    ...
    distributionKind: context.distributionKind ?? bridgeDistributionKindV0337(context.appIsPackaged),
    processResourcesPath: context.processResourcesPath,
    expectedRelativePath: `${BRIDGE_RESOURCE_DIRECTORY}/${BRIDGE_ARTIFACT_V0337}`,
    resolvedAbsolutePath: null, manifestAbsolutePath: manifestPath,
    expectedBridgeIdentity: BRIDGE_IDENTITY_V0337, observedBridgeIdentity: null,
  ...
  try {
    ...
    let manifest: PackagedBridgeManifestV0337;
    try { manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PackagedBridgeManifestV0337; }
