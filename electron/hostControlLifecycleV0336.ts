...
export const HOST_CONTROL_LIFECYCLE_VERSION_V0336 = "jaa-host-control-lifecycle-v3" as const;
export const ANALYSIS_TELEMETRY_VERSION_V0336 = "jaa-analysis-telemetry-v1" as const;

export type HostControlStateV0336 =
...
export type AnalysisTelemetryStateV0336 =
...
type Transition = {
  sequence: number;
  stateBefore: HostControlStateV0336;
  stateAfter: HostControlStateV0336;
  reason: string;
...
const ORDER: HostControlStateV0336[] = [
  "ATTEMPT_CREATED", "PROVIDER_DISPATCHED", "INPUT_READING", "INPUT_READY",
  "ARTIFACT_RECEIVED", "VALIDATING", "CANONICAL_CREATED",
...
export class HostControlLifecycleControllerV0336 {
  private state: HostControlStateV0336 = "ATTEMPT_CREATED";
  private telemetry: AnalysisTelemetryStateV0336 = "NOT_REPORTED";
  private sequence = 0;
  ...
  transition(next: HostControlStateV0336, reason: string, receiptId: string | null = null, idempotencyKey = `${next}:${receiptId ?? reason}`) {
  ...
  snapshot() {
    return Object.freeze({
      schemaVersion: HOST_CONTROL_LIFECYCLE_VERSION_V0336,
      ...
      analysisTelemetry: { schemaVersion: ANALYSIS_TELEMETRY_VERSION_V0336, state: this.telemetry, lastProgress: this.lastProgress, warnings: [...this.warnings] },
...
const controllers = new Map<string, HostControlLifecycleControllerV0336>();

export function getHostControlLifecycleControllerV0336(runDirectory: string, hooks: { failFinalizePostcondition?: boolean } = {}) {
  const key = path.resolve(runDirectory).toLowerCase();
  const existing = controllers.get(key);
  if (existing) return existing;
  const controller = new HostControlLifecycleControllerV0336(runDirectory, hooks);
  controllers.set(key, controller);
  return controller;
}

export function releaseHostControlLifecycleControllerV0336(runDirectory: string) {
