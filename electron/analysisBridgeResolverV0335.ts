...
export const BRIDGE_IDENTITY_V0335 = "0.3.35-bridge-v15" as const;
export const BRIDGE_ARTIFACT_V0335 = "analysis-bridge-v0335.cjs" as const;
export const BRIDGE_MANIFEST_V0335 = "analysis-bridge-manifest-v0335.json" as const;
...
export type PackagedBridgeManifestV0335 = {
  schemaVersion: typeof PACKAGED_BRIDGE_SCHEMA;
  bridgeIdentity: typeof BRIDGE_IDENTITY_V0335;
  artifactFileName: typeof BRIDGE_ARTIFACT_V0335;
...
export type BridgePreflightReceiptV0335 = {
  ...
  expectedBridgeIdentity: typeof BRIDGE_IDENTITY_V0335;
...
export type BridgeRuntimeContextV0335 = {
  appIsPackaged: boolean;
  processResourcesPath: string;
  developmentResourcesPath?: string;
  distributionKind?: BridgePreflightReceiptV0335["distributionKind"];
  phase: BridgePreflightReceiptV0335["phase"];
  attemptId?: string | null;
  expectedBytes?: number;
  expectedSha256?: string;
};
...
export function bridgeDistributionKindV0335(appIsPackaged: boolean): BridgePreflightReceiptV0335["distributionKind"] {
...
export function resolveAnalysisBridgeArtifactV0335(context: BridgeRuntimeContextV0335) {
  ...
  const artifactPath = path.join(resourceDirectory, BRIDGE_ARTIFACT_V0335);
  const manifestPath = path.join(resourceDirectory, BRIDGE_MANIFEST_V0335);
  ...
  const receipt: BridgePreflightReceiptV0335 = {
    ...
    distributionKind: context.distributionKind ?? bridgeDistributionKindV0335(context.appIsPackaged),
    processResourcesPath: context.processResourcesPath,
    expectedRelativePath: `${BRIDGE_RESOURCE_DIRECTORY}/${BRIDGE_ARTIFACT_V0335}`,
    resolvedAbsolutePath: null, manifestAbsolutePath: manifestPath,
    expectedBridgeIdentity: BRIDGE_IDENTITY_V0335, observedBridgeIdentity: null,
  ...
  try {
    ...
    let manifest: PackagedBridgeManifestV0335;
    try { manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PackagedBridgeManifestV0335; }
    ...
    if (manifest.schemaVersion !== PACKAGED_BRIDGE_SCHEMA || manifest.bridgeIdentity !== BRIDGE_IDENTITY_V0335 || manifest.artifactFileName !== BRIDGE_ARTIFACT_V0335) throw failure("AI_BRIDGE_VERSION_MISMATCH", "Bridge manifest identity or artifact filename does not match v0.3.35.");
