...
import { RuleSelectionTransactionServiceV0335, normalizeRuleSelectionErrorV0335 } from "./aiAnalysisRulesV0335.js";
...
import { preflightAnalysisBridgeV0335 } from "./analysisBridgeLoaderV0319.js";
...
import { RUNTIME_CONTRACT_V0335, validateDispatchIdentityV0335 } from "./runtimeContractRegistryV0335.js";
...
export function registerAiAnalysisIpc() {
  ...
  const ruleSelection = new RuleSelectionTransactionServiceV0335(bundledRulesDirectory, path.join(aiStateDirectory, "rule-selection"));
  ...
  const startupBridgeReceipt = preflightAnalysisBridgeV0335("startup", null).receipt;
  ...
  ipcMain.handle("ai-analysis:use-bundled-rules", async (event) => { try { ruleSelection.useBundled(); rules = ruleSelection.rules; notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0335(error)), snapshot: snapshot() }; } });
  ipcMain.handle("ai-analysis:load-rules", async (event) => { try { rules = ruleSelection.revalidate(); notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0335(error)), snapshot: snapshot() }; } });  ipcMain.handle("ai-analysis:choose-pending", async () => {
  ...
  ipcMain.handle("ai-analysis:start", async (event, payload: { mode: AiAnalyzerMode; datasetId: string; selectedDiffIds: string[]; service?: AiServiceKey; supplementalInstruction?: string; instructionMode?: AiInstructionMode; userAdditionalInstruction?: string; userCustomInstruction?: string; analysisRunId?: string; capacityConfirmation?: boolean }) => {
    ...
    } catch (error) { return rejectAttempt(normalizeRuleSelectionErrorV0335(error)); }
    ...
    let bridgePreflightReceipt: ReturnType<typeof preflightAnalysisBridgeV0335>["receipt"] | null = null;
    if (payload.mode === "CHATGPT") {
      const bridgeResolution = preflightAnalysisBridgeV0335("analysis_start", attempt.analysisAttemptId);
    ...
    try {
      if (payload.mode === "OFFLINE_RULE") {
        ...
        const dispatchIdentityReceipt = validateDispatchIdentityV0335({
