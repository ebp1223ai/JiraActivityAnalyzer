...
import { RuleSelectionTransactionServiceV0336, normalizeRuleSelectionErrorV0336 } from "./aiAnalysisRulesV0336.js";
...
import { preflightAnalysisBridgeV0336 } from "./analysisBridgeLoaderV0319.js";
...
import { RUNTIME_CONTRACT_V0336, validateDispatchIdentityV0336 } from "./runtimeContractRegistryV0336.js";
import { PostArtifactPipelineV0336 } from "./postArtifactPipelineV0336.js";
...
export function registerAiAnalysisIpc() {
  ...
  const ruleSelection = new RuleSelectionTransactionServiceV0336(bundledRulesDirectory, path.join(aiStateDirectory, "rule-selection"));
  ...
  const startupBridgeReceipt = preflightAnalysisBridgeV0336("startup", null).receipt;
  ...
  ipcMain.handle("ai-analysis:use-bundled-rules", async (event) => { try { ruleSelection.useBundled(); rules = ruleSelection.rules; notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0336(error)), snapshot: snapshot() }; } });
  ipcMain.handle("ai-analysis:load-rules", async (event) => { try { rules = ruleSelection.revalidate(); notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0336(error)), snapshot: snapshot() }; } });  ipcMain.handle("ai-analysis:choose-pending", async () => {
  ...
  ipcMain.handle("ai-analysis:start", async (event, payload: { mode: AiAnalyzerMode; datasetId: string; selectedDiffIds: string[]; service?: AiServiceKey; supplementalInstruction?: string; instructionMode?: AiInstructionMode; userAdditionalInstruction?: string; userCustomInstruction?: string; analysisRunId?: string; capacityConfirmation?: boolean }) => {
    ...
    } catch (error) { return rejectAttempt(normalizeRuleSelectionErrorV0336(error)); }
    ...
    let bridgePreflightReceipt: ReturnType<typeof preflightAnalysisBridgeV0336>["receipt"] | null = null;
    if (payload.mode === "CHATGPT") {
      const bridgeResolution = preflightAnalysisBridgeV0336("analysis_start", attempt.analysisAttemptId);
    ...
    let postArtifactPipeline: PostArtifactPipelineV0336 | null = null;
    ...
    try {
      if (payload.mode === "OFFLINE_RULE") {
        ...
        const dispatchIdentityReceipt = validateDispatchIdentityV0336({
        ...
        postArtifactPipeline = new PostArtifactPipelineV0336(stagingFolder, runId, persistedSubmission.submissionSha256);
        ...
        postArtifactPipeline!.runStage("CONTENT_VALIDATE", () => ({ evidencePath: run.validationReportPath, evidenceSha256: run.validationReportPath ? sha256Text(fs.readFileSync(run.validationReportPath, "utf8")) : null }));
        chatgpt.advanceBridgeArtifact(runId, "content_validated");
        postArtifactPipeline!.runStage("FORMAL_PUBLISH", () => ({ evidencePath: path.join(stagingFolder, "progress", "artifact-receipt.json"), evidenceSha256: persistedSubmission.submissionSha256 }));
