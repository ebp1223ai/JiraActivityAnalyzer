...
import { RuleSelectionTransactionServiceV0337, normalizeRuleSelectionErrorV0337 } from "./aiAnalysisRulesV0337.js";
...
import { preflightAnalysisBridgeV0337 } from "./analysisBridgeLoaderV0337.js";
...
import { writeValidationStageReceiptsV0335, type ValidationOutcomeV0335 } from "./analysisValidationTruthV0335.js";
import { RUNTIME_CONTRACT_V0337, validateDispatchIdentityV0337 } from "./runtimeContractRegistryV0337.js";
import { PostArtifactPipelineV0337 } from "./postArtifactPipelineV0337.js";
import { AnalysisProviderRegistryV0337, CHATGPT_CODEX_PROVIDER_ID_V0337, ChatGptCodexProviderAdapterV0337 } from "./analysisProviderAdaptersV0337.js";
import { PROVIDER_REQUEST_CONTRACT_V0337, type JsonValue, type ProviderAnalysisRequestV0337 } from "./providerAnalysisContractsV0337.js";
...
import { inspectRecoveryCandidateV0337, recoverPersistedArtifactV0337, type RecoveryActionsV0337 } from "./recoveryServiceV0337.js";
import { appendTerminalEventV0337, reduceTerminalOutcomeV0337 } from "./terminalOutcomeReducerV0337.js";
...
const handlers = [
  ...
  "ai-analysis:choose-pending", "ai-analysis:verify-pending", "ai-analysis:select-pending", "ai-analysis:choose-analyzed", "ai-analysis:select-run", "ai-analysis:activate-result", "ai-analysis:start", "ai-analysis:reprocess-persisted", "ai-analysis:accept-warnings", "ai-analysis:cancel-capacity-warning", "ai-analysis:cancel", "ai-analysis:review",
...
function durableValidationEvidence(filePath: string | null | undefined) {
...
function writeTerminalValidationReceiptsV0335(input: { run: AiAnalysisRun; runDirectory: string; inputHash: string; findings: Array<{ stage?: string; code?: string }>; activeRegistryPath: string }) {
  const { run } = input;
  const activeCommitted = Boolean(run.analyzedFileSha256 && fs.existsSync(input.activeRegistryPath) && fs.readFileSync(input.activeRegistryPath, "utf8").includes(run.analyzedFileSha256));
  const sqliteCommitted = run.lifecycle?.sqliteStatus === "committed" && Boolean(run.databaseCommitReceiptPath);
  const sqliteFailed = run.lifecycle?.sqliteStatus === "commit_failed";
  const warningPending = run.status === "completed_with_quality_warnings";
  const outcomes: Partial<Record<ValidationStageV0332, ValidationOutcomeV0335>> = {
    TOKEN_BINDING: "PASSED", SUBMISSION_DECODE: "PASSED", DECISION_SCHEMA: "PASSED", COUNT_INDEX_ORDER: "PASSED",
    EVIDENCE_QUOTE_REFERENCE: "PASSED", SOURCE_ROLE_ATTRIBUTION: "PASSED", STATUS_SEMANTIC: "PASSED",
    QUALITY_GATE: run.qualityGate?.status === "WARNING" ? "WARNING" : "PASSED",
    CANONICAL_ASSEMBLY: run.analyzedFilePath ? "PASSED" : "BLOCKED_BY_PRIOR_STAGE",
    ANALYZED_RESULT_PUBLISH: run.analyzedFilePath ? "PASSED" : "BLOCKED_BY_PRIOR_STAGE",
    ACTIVE_RESULT_COMMIT: activeCommitted ? "PASSED" : warningPending ? "BLOCKED_PENDING_WARNING_ACCEPTANCE" : "BLOCKED_BY_PRIOR_STAGE",
    REPORT_PACKAGE: run.reportDataPackagePath ? "PASSED" : "BLOCKED_BY_PRIOR_STAGE",
    HTML_RENDER: run.htmlRenderStatus === "completed" && run.reportFilePath ? "PASSED" : "WARNING",
    SQLITE: sqliteCommitted ? "PASSED" : warningPending ? "BLOCKED_PENDING_WARNING_ACCEPTANCE" : sqliteFailed ? "FAILED" : "BLOCKED_BY_PRIOR_STAGE"
  };
  writeValidationStageReceiptsV0335({
    runDirectory: input.runDirectory, inputHash: input.inputHash, outcomes,
    rootErrorCode: sqliteFailed ? run.progress.errorCode ?? "AI_SQLITE_COMMIT_FAILED" : null,
    findings: input.findings,
    reasons: {
      QUALITY_GATE: run.qualityGate?.status === "WARNING" ? "Quality warning requires durable operator acceptance before SQLite." : "Quality v4 checks completed.",
      CANONICAL_ASSEMBLY: "Canonical Result atomic write, reopen, count, and hash verification completed.",
      ANALYZED_RESULT_PUBLISH: "Analyzed Result durable publish and hash verification completed.",
      ACTIVE_RESULT_COMMIT: activeCommitted ? "Active Result registry commit is durable." : "Active Result was not committed.",
      REPORT_PACKAGE: "Report Data Package durable write and hash verification completed.",
      HTML_RENDER: run.htmlRenderStatus === "completed" ? "Local HTML and renderer receipt are durable." : "Local HTML render did not complete.",
      SQLITE: sqliteCommitted ? "SQLite transaction commit receipt is durable." : warningPending ? "SQLite is blocked pending durable warning acceptance." : "SQLite commit did not complete."
    },
    evidence: {
      CANONICAL_ASSEMBLY: durableValidationEvidence(run.analyzedFilePath), ANALYZED_RESULT_PUBLISH: durableValidationEvidence(run.analyzedFilePath),
      ACTIVE_RESULT_COMMIT: activeCommitted ? durableValidationEvidence(input.activeRegistryPath) : undefined,
      REPORT_PACKAGE: durableValidationEvidence(run.reportDataPackagePath), HTML_RENDER: run.htmlRenderStatus === "completed" ? durableValidationEvidence(run.reportFilePath) : undefined,
      SQLITE: sqliteCommitted ? durableValidationEvidence(run.databaseCommitReceiptPath) : undefined
    }
  });
}
...
function persistRunDebugEvidence(stagingFolder: string, run: AiAnalysisRun, requestSanitized: unknown, responseSanitized: unknown, events: unknown[], actions: string[]) {
  ...
  const submissionObserved = ["submitting", "submission_rejected", "published", "failed"].includes(run.lifecycle?.artifactStatus ?? "not_started") || run.durableArtifactTruth?.status === "PERSISTED_VALID" || fs.existsSync(path.join(stagingFolder, "artifact-attempts"));
  ...
  collectFile(path.join(stagingFolder, "progress", "host-control-lifecycle.json"), "host-control-lifecycle.json", "host_control_lifecycle");
  collectFile(path.join(stagingFolder, "progress", "host-control-transitions.jsonl"), "host-control-transitions.jsonl", "host_control_transitions");
  ...
  collectFile(path.join(stagingFolder, "progress", "provider-adapter-receipt.json"), "provider-adapter-receipt.json", "provider_adapter_selection");
  collectFile(path.join(stagingFolder, "progress", "artifact-receipt.json"), "artifact-receipt.json", "durable_artifact_receipt");
  collectFile(path.join(stagingFolder, "progress", "reducer-facts.jsonl"), "reducer-facts.jsonl", "reducer_fact_journal");
  collectFile(path.join(stagingFolder, "progress", "durable-artifact-truth-resolver-receipt.json"), "durable-artifact-truth-resolver-receipt.json", "durable_artifact_truth_resolver");
  collectFile(path.join(stagingFolder, "progress", "terminal-decision-receipt.json"), "terminal-decision-receipt.json", "terminal_reducer_decision");
  collectFile(path.join(stagingFolder, "progress", "run-terminal.json"), "run-terminal.json", "run_terminal");
  collectDirectory(path.join(stagingFolder, "progress", "post-artifact-stages"), "post-artifact-stages", "post_artifact_stage_receipt");
  collectDirectory(path.join(stagingFolder, "recovery"), "recovery", "recovery_lineage");
  ...
  const submissionPersisted = v0326Evidence.some((entry) => entry.role === "ai_submission_and_identity" && fs.existsSync(entry.source));
  const completeness = canonicalEvidence.map((entry) => {
    ...
    const contradictory = run.durableArtifactTruth?.status === "CONTRADICTORY" || run.durableArtifactTruth?.status === "CORRUPT";
    const classification = contradictory ? "CONTRADICTORY_EVIDENCE" : present ? "expected_and_present" : entry.expected ? "expected_but_missing" : entry.role.includes("diagnostic") ? submissionPersisted ? "submission_received_and_persisted" : modelDeliveryObserved ? "not_produced_due_to_prior_failure" : "not_applicable_no_model_delivery" : run.instructionMode === "CUSTOM_DIAGNOSTIC" ? "not_applicable_instruction_mode" : "not_produced_due_to_prior_failure";
...
export function registerAiAnalysisIpc() {
  ...
  const providerRegistry = new AnalysisProviderRegistryV0337();
  providerRegistry.register(new ChatGptCodexProviderAdapterV0337(
    async (providerRequest) => chatgpt.runAnalysis(providerRequest.options.transportRequest as never, providerRequest.cancellation),
    async (runId, _reason) => chatgpt.cancelRun(runId),
    () => chatgpt.getStatus().state === "connected"
  ));
  ...
  const ruleSelection = new RuleSelectionTransactionServiceV0337(bundledRulesDirectory, path.join(aiStateDirectory, "rule-selection"));
  ...
  const startupBridgeReceipt = preflightAnalysisBridgeV0337("startup", null).receipt;
  ...
  const refreshRecoveryProjection = (run: AiAnalysisRun) => {
    if (!run.runDirectory) return;
    const candidate = inspectRecoveryCandidateV0337(run.runDirectory, run.runId);
    run.durableArtifactTruth = candidate.resolver ? { status: candidate.resolver.status, artifactSha256: candidate.resolver.artifactSha256, artifactBytes: candidate.resolver.artifactBytes, findingCodes: candidate.resolver.findings.map((finding) => String(finding.code ?? "UNKNOWN")) } : null;
    run.postArtifactRecovery = { eligible: candidate.eligible, status: candidate.eligible ? "AVAILABLE" : candidate.reason === "ALREADY_COMPLETE" ? "NOT_REQUIRED" : "FAILED", nextStage: candidate.nextStage, supersessionReceiptPath: run.postArtifactRecovery?.supersessionReceiptPath ?? null, providerContacted: false };
  };
  runs.forEach(refreshRecoveryProjection);
  ...
  const snapshot = (): AiAnalysisSnapshot => ({
    ...
    successfulResults: resultRegistry.successful.filter((result) => !runs.some((run) => run.runId === result.runId && run.quarantineStatus === "QUARANTINED_SYSTEMIC_NO_RESULT")), lastNavigationDecision, providerAdapters: [{ id: CHATGPT_CODEX_PROVIDER_ID_V0337, displayName: "ChatGPT / Bundled Codex", productionSelectable: true }], analysisBridge: startupBridgeReceipt
  });
  ...
  ipcMain.handle("ai-analysis:use-bundled-rules", async (event) => { try { ruleSelection.useBundled(); rules = ruleSelection.rules; notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0337(error)), snapshot: snapshot() }; } });
  ipcMain.handle("ai-analysis:load-rules", async (event) => { try { rules = ruleSelection.revalidate(); notify(event); return { ok: true, snapshot: snapshot() }; } catch (error) { return { ...errorPayload(normalizeRuleSelectionErrorV0337(error)), snapshot: snapshot() }; } });  ipcMain.handle("ai-analysis:choose-pending", async () => {
  ...
  ipcMain.handle("ai-analysis:start", async (event, payload: { mode: AiAnalyzerMode; datasetId: string; selectedDiffIds: string[]; service?: AiServiceKey; supplementalInstruction?: string; instructionMode?: AiInstructionMode; userAdditionalInstruction?: string; userCustomInstruction?: string; analysisRunId?: string; capacityConfirmation?: boolean }) => {
    ...
    } catch (error) { return rejectAttempt(normalizeRuleSelectionErrorV0337(error)); }
    ...
    let bridgePreflightReceipt: ReturnType<typeof preflightAnalysisBridgeV0337>["receipt"] | null = null;
    if (payload.mode === "CHATGPT") {
      const bridgeResolution = preflightAnalysisBridgeV0337("analysis_start", attempt.analysisAttemptId);
    ...
    let postArtifactPipeline: PostArtifactPipelineV0337 | null = null;
    ...
    try {
      if (payload.mode === "OFFLINE_RULE") {
        ...
        const dispatchIdentityReceipt = validateDispatchIdentityV0337({
        ...
        const transportRequest = { runId, requestPurpose: run.instructionMode === "CUSTOM_DIAGNOSTIC" ? "CUSTOM_DIAGNOSTIC" : "FORMAL_ANALYSIS", instructionMode: run.instructionMode, prompt: request.prompt, model: chatgpt.getStatus().selectedModel, deliveryMode: "LOCAL_FILE_WORKSPACE", runDirectory: stagingFolder, workspacePath: request.workspace, outputWorkspacePath: request.output, allowedReadRoots: [request.workspace], finalProviderPayloadSha256: request.requestPackage.finalProviderPayloadSha256, outputSchema: null, outputSchemaSha256: request.requestPackage.outputSchemaSha256, requestPackage: request.requestPackage, rulesSnapshotId: run.rules.snapshotId ?? run.rules.ruleSetId, catalogSkillIds: run.rules.catalog.map((item) => item.id), analysisAttemptId: attempt.analysisAttemptId, requestId: runId, manifestSha256: run.rules.roleDocuments?.find((item) => item.role === "manifest")?.sha256, commonRulesSha256: run.rules.roleDocuments?.find((item) => item.role === "common_rules")?.sha256, catalogSha256: run.rules.roleDocuments?.find((item) => item.role === "catalog")?.sha256, quoteCatalog, evidenceSegments: segmentCatalog.segments };
        const providerRequest: ProviderAnalysisRequestV0337 = {
          schemaVersion: PROVIDER_REQUEST_CONTRACT_V0337,
          runId,
          providerId: CHATGPT_CODEX_PROVIDER_ID_V0337,
          inputBindings: request.requestPackage.documents.map((document) => ({ role: document.role, fileName: document.originalFileName, bytes: document.snapshotByteLength, sha256: document.snapshotSha256, providerVisible: document.modelVisible })),
          effectiveInstruction: { text: request.prompt, bytes: request.requestPackage.finalProviderPayloadBytes, sha256: request.requestPackage.finalProviderPayloadSha256, identity: request.requestPackage.promptIdentity ?? RUNTIME_CONTRACT_V0337.promptIdentity },
          expectedRecordCount: compact.payload.eventCount,
          decisionContract: request.requestPackage.decisionContractVersion ?? RUNTIME_CONTRACT_V0337.decisionContract,
          artifactContract: RUNTIME_CONTRACT_V0337.providerArtifact,
          locale: "zh-TW",
          cancellation: controller.signal,
          options: { transportRequest: transportRequest as unknown as JsonValue }
        };
        const providerAdapter = providerRegistry.require(CHATGPT_CODEX_PROVIDER_ID_V0337) as ChatGptCodexProviderAdapterV0337<Awaited<ReturnType<typeof chatgpt.runAnalysis>>>;
        const providerPreflight = await providerAdapter.preflight(providerRequest);
        if (!providerPreflight.ok) throw new AiAnalysisError("AI_PROVIDER_DISPATCH_BLOCKED", providerPreflight.findingCode ?? "Provider preflight failed.");
        run.providerAdapterId = CHATGPT_CODEX_PROVIDER_ID_V0337;
        run.providerAdapterDisplayName = providerPreflight.capability.displayName;
        durableJsonWriteV0324(path.join(stagingFolder, "progress", "provider-adapter-receipt.json"), { schemaVersion: "jaa-provider-adapter-receipt-v1", runId, providerId: providerAdapter.id, requestContract: providerRequest.schemaVersion, artifactContract: providerRequest.artifactContract, capability: providerPreflight.capability, preflight: { ok: providerPreflight.ok, findingCode: providerPreflight.findingCode }, fallbackAllowed: false, selectedAtUtc: now() });
        const providerPromise = providerAdapter.analyze(providerRequest, { append: (providerEvent) => {
          const factType = providerEvent.type === "prepared" ? "PROVIDER_PREPARED" : providerEvent.type === "sent" ? "PROVIDER_SENT" : providerEvent.type === "accepted" ? "PROVIDER_ACCEPTED" : providerEvent.type === "completed" ? "PROVIDER_COMPLETED" : providerEvent.type === "cancelled" ? "RUN_CANCELLED" : "PROVIDER_FAILED";
          new ReducerFactJournalV0337(stagingFolder, runId).append(factType, "chatgpt-codex-provider-adapter-v0337", { providerId: CHATGPT_CODEX_PROVIDER_ID_V0337, eventType: providerEvent.type });
        } });
        ...
        if (run.instructionMode === "CUSTOM_DIAGNOSTIC") {
          ...
          const finalArtifact = atomicExport(path.join(stagingFolder, "ai-output", "provider-final-assistant-message.txt"), customText + "\n");
        ...
        const hostControlState = String(bridgeSnapshot.hostControlLifecycle?.state ?? bridgeSnapshot.lifecycle.controlState ?? "");
        if (!["ARTIFACT_RECEIVED", "VALIDATING", "CANONICAL_CREATED", "ANALYZED_RESULT_PUBLISHED", "ACTIVE_RESULT_COMMITTED", "REPORT_PACKAGE_CREATED", "HTML_RENDERED", "SQLITE_GATE", "TERMINAL"].includes(hostControlState)) throw new AiAnalysisError("AI_ANALYSIS_INCOMPLETE", `Artifact completion boundary is missing; controlState=${hostControlState || "unavailable"}.`);
        if (!bridgeSnapshot.artifactReceipt || !["persisted", "content_validated", "formally_published", "published"].includes(bridgeSnapshot.lifecycle.artifactStatus)) throw new AiAnalysisError("AI_ARTIFACT_RECEIPT_INVALID", "Durably persisted Artifact Receipt is missing or invalid.");
        ...
        const providerFinal = atomicExport(path.join(stagingFolder, "ai-output", "provider-final-assistant-message.txt"), redactChatGptTextComplete(response.text).trim() + "\n"); run.finalAssistantMessagePath = artifactPaths.finalMessage; const raw = stageVisibleProviderResponse(stagingFolder, response.text); run.providerResponseRawPath = raw.rawFilePath; run.providerResponseGzipPath = raw.filePath; run.providerResponseSha256 = raw.rawSha256; run.providerResponseGzipSha256 = raw.gzipSha256;
        ...
        const truthPaths = discoverArtifactTruthPathsV0337(stagingFolder);
        if (!truthPaths) throw new AiAnalysisError("AI_TERMINAL_FACT_RECONCILIATION_REQUIRED", "Artifact path disappeared after durable submission receipt.");
        const durableTruth = resolveDurableArtifactTruthV0337(runId, truthPaths);
        durableJsonWriteV0324(path.join(stagingFolder, "progress", "durable-artifact-truth-resolver-receipt.json"), durableTruth);
        run.durableArtifactTruth = { status: durableTruth.status, artifactSha256: durableTruth.artifactSha256, artifactBytes: durableTruth.artifactBytes, findingCodes: durableTruth.findings.map((finding) => String(finding.code ?? "UNKNOWN")) };
        if (durableTruth.status !== "PERSISTED_VALID") throw new AiAnalysisError("AI_TERMINAL_FACT_RECONCILIATION_REQUIRED", `Durable Artifact Resolver returned ${durableTruth.status}.`);
        postArtifactPipeline = new PostArtifactPipelineV0337(stagingFolder, runId, persistedSubmission.submissionSha256);
        postArtifactPipeline.runStage("TOKEN_BINDING", () => ({ evidencePath: bridgeSnapshot.artifactReceipt?.identityReceiptPath ? String(bridgeSnapshot.artifactReceipt.identityReceiptPath) : path.join(stagingFolder, "progress", "artifact-receipt.json") }));
        postArtifactPipeline.runStage("SUBMISSION_DECODE", () => ({ evidencePath: persistedSubmission.submissionPath, evidenceSha256: persistedSubmission.submissionSha256 }));
        ...
        if (run.reportFilePath && run.reportFileSha256) postArtifactPipeline!.runStage("HTML_RENDER", () => ({ evidencePath: run.reportFilePath, evidenceSha256: run.reportFileSha256 ?? undefined }));
        ...
        if (!validated.validation.valid || quality.status === "BLOCKED" || systemicGate.decision === "BLOCKED") {
          ...
          const stageChecks = [
            ["DECISION_SCHEMA", validated.validation.schemaValid, "AI_DECISION_SCHEMA_INVALID"],
            ["COUNT_INDEX_ORDER", validated.validation.actualCount === validated.validation.expectedCount, "AI_DECISION_COUNT_MISMATCH"],
            ["EVIDENCE_QUOTE_REFERENCE", validated.validation.evidenceValid, "EVIDENCE_QUOTE_ID_NOT_FOUND"],
            ["SOURCE_ROLE_ATTRIBUTION", validated.validation.attributionValid, "EVIDENCE_QUOTE_ROLE_INELIGIBLE"],
            ["STATUS_SEMANTIC", validated.validation.semanticValid, "AI_DECISION_SEMANTIC_VALIDATION_FAILED"],
            ["QUALITY_GATE", quality.status !== "BLOCKED" && systemicGate.decision !== "BLOCKED", rootCode]
          ] as const;
          for (const [stage, passed, code] of stageChecks) postArtifactPipeline!.runStage(stage, () => { if (!passed) throw Object.assign(new Error(code), { code }); return { evidencePath: run.validationReportPath }; });
        ...
        const validationEvidence = () => ({ evidencePath: run.validationReportPath, evidenceSha256: run.validationReportPath ? sha256Text(fs.readFileSync(run.validationReportPath, "utf8")) : null });
        for (const stage of ["DECISION_SCHEMA", "COUNT_INDEX_ORDER", "EVIDENCE_QUOTE_REFERENCE", "SOURCE_ROLE_ATTRIBUTION", "STATUS_SEMANTIC"] as const) postArtifactPipeline!.runStage(stage, validationEvidence);
        postArtifactPipeline!.runStage("QUALITY_GATE", () => ({ evidencePath: run.qualityGateReportPath, evidenceSha256: run.qualityGateReportPath ? sha256Text(fs.readFileSync(run.qualityGateReportPath, "utf8")) : null }));
        chatgpt.advanceBridgeArtifact(runId, "content_validated");
        chatgpt.advanceBridgeArtifact(runId, "formally_published"); chatgpt.advanceBridgeLifecycle(runId, "VALIDATION_COMPLETED", "completed"); run.bridgeEvidence = chatgpt.getBridgeEvidence(runId); run.lifecycle = run.bridgeEvidence?.lifecycle ?? run.lifecycle ?? null;
        ...
        const canonicalDocument = { schemaName: "JiraActivityAnalyzerAnalyzedActivityEvents", schemaVersion: "0.3.24-v1", canonicalContractVersion: "jaa-canonical-analysis-result-v5", run }; const canonicalArtifact = atomicWriteCanonical(artifactPaths.canonicalResult, canonicalDocument); run.analyzedFilePath = canonicalArtifact.filePath; run.analyzedFileSizeBytes = canonicalArtifact.sizeBytes; run.analyzedFileSha256 = canonicalArtifact.sha256; run.formalArtifactRecordCount = run.results.length; postArtifactPipeline!.runStage("CANONICAL_ASSEMBLY", () => ({ evidencePath: canonicalArtifact.filePath, evidenceSha256: canonicalArtifact.sha256 })); postArtifactPipeline!.runStage("ANALYZED_RESULT_PUBLISH", () => ({ evidencePath: canonicalArtifact.filePath, evidenceSha256: canonicalArtifact.sha256 })); run.telemetry.canonicalAssemblyMs = Date.now() - assemblyStarted; run.telemetry.canonicalOutputCompletedAt = now(); chatgpt.advanceBridgeLifecycle(runId, "CANONICAL_ASSEMBLY_COMPLETED", "completed"); chatgpt.advanceBridgeControl(runId, "ANALYZED_RESULT_PUBLISHED", "Analyzed Result atomic publish and hash verification completed.", canonicalArtifact.sha256); run.bridgeEvidence = chatgpt.getBridgeEvidence(runId); run.lifecycle = run.bridgeEvidence?.lifecycle ?? run.lifecycle ?? null;
        if (systemicGate.activeResultAllowed) { resultRegistry.activateRun(run, "ANALYSIS_RUN"); postArtifactPipeline!.runStage("ACTIVE_RESULT_COMMIT", () => ({ evidencePath: path.join(aiStateDirectory, "results", "successful-result-registry.json"), evidenceSha256: run.analyzedFileSha256 })); chatgpt.advanceBridgeControl(runId, "ACTIVE_RESULT_COMMITTED", "Active Result registry commit completed.", run.analyzedFileSha256 ?? null); }
        selectedCanonicalAiRunDirectory = run.runDirectory ?? null;
        const formalPackage = buildReportDataPackageV0324({ mode: "FORMAL_CANONICAL", run, rules: run.rules, results: run.results, segments: segmentCatalog.segments, issueSnapshots: snapshotProfiles, validationFindings: allFindings, quality, canonicalSha256: canonicalArtifact.sha256, inputReceipts: [run.bridgeEvidence?.sourceInputReceipt, run.bridgeEvidence?.modelDeliveryReceipt].filter(Boolean) }); const formalPackagePath = path.join(artifactPaths.canonical, "report-data-package.json"); const formalWritten = durableJsonWriteV0324(formalPackagePath, formalPackage); run.reportDataPackagePath = formalWritten.filePath; run.reportDataPackageSha256 = formalPackage.packageSha256; run.reportPackageMode = "FORMAL_CANONICAL"; postArtifactPipeline!.runStage("REPORT_PACKAGE", () => ({ evidencePath: formalWritten.filePath, evidenceSha256: formalWritten.sha256 })); chatgpt.advanceBridgeControl(runId, "REPORT_PACKAGE_CREATED", "Report Data Package durable write and hash verification completed.", formalWritten.sha256);
      ...
      let rootErrorCode = (initialBridgeEvidence?.lifecycle.rootErrorCode ?? value.code) as AiAnalysisErrorCode;
      const initialValidationFailure = rootErrorCode.startsWith("AI_DECISION_") || rootErrorCode.startsWith("AI_ARTIFACT_") || rootErrorCode.startsWith("EVIDENCE_QUOTE_") || ["AI_ARTIFACT_RECEIPT_INVALID", "AI_ARTIFACT_PUBLISH_FAILED", "AI_ANALYSIS_REPORT_MISSING", "AI_FINAL_SUMMARY_MISSING", "AI_OUTPUT_PATH_ESCAPE_BLOCKED", "AI_OUTPUT_PUBLISH_INCOMPLETE", "AI_RESULT_COUNT_MISMATCH", "AI_RESULT_IDENTITY_VALIDATION_FAILED"].includes(rootErrorCode);
      if (runStagingFolder && postArtifactPipeline) postArtifactPipeline.completeFailure(value);
      if (runStagingFolder) {
        const truthPaths = discoverArtifactTruthPathsV0337(runStagingFolder);
        if (truthPaths) {
          const terminalDecision = reduceTerminalOutcomeV0337({ runDirectory: runStagingFolder, runId, providerTerminal: rootErrorCode === "RUN_CANCELLED" ? "cancelled" : (run.progress.turnCompletedCount ?? 0) > 0 ? "completed" : "failed", artifactPaths: truthPaths, contentValidationFailed: initialValidationFailure, postArtifactTerminalOutcome: "FAILED" });
          durableJsonWriteV0324(path.join(runStagingFolder, "progress", "durable-artifact-truth-resolver-receipt.json"), terminalDecision.resolver);
          appendTerminalEventV0337(runStagingFolder, runId, terminalDecision);
          run.durableArtifactTruth = { status: terminalDecision.resolver.status, artifactSha256: terminalDecision.resolver.artifactSha256, artifactBytes: terminalDecision.resolver.artifactBytes, findingCodes: terminalDecision.resolver.findings.map((finding) => String(finding.code ?? "UNKNOWN")) };
          run.postArtifactRecovery = { eligible: terminalDecision.resolver.status === "PERSISTED_VALID", status: "AVAILABLE", nextStage: inspectRecoveryCandidateV0337(runStagingFolder, runId).nextStage, supersessionReceiptPath: null, providerContacted: false };
          if (terminalDecision.action === "RECONCILIATION_REQUIRED") rootErrorCode = "AI_TERMINAL_FACT_RECONCILIATION_REQUIRED";
          else if (terminalDecision.resolver.status === "PERSISTED_VALID" && rootErrorCode === "AI_ARTIFACT_SUBMISSION_MISSING") rootErrorCode = "AI_POST_ARTIFACT_PIPELINE_FAILED";
        }
      }
  ...
  ipcMain.handle("ai-analysis:reprocess-persisted", async (event, runId: string) => {
    const run = runs.find((item) => item.runId === runId);
    if (!run?.runDirectory) return { ok: false, errorCode: "RECOVERY_RUN_NOT_FOUND", message: "找不到可恢復的 Run。", snapshot: snapshot() };
    try {
      const paths = discoverArtifactTruthPathsV0337(run.runDirectory);
      if (!paths) throw Object.assign(new Error("已保存的 Artifact 不存在。"), { code: "RECOVERY_ARTIFACT_NOT_FOUND" });
      const truth = resolveDurableArtifactTruthV0337(run.runId, paths);
      if (truth.status !== "PERSISTED_VALID" || !truth.artifactSha256) throw Object.assign(new Error(`Artifact Resolver=${truth.status}`), { code: "AI_TERMINAL_FACT_RECONCILIATION_REQUIRED" });
      run.postArtifactRecovery = { eligible: true, status: "RUNNING", nextStage: inspectRecoveryCandidateV0337(run.runDirectory, run.runId).nextStage, supersessionReceiptPath: null, providerContacted: false };
      notify(event);
      const requireEvidence = (filePath: string | null | undefined, stage: string) => () => { if (!filePath || !fs.existsSync(filePath)) throw Object.assign(new Error(`RECOVERY_STAGE_EVIDENCE_MISSING:${stage}`), { code: "RECOVERY_STAGE_EVIDENCE_MISSING", stage }); const bytes = fs.readFileSync(filePath); return { evidencePath: filePath, evidenceSha256: crypto.createHash("sha256").update(bytes).digest("hex") }; };
      const artifactReceipt = JSON.parse(fs.readFileSync(paths.receiptPath, "utf8")) as Record<string, unknown>;
      const validationEvidence = run.validationReportPath ?? path.join(run.runDirectory, "canonical-output", "validation-report.json");
      const actions: RecoveryActionsV0337 = {
        TOKEN_BINDING: requireEvidence(String(artifactReceipt.identityReceiptPath ?? ""), "TOKEN_BINDING"),
        SUBMISSION_DECODE: requireEvidence(paths.artifactPath, "SUBMISSION_DECODE"),
        DECISION_SCHEMA: requireEvidence(validationEvidence, "DECISION_SCHEMA"), COUNT_INDEX_ORDER: requireEvidence(validationEvidence, "COUNT_INDEX_ORDER"),
        EVIDENCE_QUOTE_REFERENCE: requireEvidence(validationEvidence, "EVIDENCE_QUOTE_REFERENCE"), SOURCE_ROLE_ATTRIBUTION: requireEvidence(validationEvidence, "SOURCE_ROLE_ATTRIBUTION"), STATUS_SEMANTIC: requireEvidence(validationEvidence, "STATUS_SEMANTIC"),
        QUALITY_GATE: requireEvidence(run.qualityGateReportPath, "QUALITY_GATE"), CANONICAL_ASSEMBLY: requireEvidence(run.analyzedFilePath, "CANONICAL_ASSEMBLY"), ANALYZED_RESULT_PUBLISH: requireEvidence(run.analyzedFilePath, "ANALYZED_RESULT_PUBLISH"),
        ACTIVE_RESULT_COMMIT: () => { if (resultRegistry.active?.runId !== run.runId) throw Object.assign(new Error("RECOVERY_WOULD_REPLACE_NEWER_ACTIVE_RESULT"), { code: "RECOVERY_WOULD_REPLACE_NEWER_ACTIVE_RESULT" }); return requireEvidence(path.join(aiStateDirectory, "results", "successful-result-registry.json"), "ACTIVE_RESULT_COMMIT")(); },
        REPORT_PACKAGE: requireEvidence(run.reportDataPackagePath, "REPORT_PACKAGE"), HTML_RENDER: requireEvidence(run.reportFilePath, "HTML_RENDER"), SQLITE: requireEvidence(run.databaseCommitReceiptPath, "SQLITE")
      };
      const recovered = recoverPersistedArtifactV0337({ runDirectory: run.runDirectory, runId: run.runId, actions, sqliteIsolation: false, oldTerminalPath: path.join(run.runDirectory, "progress", "run-terminal.json") });
      run.postArtifactRecovery = { eligible: false, status: "SUCCEEDED", nextStage: null, supersessionReceiptPath: recovered.supersessionReceiptPath, providerContacted: false };
      refreshRecoveryProjection(run); notify(event); return { ok: true, recovery: recovered, snapshot: snapshot() };
    } catch (error) {
      const value = asAnalysisError(error); run.postArtifactRecovery = { eligible: true, status: "FAILED", nextStage: inspectRecoveryCandidateV0337(run.runDirectory, run.runId).nextStage, supersessionReceiptPath: null, providerContacted: false }; notify(event); return { ...errorPayload(value), snapshot: snapshot() };
    }
