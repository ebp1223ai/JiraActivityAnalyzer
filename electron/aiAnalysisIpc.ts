...
function renderExistingCanonical(run: AiAnalysisRun, templatePath?: string) {
  ...
  try {
    const result = renderReportDataPackageHtmlV0324({ reportDataPackagePath: run.reportDataPackagePath, templatePath: templatePath ?? run.htmlTemplateSnapshotPath!, outputDirectory: path.join(run.runDirectory, "canonical-output") });
    run.reportFilePath = result.receipt.outputHtmlPath; run.reportFileSizeBytes = result.receipt.outputHtmlSizeBytes; run.reportFileSha256 = result.receipt.outputHtmlSha256; run.htmlRenderReceiptPath = result.receiptPath; run.htmlRenderDiagnosticsPath = null; run.htmlRendererVersion = HTML_RENDERER_VERSION_V0324; run.htmlRenderStatus = "completed"; getChatGptService().setBridgeHtmlRenderStatus(run.runId, "completed"); if (run.lifecycle) run.lifecycle.htmlRenderStatus = "completed";
    return result;
  } catch (error) { run.htmlRenderStatus = "failed"; getChatGptService().setBridgeHtmlRenderStatus(run.runId, "failed"); if (run.lifecycle) run.lifecycle.htmlRenderStatus = "failed"; throw error; }
}
...
export function registerAiAnalysisIpc() {
  ...
  ipcMain.handle("ai-analysis:start", async (event, payload: { mode: AiAnalyzerMode; datasetId: string; selectedDiffIds: string[]; service?: AiServiceKey; supplementalInstruction?: string; instructionMode?: AiInstructionMode; userAdditionalInstruction?: string; userCustomInstruction?: string; analysisRunId?: string; capacityConfirmation?: boolean }) => {
    ...
    try {
      if (payload.mode === "OFFLINE_RULE") {
        ...
        run.decisionContractVersion = "jaa-ai-analysis-decisions-v4";
        ...
        const modelPayload = modelPayloadWithSegmentsV0324(compact.payload, segmentCatalog); const modelPayloadPath = path.join(derivedFolder, "model-analysis-payload.json");
        ...
        run.htmlTemplateSnapshotPath = frozenTemplatePath; run.htmlRendererVersion = HTML_RENDERER_VERSION_V0324; run.htmlRenderStatus = "not_started";
        ...
        if (!validated.validation.valid || quality.status === "BLOCKED") {
          ...
          const first = allFindings.find((finding) => finding.severity === "ERROR"); throw new AiAnalysisError("AI_RESPONSE_SEMANTIC_VALIDATION_FAILED", first ? `${first.code} ${first.jsonPointer}: ${first.message}` : "Aggregated v0.3.24 validation failed.");
        }
        ...
        run.progress.completedBatches = 0; run.progress.message = warningGate.requiresHumanAcceptance ? "Analysis completed with warnings. Review the report and explicitly accept warnings before SQLite write." : "Decision v4, Canonical v5, Report Data Package, offline HTML, and validation evidence completed.";
      } else {
  ...
  ipcMain.handle("ai-analysis:rerender-html", async (event, runId: string) => {
    ...
    try { const result = renderExistingCanonical(run, choice.filePaths[0]); runArchives.get(runId)?.writeManifest(run); notify(event); return { ok: true, receipt: result.receipt, snapshot: snapshot() }; } catch (error) { runArchives.get(runId)?.writeManifest(run); notify(event); return { ...errorPayload(error), snapshot: snapshot() }; }
  });
  ipcMain.handle("ai-analysis:render-external-package", async (event) => {
    ...
    try { const outputDirectory = ensureDir(path.join(getExportsDir(), "ai-analysis", "external-package-renders")); const result = renderReportDataPackageHtmlV0324({ reportDataPackagePath: packageChoice.filePaths[0], templatePath: templateChoice.filePaths[0], outputDirectory }); return { ok: true, externalViewOnly: true, sqliteEligible: false, receipt: result.receipt, filePath: result.receipt.outputHtmlPath }; } catch (error) { return errorPayload(error); }
  });
